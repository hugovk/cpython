...
#define _Py_FREELIST_FREE(NAME, op, freefunc) \
    _PyFreeList_Free(                         \
        &_Py_freelists_GET()->NAME,           \
        _PyObject_CAST(op),                   \
        Py_##NAME##_MAXFREELIST,              \
        freefunc                              \
    )
...
// freelist is empty.
#define _Py_FREELIST_POP_MEM(NAME) _PyFreeList_PopMem(&_Py_freelists_GET()->NAME)
...
static inline void
_PyFreeList_Free(
    struct _Py_freelist *fl, void *obj, Py_ssize_t maxsize, freefunc dofree
) {
