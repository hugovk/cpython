...
#define IS_WITHIN_OPCODE_RANGE(opcode) \
    (((opcode) >= 0 && (opcode) <= MAX_REAL_OPCODE) || IS_PSEUDO_INSTR(opcode))

#define IS_BLOCK_PUSH_OPCODE(opcode) \
    ((opcode) == SETUP_FINALLY || (opcode) == SETUP_WITH || (opcode) == SETUP_CLEANUP)

#define HAS_TARGET(opcode) (OPCODE_HAS_JUMP(opcode) || IS_BLOCK_PUSH_OPCODE(opcode))
...
#define IS_UNCONDITIONAL_JUMP_OPCODE(opcode)                                          \
    ((opcode) == JUMP || (opcode) == JUMP_NO_INTERRUPT || (opcode) == JUMP_FORWARD || \
     (opcode) == JUMP_BACKWARD || (opcode) == JUMP_BACKWARD_NO_INTERRUPT)
