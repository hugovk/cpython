...
_Py_IsMainInterpreterFinalizing(PyInterpreterState *interp) {
    ...
    return (
        _PyRuntimeState_GetFinalizing(&_PyRuntime) != NULL &&
...
extern int
_PyThreadState_IsRunningMain(PyThreadState *);
extern void
_PyInterpreterState_ReinitRunningMain(PyThreadState *);
...
#define _Py_EnsureTstateNotNULL(tstate) _Py_EnsureFuncTstateNotNULL(__func__, (tstate))
...
PyAPI_FUNC(PyThreadState *) _PyThreadState_New(PyInterpreterState *interp, int whence);
extern void
...
#define HEAD_UNLOCK(runtime) PyMutex_Unlock(&(runtime)->interpreters.mutex)
