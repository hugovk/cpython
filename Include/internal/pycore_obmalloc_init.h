...
#define PT_8(p, start)                                                  \
    PT(p, start), PT(p, start + 1), PT(p, start + 2), PT(p, start + 3), \
        PT(p, start + 4), PT(p, start + 5), PT(p, start + 6), PT(p, start + 7)

#if NB_SMALL_SIZE_CLASSES <= 8
#define _obmalloc_pools_INIT(p) {PT_8(p, 0)}
#elif NB_SMALL_SIZE_CLASSES <= 16
#define _obmalloc_pools_INIT(p) {PT_8(p, 0), PT_8(p, 8)}
#elif NB_SMALL_SIZE_CLASSES <= 24
#define _obmalloc_pools_INIT(p) {PT_8(p, 0), PT_8(p, 8), PT_8(p, 16)}
#elif NB_SMALL_SIZE_CLASSES <= 32
#define _obmalloc_pools_INIT(p) {PT_8(p, 0), PT_8(p, 8), PT_8(p, 16), PT_8(p, 24)}
