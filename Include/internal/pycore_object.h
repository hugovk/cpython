...
#define _PyObject_HEAD_INIT(type) \
    {.ob_ref_local = _Py_IMMORTAL_REFCNT_LOCAL, .ob_type = (type)}
#else
#define _PyObject_HEAD_INIT(type) \
    {.ob_refcnt = _Py_IMMORTAL_INITIAL_REFCNT, .ob_type = (type)}
#endif
#define _PyVarObject_HEAD_INIT(type, size) \
    {.ob_base = _PyObject_HEAD_INIT(type), .ob_size = size}

PyAPI_FUNC(void) _Py_NO_RETURN
    _Py_FatalRefcountErrorFunc(const char *func, const char *message);
...
extern void
_Py_AddRefTotal(PyThreadState *, Py_ssize_t);
extern void
_Py_IncRefTotal(PyThreadState *);
extern void
_Py_DecRefTotal(PyThreadState *);

#define _Py_DEC_REFTOTAL(interp) interp->object_state.reftotal--
#endif
...
static inline void
_Py_SetMortal(PyObject *op, Py_ssize_t refcnt) {
    if (op) {
...
extern int
_PyDict_CheckConsistency(PyObject *mp, int check_content);
...
extern int
_PyTraceMalloc_TraceRef(PyObject *op, PyRefTracerEvent event, void *);
...
// is called.
#ifdef NDEBUG
#define _PyObject_GC_TRACK(op) _PyObject_GC_TRACK(_PyObject_CAST(op))
#define _PyObject_GC_UNTRACK(op) _PyObject_GC_UNTRACK(_PyObject_CAST(op))
#else
...
_Py_TryIncRefShared(PyObject *op) {
    ...
    for (;;) {
        ...
        if (_Py_atomic_compare_exchange_ssize(
                &op->ob_ref_shared, &shared, shared + (1 << _Py_REF_SHARED_SHIFT)
            )) {
#ifdef Py_REF_DEBUG
            _Py_IncRefTotal(_PyThreadState_GET());
#endif
...
#ifdef Py_REF_DEBUG
extern void
_PyInterpreterState_FinalizeRefTotal(PyInterpreterState *);
extern void
_Py_FinalizeRefTotal(_PyRuntimeState *);
extern void
...
extern void
_Py_PrintReferences(PyInterpreterState *, FILE *);
extern void
_Py_PrintReferenceAddresses(PyInterpreterState *, FILE *);
#endif
...
_PyObject_GET_WEAKREFS_LISTPTR(PyObject *op) {
    if (PyType_Check(op) &&
        ...
        managed_static_type_state *state =
            _PyStaticType_GetState(interp, (PyTypeObject *)op);
...
_PyObject_GET_WEAKREFS_LISTPTR_FROM_OFFSET(PyObject *op) {
    assert(!PyType_Check(op) || ((PyTypeObject *)op)->tp_flags & Py_TPFLAGS_HEAPTYPE);
    Py_ssize_t offset = Py_TYPE(op)->tp_weaklistoffset;
...
_PyObject_IS_GC(PyObject *obj) {
    ...
    return (_PyType_IS_GC(type) && (type->tp_is_gc == NULL || type->tp_is_gc(obj)));
}
...
_PyObject_HashFast(PyObject *op) {
    if (PyUnicode_CheckExact(op)) {
        Py_hash_t hash = FT_ATOMIC_LOAD_SSIZE_RELAXED(_PyASCIIObject_CAST(op)->hash);
        if (hash != -1) {
            return hash;
        }
    }
...
static inline int
_PyType_SUPPORTS_WEAKREFS(PyTypeObject *type) {
...
extern PyObject *
_PyType_GetTextSignatureFromInternalDoc(const char *, const char *, int);
extern int
...
extern PyObject **
_PyObject_ComputedDictPointer(PyObject *);
extern int
_PyObject_IsInstanceDictEmpty(PyObject *);

// Export for 'math' shared extension
PyAPI_FUNC(PyObject *) _PyObject_LookupSpecial(PyObject *, PyObject *);
PyAPI_FUNC(PyObject *) _PyObject_LookupSpecialMethod(
    PyObject *self, PyObject *attr, PyObject **self_or_null
);

extern int
_PyObject_IsAbstract(PyObject *);
