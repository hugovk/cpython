...
#define _PyStatus_ERR(ERR_MSG)                                       \
    (PyStatus) {                                                     \
        ._type = _PyStatus_TYPE_ERROR, .func = _PyStatus_GET_FUNC(), \
        .err_msg = (ERR_MSG)                                         \
    }
/* other fields are set to 0 */
...
#define _PyStatus_IS_ERROR(err) ((err)._type == _PyStatus_TYPE_ERROR)
#define _PyStatus_IS_EXIT(err) ((err)._type == _PyStatus_TYPE_EXIT)
#define _PyStatus_EXCEPTION(err) ((err)._type != _PyStatus_TYPE_OK)
#define _PyStatus_UPDATE_FUNC(err)         \
    do {                                   \
        (err).func = _PyStatus_GET_FUNC(); \
    } while (0)
...
#define _PyWideStringList_INIT \
    (PyWideStringList) { .length = 0, .items = NULL }
...
#define _PyPreCmdline_INIT \
    (_PyPreCmdline){.use_environment = -1, .isolated = -1, .dev_mode = -1}
...
extern PyStatus
_PyPreConfig_InitFromPreConfig(PyPreConfig *preconfig, const PyPreConfig *config2);
extern PyObject *
_PyPreConfig_AsDict(const PyPreConfig *preconfig);
extern void
_PyPreConfig_GetConfig(PyPreConfig *preconfig, const PyConfig *config);
extern PyStatus
_PyPreConfig_Read(PyPreConfig *preconfig, const _PyArgv *args);
extern PyStatus
_PyPreConfig_Write(const PyPreConfig *preconfig);
...
extern PyStatus
_PyConfig_Write(const PyConfig *config, struct pyruntimestate *runtime);
extern PyStatus
