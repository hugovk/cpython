...
typedef struct _PyExecutorObject {
    PyObject_VAR_HEAD const _PyUOpInstruction *trace;
...
/* Should return > 0 if a new executor is created. O if no executor is produced and < 0
...
struct _PyOptimizerObject {
    PyObject_HEAD _Py_optimize_func optimize;
...
void
_Py_ExecutorInit(_PyExecutorObject *, const _PyBloomFilter *);
void
_Py_ExecutorDetach(_PyExecutorObject *);
void
_Py_BloomFilter_Init(_PyBloomFilter *);
void
_Py_BloomFilter_Add(_PyBloomFilter *bloom, void *obj);
...
#ifdef _Py_TIER2
PyAPI_FUNC(void) _Py_Executors_InvalidateDependency(
...
uop_get_target(const _PyUOpInstruction *inst) {
    assert(inst->format == UOP_FORMAT_TARGET);
    return inst->target;
}

static inline uint16_t
uop_get_jump_target(const _PyUOpInstruction *inst) {
    assert(inst->format == UOP_FORMAT_JUMP);
    return inst->jump_target;
}

static inline uint16_t
uop_get_error_target(const _PyUOpInstruction *inst) {
    assert(inst->format != UOP_FORMAT_TARGET);
...
PyAPI_FUNC(int) _PyOptimizer_Optimize(
    struct _PyInterpreterFrame *frame,
    _Py_CODEUNIT *start,
...
is_terminator(const _PyUOpInstruction *uop) {
    ...
    return (opcode == _EXIT_TRACE || opcode == _JUMP_TO_TOP || opcode == _DYNAMIC_EXIT);
}
