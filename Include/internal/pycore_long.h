...
extern PyStatus
_PyLong_InitTypes(PyInterpreterState *);
extern void
...
static inline PyObject *
_PyLong_FromUnsignedChar(unsigned char i) {
...
extern PyObject *
_PyLong_FromBytes(const char *, Py_ssize_t, int);
...
_PyLong_SetDigitCount(PyLongObject *op, Py_ssize_t size) {
    ...
    op->long_value.lv_tag =
        (((size_t)size) << NON_SIZE_BITS) | (op->long_value.lv_tag & SIGN_MASK);
}
...
#define _PyLong_DIGIT_INIT(val)                                           \
    {                                                                     \
        .ob_base = _PyObject_HEAD_INIT(&PyLong_Type), .long_value = {     \
            .lv_tag = TAG_FROM_SIGN_AND_SIZE(                             \
                (val) == 0 ? 0 : ((val) < 0 ? -1 : 1), (val) == 0 ? 0 : 1 \
            ),                                                            \
            {((val) >= 0 ? (val) : -(val))},                              \
        }                                                                 \
    }
