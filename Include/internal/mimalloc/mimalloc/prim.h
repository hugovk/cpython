...
typedef struct mi_os_mem_config_s {
    ...
    bool must_free_whole;  // must allocated blocks be freed as a whole (false for mmap,
                           // true for VirtualAlloc)
    bool has_virtual_reserve;  // supports virtual address space reservation? (if true
                               // we can reserve virtual address space without using
...
// The `try_alignment` is just a hint and the returned pointer does not have to be
// aligned. If `commit` is false, the virtual memory range only needs to be reserved
// (with no access) which will later be committed explicitly using `_mi_prim_commit`.
...
// Decommit memory. Returns error code or 0 on success. The `needs_recommit` result is
// true if the memory would need to be re-committed. For example, on Windows this is
// always true, but on Linux we could use MADV_DONTNEED to decommit which does not need
// a recommit. pre: needs_recommit != NULL
int
...
// Called on the first thread start, and should ensure `_mi_thread_done` is called on
// thread termination.
void
_mi_prim_thread_init_auto_done(void);

// Called on process exit and may take action to clean up resources associated with the
// thread auto done.
void
...
// otherwise use portable C, taking the address of a thread local variable (this is
// still very fast on most platforms).
static inline mi_threadid_t
_mi_prim_thread_id(void) mi_attr_noexcept {
...
__thread local variable (`_mi_heap_default`). With the initial-exec TLS model this
ensures that the storage will always be available (allocated on the thread stacks).

On some platforms though we cannot use that when overriding `malloc` since the
underlying TLS implementation (or the loader) will call itself `malloc` on a first
access and recurse. We try to circumvent this in an efficient way:
- macOSX : we use an unused TLS slot from the OS allocated slots (MI_TLS_SLOT). On OSX,
the loader itself calls `malloc` even before the modules are initialized.
...
// use end bytes of a name; goes wrong if anyone uses names > 23 characters (ptrhread
// specifies 16) see
// <https://github.com/openbsd/src/blob/master/lib/libc/include/thread_private.h#L371>
#define MI_TLS_PTHREAD_SLOT_OFS (6 * sizeof(int) + 4 * sizeof(void *) + 24)
// #elif defined(__DragonFly__)
// #warning "mimalloc is not working correctly on DragonFly yet."
// #define MI_TLS_PTHREAD_SLOT_OFS   (4 + 1*sizeof(void*))  // offset `uniqueid` (also
// used by gdb?)
...
mi_prim_get_default_heap(void) {
    mi_heap_t *heap =
        (mi_unlikely(_mi_heap_default_key == (pthread_key_t)(-1))
             ? _mi_heap_main_get()
