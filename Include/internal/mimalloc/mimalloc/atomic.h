...
// We base the primitives on the C/C++ atomics and create a minimal wrapper for MSVC in
// C compilation mode. This is why we try to use only `uintptr_t` and `<type>*` as
// atomic types. To gain better insight in the range of used atomics, we use explicitly
// named memory order operations instead of passing the memory order as a parameter.
// -----------------------------------------------------------------------------------------------
...
#define mi_atomic_cas_weak(p, expected, desired, mem_success, mem_fail) \
    mi_atomic(compare_exchange_weak_explicit)(                          \
        p, expected, desired, mem_success, mem_fail                     \
    )

#define mi_atomic_cas_strong(p, expected, desired, mem_success, mem_fail) \
    mi_atomic(compare_exchange_strong_explicit)(                          \
        p, expected, desired, mem_success, mem_fail                       \
    )

#define mi_atomic_load_acquire(p) mi_atomic(load_explicit)(p, mi_memory_order(acquire))
#define mi_atomic_load_relaxed(p) mi_atomic(load_explicit)(p, mi_memory_order(relaxed))
#define mi_atomic_store_release(p, x) \
    mi_atomic(store_explicit)(p, x, mi_memory_order(release))
#define mi_atomic_store_relaxed(p, x) \
    mi_atomic(store_explicit)(p, x, mi_memory_order(relaxed))
...
#define mi_atomic_cas_strong_release(p, exp, des)                       \
    mi_atomic_cas_strong(                                               \
        p, exp, des, mi_memory_order(release), mi_memory_order(relaxed) \
    )
#define mi_atomic_cas_strong_acq_rel(p, exp, des)                       \
    mi_atomic_cas_strong(                                               \
        p, exp, des, mi_memory_order(acq_rel), mi_memory_order(acquire) \
    )

#define mi_atomic_add_relaxed(p, x) \
    mi_atomic(fetch_add_explicit)(p, x, mi_memory_order(relaxed))
#define mi_atomic_sub_relaxed(p, x) \
    mi_atomic(fetch_sub_explicit)(p, x, mi_memory_order(relaxed))
#define mi_atomic_add_acq_rel(p, x) \
    mi_atomic(fetch_add_explicit)(p, x, mi_memory_order(acq_rel))
#define mi_atomic_sub_acq_rel(p, x) \
    mi_atomic(fetch_sub_explicit)(p, x, mi_memory_order(acq_rel))
#define mi_atomic_and_acq_rel(p, x) \
    mi_atomic(fetch_and_explicit)(p, x, mi_memory_order(acq_rel))
#define mi_atomic_or_acq_rel(p, x) \
    mi_atomic(fetch_or_explicit)(p, x, mi_memory_order(acq_rel))
...
// In C++/C11 atomics we have polymorphic atomics so can use the typed `ptr` variants
// (where `tp` is the type of atomic value) We use these macros so we can provide a
// typed wrapper in MSVC in C compilation mode as well
...
#define mi_atomic_cas_ptr_weak_release(tp, p, exp, des) \
    mi_atomic_cas_weak_release(p, exp, (tp *)des)
#define mi_atomic_cas_ptr_weak_acq_rel(tp, p, exp, des) \
    mi_atomic_cas_weak_acq_rel(p, exp, (tp *)des)
...
mi_atomic_addi64_relaxed(volatile int64_t *p, int64_t add) {
    return mi_atomic(fetch_add_explicit)(
        (_Atomic(int64_t) *)p, add, mi_memory_order(relaxed)
    );
}
static inline void
mi_atomic_maxi64_relaxed(volatile int64_t *p, int64_t x) {
    ...
    while (
        current < x && !mi_atomic_cas_weak_release((_Atomic(int64_t) *)p, &current, x)
    ) { /* nothing */
    };
}

// Used by timers
#define mi_atomic_loadi64_acquire(p) \
    mi_atomic(load_explicit)(p, mi_memory_order(acquire))
#define mi_atomic_loadi64_relaxed(p) \
    mi_atomic(load_explicit)(p, mi_memory_order(relaxed))
...
mi_atomic_fetch_add_explicit(_Atomic(uintptr_t) *p, uintptr_t add, mi_memory_order mo) {
    (void)(mo);
    return (uintptr_t
    )MI_64(_InterlockedExchangeAdd)((volatile msc_intptr_t *)p, (msc_intptr_t)add);
}
static inline uintptr_t
mi_atomic_fetch_sub_explicit(_Atomic(uintptr_t) *p, uintptr_t sub, mi_memory_order mo) {
    (void)(mo);
    return (uintptr_t
    )MI_64(_InterlockedExchangeAdd)((volatile msc_intptr_t *)p, -((msc_intptr_t)sub));
}
static inline uintptr_t
mi_atomic_fetch_and_explicit(_Atomic(uintptr_t) *p, uintptr_t x, mi_memory_order mo) {
    (void)(mo);
    return (uintptr_t
    )MI_64(_InterlockedAnd)((volatile msc_intptr_t *)p, (msc_intptr_t)x);
}
static inline uintptr_t
mi_atomic_fetch_or_explicit(_Atomic(uintptr_t) *p, uintptr_t x, mi_memory_order mo) {
    (void)(mo);
    return (uintptr_t
    )MI_64(_InterlockedOr)((volatile msc_intptr_t *)p, (msc_intptr_t)x);
}
static inline bool
mi_atomic_compare_exchange_strong_explicit(
    ...
    (void)(mo1);
    (void)(mo2);
    uintptr_t read = (uintptr_t)MI_64(_InterlockedCompareExchange)(
        (volatile msc_intptr_t *)p, (msc_intptr_t)desired, (msc_intptr_t)(*expected)
    );
    if (read == *expected) {
        return true;
    } else {
...
static inline bool
mi_atomic_compare_exchange_weak_explicit(
    _Atomic(uintptr_t) *p,
    uintptr_t *expected,
    uintptr_t desired,
    mi_memory_order mo1,
...
static inline uintptr_t
mi_atomic_exchange_explicit(
    _Atomic(uintptr_t) *p, uintptr_t exchange, mi_memory_order mo
) {
    (void)(mo);
    return (uintptr_t
    )MI_64(_InterlockedExchange)((volatile msc_intptr_t *)p, (msc_intptr_t)exchange);
}
static inline void
...
mi_atomic_casi64_strong_acq_rel(
    volatile _Atomic(int64_t *) p, int64_t *exp, int64_t des
) {
...
#define mi_atomic_load_ptr_acquire(tp, p) \
    (tp *)mi_atomic_load_acquire((_Atomic(uintptr_t) *)(p))
#define mi_atomic_load_ptr_relaxed(tp, p) \
    (tp *)mi_atomic_load_relaxed((_Atomic(uintptr_t) *)(p))
...
#define mi_atomic_loadi64_acquire(p) \
    mi_atomic(loadi64_explicit)(p, mi_memory_order(acquire))
#define mi_atomic_loadi64_relaxed(p) \
    mi_atomic(loadi64_explicit)(p, mi_memory_order(relaxed))
#define mi_atomic_storei64_release(p, x) \
...
#define mi_atomic_guard(guard)                                                   \
    ...
    for (bool _mi_guard_once = true;                                             \
         _mi_guard_once &&                                                       \
         mi_atomic_cas_strong_acq_rel(guard, &_mi_guard_expected, (uintptr_t)1); \
...
mi_atomic_yield(void) {
    _mm_pause();
        ...
        (defined(__x86_64__) || defined(__i386__) || defined(__arm__) ||     \
         defined(__armel__) || defined(__ARMEL__) || defined(__aarch64__) || \
