...
void
_mi_random_split(mi_random_ctx_t *ctx, mi_random_ctx_t *new_ctx);
uintptr_t
...
bool
_mi_preloading(void);  // true while the C runtime is not initialized yet
mi_threadid_t
_mi_thread_id(void) mi_attr_noexcept;
mi_heap_t *
_mi_heap_main_get(void);  // statically allocated main backing heap
void
...
// os.c
void
_mi_os_init(void);  // called from process init
void *
_mi_os_alloc(size_t size, mi_memid_t *memid, mi_stats_t *stats);
void
_mi_os_free(void *p, size_t size, mi_memid_t memid, mi_stats_t *stats);
void
_mi_os_free_ex(
    void *p, size_t size, bool still_committed, mi_memid_t memid, mi_stats_t *stats
);
...
bool
_mi_os_commit(void *p, size_t size, bool *is_zero, mi_stats_t *stats);
bool
...
_mi_segment_try_reclaim_abandoned(
    mi_heap_t *heap, bool try_all, mi_segments_tld_t *tld
);
void
...
void
_mi_page_reclaim(mi_heap_t *heap, mi_page_t *page);  // callback from segments

size_t
_mi_bin_size(uint8_t bin);  // for stats
uint8_t
_mi_bin(size_t size);  // for stats

// "heap.c"
void
_mi_heap_init_ex(
    mi_heap_t *heap, mi_tld_t *tld, mi_arena_id_t arena_id, bool no_reclaim, uint8_t tag
);
void
...
void
_mi_heap_area_init(mi_heap_area_t *area, mi_page_t *page);
bool
_mi_heap_area_visit_blocks(
    const mi_heap_area_t *area, mi_page_t *page, mi_block_visit_fun *visitor, void *arg
);
...
_mi_free_generic(const mi_segment_t *segment, mi_page_t *page, bool is_local, void *p)
    mi_attr_noexcept;  // for runtime integration
void
_mi_padding_shrink(
...
/* -----------------------------------------------------------
  ...
  All are recoverable but EFAULT is a serious error and aborts by default in secure
mode. For portability define undefined error codes using common Unix codes:
...
#include <string.h>
// initialize a local variable to zero; use memset as compilers optimize constant sized
// memset's
...
#define MI_MUL_NO_OVERFLOW ((size_t)1 << (4 * sizeof(size_t)))  // sqrt(SIZE_MAX)
    *total = count * size;
    ...
    return (
        (size >= MI_MUL_NO_OVERFLOW || count >= MI_MUL_NO_OVERFLOW) && size > 0 &&
        (SIZE_MAX / size) < count
    );
}
#endif
...
mi_count_size_overflow(size_t count, size_t size, size_t *total) {
    if (count ==
        1) {  // quick check for the case where count is one (common for C++ allocators)
        *total = size;
        return false;
...
extern const mi_heap_t _mi_heap_empty;  // read-only empty heap, initial value of the
...
// MI_SEGMENT_SIZE), and we need align "down" to the segment info which is
// `MI_SEGMENT_SIZE` bytes before it; therefore we align one byte before `p`.
...
mi_page_thread_free(const mi_page_t *page) {
    return (mi_block_t *)(mi_atomic_load_relaxed(&((mi_page_t *)page)->xthread_free) &
                          ~3);
}
...
static inline void
mi_block_set_nextx(
    const void *null, mi_block_t *block, const mi_block_t *next, const uintptr_t *keys
) {
...
mi_block_next(const mi_page_t *page, const mi_block_t *block) {
    ...
    if mi_unlikely (next != NULL && !mi_is_in_same_page(block, next)) {
        _mi_error_message(
            EFAULT,
            "corrupted free list entry of size %zub at %p: value 0x%zx\n",
...
mi_ctz32(uint32_t x) {
    ...
    static const unsigned char debruijn[32] = {0,  1,  28, 2,  29, 14, 24, 3,
...
mi_clz32(uint32_t x) {
    // de Bruijn multiplication, see
    // <http://supertech.csail.mit.edu/papers/debruijn.pdf>
    static const uint8_t debruijn[32] = {31, 22, 30, 21, 18, 10, 29, 2,  20, 17, 15,
...
// "bit scan reverse": Return index of the highest bit (or MI_INTPTR_BITS if `x` is
// zero)
static inline size_t
mi_bsr(uintptr_t x) {
