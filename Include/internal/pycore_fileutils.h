...
PyAPI_FUNC(int) _Py_set_inheritable(int fd, int inheritable, int *atomic_flag_works);
...
#define DECODE_LOCALE_ERR(NAME, LEN) \
    ((LEN) == (size_t)-2) ? _PyStatus_ERR("cannot decode " NAME) : _PyStatus_NO_MEMORY()
...
extern wchar_t *
_Py_normpath_and_size(wchar_t *path, Py_ssize_t size, Py_ssize_t *length);
...
extern HRESULT
PathCchSkipRoot(const wchar_t *pszPath, const wchar_t **ppszRootEnd);
...
PyAPI_FUNC(char *)
    _Py_UniversalNewlineFgetsWithSize(char *, int, FILE *, PyObject *, size_t *);

extern int
_PyFile_Flush(PyObject *);
