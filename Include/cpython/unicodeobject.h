...
static inline int
Py_UNICODE_IS_SURROGATE(Py_UCS4 ch) {
    return (0xD800 <= ch && ch <= 0xDFFF);
}
static inline int
Py_UNICODE_IS_HIGH_SURROGATE(Py_UCS4 ch) {
    return (0xD800 <= ch && ch <= 0xDBFF);
}
static inline int
Py_UNICODE_IS_LOW_SURROGATE(Py_UCS4 ch) {
    return (0xDC00 <= ch && ch <= 0xDFFF);
}

// Join two surrogate characters and return a single Py_UCS4 value.
static inline Py_UCS4
Py_UNICODE_JOIN_SURROGATES(Py_UCS4 high, Py_UCS4 low) {
...
static inline Py_UCS4
Py_UNICODE_HIGH_SURROGATE(Py_UCS4 ch) {
...
#define _PyASCIIObject_CAST(op) \
    (assert(PyUnicode_Check(op)), _Py_CAST(PyASCIIObject *, (op)))
#define _PyCompactUnicodeObject_CAST(op) \
    (assert(PyUnicode_Check(op)), _Py_CAST(PyCompactUnicodeObject *, (op)))
#define _PyUnicodeObject_CAST(op) \
    (assert(PyUnicode_Check(op)), _Py_CAST(PyUnicodeObject *, (op)))
...
static inline void
PyUnicode_WRITE(int kind, void *data, Py_ssize_t index, Py_UCS4 value) {
    assert(index >= 0);
...
#define PyUnicode_WRITE(kind, data, index, value) \
    PyUnicode_WRITE(                              \
        _Py_STATIC_CAST(int, kind),               \
        _Py_CAST(void *, data),                   \
        (index),                                  \
        _Py_STATIC_CAST(Py_UCS4, value)           \
    )

/* Read a code point from the string's canonical representation.  No checks
   or ready calls are performed. */
static inline Py_UCS4
PyUnicode_READ(int kind, const void *data, Py_ssize_t index) {
    assert(index >= 0);
    if (kind == PyUnicode_1BYTE_KIND) {
...
#define PyUnicode_READ(kind, data, index)                                        \
    PyUnicode_READ(                                                              \
        _Py_STATIC_CAST(int, kind), _Py_STATIC_CAST(const void *, data), (index) \
    )
...
static inline Py_UCS4
PyUnicode_MAX_CHAR_VALUE(PyObject *op) {
    int kind;
...
#define PyUnicode_MAX_CHAR_VALUE(op) PyUnicode_MAX_CHAR_VALUE(_PyObject_CAST(op))
...
/* For backward compatibility */
static inline int
PyUnicode_READY(PyObject *Py_UNUSED(op)) {
    return 0;
}
...
PyAPI_FUNC(int) PyUnicodeWriter_WriteChar(PyUnicodeWriter *writer, Py_UCS4 ch);
PyAPI_FUNC(int
...
PyAPI_FUNC(int) PyUnicodeWriter_WriteWideChar(
    PyUnicodeWriter *writer, const wchar_t *str, Py_ssize_t size
);
PyAPI_FUNC(int)
    PyUnicodeWriter_WriteUCS4(PyUnicodeWriter *writer, Py_UCS4 *str, Py_ssize_t size);

PyAPI_FUNC(int) PyUnicodeWriter_WriteStr(PyUnicodeWriter *writer, PyObject *obj);
PyAPI_FUNC(int) PyUnicodeWriter_WriteRepr(PyUnicodeWriter *writer, PyObject *obj);
...
/* Append a Unicode character.
   Return 0 on success, raise an exception and return -1 on error. */
PyAPI_FUNC(int) _PyUnicodeWriter_WriteChar(_PyUnicodeWriter *writer, Py_UCS4 ch);

/* Append a Unicode string.
   Return 0 on success, raise an exception and return -1 on error. */
PyAPI_FUNC(int) _PyUnicodeWriter_WriteStr(
    _PyUnicodeWriter *writer, PyObject *str /* Unicode string */
);
...
PyAPI_FUNC(int) _PyUnicode_IsUppercase(Py_UCS4 ch /* Unicode character */
);

PyAPI_FUNC(int) _PyUnicode_IsTitlecase(Py_UCS4 ch /* Unicode character */
);

PyAPI_FUNC(int) _PyUnicode_IsWhitespace(const Py_UCS4 ch /* Unicode character */
);

PyAPI_FUNC(int) _PyUnicode_IsLinebreak(const Py_UCS4 ch /* Unicode character */
);

PyAPI_FUNC(Py_UCS4) _PyUnicode_ToLowercase(Py_UCS4 ch /* Unicode character */
);

PyAPI_FUNC(Py_UCS4) _PyUnicode_ToUppercase(Py_UCS4 ch /* Unicode character */
);

PyAPI_FUNC(Py_UCS4) _PyUnicode_ToTitlecase(Py_UCS4 ch /* Unicode character */
);

PyAPI_FUNC(int) _PyUnicode_ToDecimalDigit(Py_UCS4 ch /* Unicode character */
);

PyAPI_FUNC(int) _PyUnicode_ToDigit(Py_UCS4 ch /* Unicode character */
);

PyAPI_FUNC(double) _PyUnicode_ToNumeric(Py_UCS4 ch /* Unicode character */
);
...
Py_UNICODE_ISALNUM(Py_UCS4 ch) {
    return (
        Py_UNICODE_ISALPHA(ch) || Py_UNICODE_ISDECIMAL(ch) || Py_UNICODE_ISDIGIT(ch) ||
