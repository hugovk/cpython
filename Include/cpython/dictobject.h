...
typedef struct {
     ...
     * Bits 32-63 are a unique id in the free threading build (used for per-thread
     * refcounting)
     */
...
PyAPI_FUNC(int) PyDict_SetDefaultRef(
    PyObject *mp, PyObject *key, PyObject *default_value, PyObject **result
);

/* Get the number of items of a dictionary. */
static inline Py_ssize_t
PyDict_GET_SIZE(PyObject *op) {
    PyDictObject *mp;
...
// In clear/dealloc case, key and new_value will be NULL. Otherwise, new_value will be
// the new value for key, NULL if key is being deleted.
