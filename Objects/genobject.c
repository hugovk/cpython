...
static PyObject *
gen_close(PyObject *, PyObject *);
static PyObject *
async_gen_asend_new(PyAsyncGenObject *, PyObject *);
static PyObject *
async_gen_athrow_new(PyAsyncGenObject *, PyObject *);

#define _PyGen_CAST(op) _Py_CAST(PyGenObject *, (op))
#define _PyCoroObject_CAST(op) \
    (assert(PyCoro_CheckExact(op)), _Py_CAST(PyCoroObject *, (op)))
#define _PyAsyncGenObject_CAST(op) _Py_CAST(PyAsyncGenObject *, (op))

static const char *NON_INIT_CORO_MSG =
    "can't send non-None value to a "
    "just-started coroutine";

static const char *ASYNC_GEN_IGNORED_EXIT_MSG = "async generator ignored GeneratorExit";
...
static PySendResult
gen_send_ex2(
    PyGenObject *gen, PyObject *arg, PyObject **presult, int exc, int closing
) {
...
PyDoc_STRVAR(close_doc, "close() -> raise GeneratorExit inside generator.");
...
static PyObject *
_gen_throw(
    PyGenObject *gen, int close_on_genexit, PyObject *typ, PyObject *val, PyObject *tb
) {
    PyObject *yf = _PyGen_yf(gen);
...
_PyGen_SetStopIterationValue(PyObject *value) {
    ...
    if (value == NULL || (!PyTuple_Check(value) && !PyExceptionInstance_Check(value))) {
...
gen_repr(PyObject *self) {
    PyGenObject *gen = _PyGen_CAST(self);
    return PyUnicode_FromFormat("<generator object %S at %p>", gen->gi_qualname, gen);
}
...
static PyGetSetDef gen_getsetlist[] = {
    {"__name__", gen_get_name, gen_set_name, PyDoc_STR("name of the generator")},
    {"__qualname__",
     gen_get_qualname,
     gen_set_qualname,
...
PyDoc_STRVAR(sizeof__doc__, "gen.__sizeof__() -> size of gen in memory, in bytes");
...
#define _PyCoroWrapper_CAST(op) \
    (assert(Py_IS_TYPE((op), &_PyCoroWrapper_Type)), _Py_CAST(PyCoroWrapper *, (op)))
...
coro_repr(PyObject *self) {
    ...
    return PyUnicode_FromFormat("<coroutine object %S at %p>", coro->cr_qualname, coro);
}
...
static PyGetSetDef coro_getsetlist[] = {
    {"__name__", gen_get_name, gen_set_name, PyDoc_STR("name of the coroutine")},
    {"__qualname__",
     gen_get_qualname,
     gen_set_qualname,
     PyDoc_STR("qualified name of the coroutine")},
    {"cr_await", coro_get_cr_await, NULL, PyDoc_STR("object being awaited on, or None")
    },
...
PyDoc_STRVAR(coro_close_doc, "close() -> raise GeneratorExit inside coroutine.");
...
static PyMethodDef coro_wrapper_methods[] = {
    ...
    {"throw", _PyCFunction_CAST(coro_wrapper_throw), METH_FASTCALL, coro_throw_doc},
...
compute_cr_origin(int origin_depth, _PyInterpreterFrame *current_frame) {
    ...
    for (int i = 0; i < frame_count; ++i) {
        ...
        PyObject *frameinfo =
            Py_BuildValue("OiO", code->co_filename, line, code->co_name);
        if (!frameinfo) {
            Py_DECREF(cr_origin);
            return NULL;
        }
...
#define _PyAsyncGenASend_CAST(op) _Py_CAST(PyAsyncGenASend *, (op))

typedef struct PyAsyncGenAThrow {
    PyObject_HEAD PyAsyncGenObject *agt_gen;
...
typedef struct _PyAsyncGenWrappedValue {
    PyObject_HEAD PyObject *agw_val;
...
async_gen_repr(PyObject *self) {
    ...
    return PyUnicode_FromFormat("<async_generator object %S at %p>", o->ag_qualname, o);
}
...
static PyGetSetDef async_gen_getsetlist[] = {
    {"__name__", gen_get_name, gen_set_name, PyDoc_STR("name of the async generator")},
    {"__qualname__",
     gen_get_qualname,
     gen_set_qualname,
     PyDoc_STR("qualified name of the async generator")},
    {"ag_await", coro_get_cr_await, NULL, PyDoc_STR("object being awaited on, or None")
    },
...
static PyMemberDef async_gen_memberlist[] = {
    {"ag_running", Py_T_BOOL, offsetof(PyAsyncGenObject, ag_running_async), Py_READONLY
    },
    {NULL} /* Sentinel */
};

PyDoc_STRVAR(async_aclose_doc, "aclose() -> raise GeneratorExit inside generator.");

PyDoc_STRVAR(async_asend_doc, "asend(v) -> send 'v' in generator.");
...
PyTypeObject PyAsyncGen_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type, 0) "async_generator", /* tp_name */
...
PyAsyncGen_New(PyFrameObject *f, PyObject *name, PyObject *qualname) {
    PyAsyncGenObject *ag;
    ag = (PyAsyncGenObject *)gen_new_with_qualname(&PyAsyncGen_Type, f, name, qualname);
    if (ag == NULL) {
        return NULL;
    }
...
async_gen_asend_send(PyObject *self, PyObject *arg) {
    ...
    if (o->ags_state == AWAITABLE_STATE_CLOSED) {
        PyErr_SetString(
            PyExc_RuntimeError, "cannot reuse already awaited __anext__()/asend()"
        );
        return NULL;
    }
    ...
    if (o->ags_state == AWAITABLE_STATE_INIT) {
        if (o->ags_gen->ag_running_async) {
            ...
            PyErr_SetString(
                PyExc_RuntimeError, "anext(): asynchronous generator is already running"
            );
            return NULL;
        }
...
PyTypeObject _PyAsyncGenASend_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type, 0) "async_generator_asend", /* tp_name */
...
async_gen_athrow_send(PyObject *self, PyObject *arg) {
    ...
    if (o->agt_state == AWAITABLE_STATE_CLOSED) {
        PyErr_SetString(
            PyExc_RuntimeError, "cannot reuse already awaited aclose()/athrow()"
        );
        return NULL;
    }
    ...
    if (o->agt_state == AWAITABLE_STATE_INIT) {
        ...
        if (o->agt_args == NULL) {
            ...
            if (!PyArg_UnpackTuple(o->agt_args, "athrow", 1, 3, &typ, &val, &tb)) {
                return NULL;
            }
...
async_gen_athrow_close(PyObject *self, PyObject *args) {
    ...
    PyObject *result = async_gen_athrow_throw((PyObject *)agt, &PyExc_GeneratorExit, 1);
    if (result == NULL) {
        if (PyErr_ExceptionMatches(PyExc_StopIteration) ||
            PyErr_ExceptionMatches(PyExc_StopAsyncIteration) ||
            PyErr_ExceptionMatches(PyExc_GeneratorExit)) {
...
static PyMethodDef async_gen_athrow_methods[] = {
    ...
    {"throw", _PyCFunction_CAST(async_gen_athrow_throw), METH_FASTCALL, throw_doc},
...
PyTypeObject _PyAsyncGenAThrow_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type, 0) "async_generator_athrow", /* tp_name */
