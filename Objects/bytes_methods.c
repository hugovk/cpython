...
_Py_bytes_isspace(const char *cptr, Py_ssize_t len) {
    const unsigned char *p = (const unsigned char *)cptr;
...
_Py_bytes_maketrans(Py_buffer *frm, Py_buffer *to) {
    ...
    if (frm->len != to->len) {
        PyErr_Format(PyExc_ValueError, "maketrans arguments must have same length");
        return NULL;
    }
...
Py_LOCAL_INLINE(Py_ssize_t) find_internal(
    const char *str,
    Py_ssize_t len,
    ...
    if (end - start < sub_len)
        ...
        if (dir > 0)
            res = stringlib_find_slice(str, len, sub, sub_len, start, end);
        else
            res = stringlib_rfind_slice(str, len, sub, sub_len, start, end);
    }
...
PyObject *
_Py_bytes_find(
    const char *str, Py_ssize_t len, PyObject *sub, Py_ssize_t start, Py_ssize_t end
) {
...
PyObject *
_Py_bytes_count(
    const char *str, Py_ssize_t len, PyObject *sub_obj, Py_ssize_t start, Py_ssize_t end
) {
    const char *sub;
    Py_ssize_t sub_len;
    char byte;
...
_Py_bytes_contains(const char *str, Py_ssize_t len, PyObject *arg) {
    ...
    if (ival == -1 && PyErr_Occurred()) {
        ...
        if (PyObject_GetBuffer(arg, &varg, PyBUF_SIMPLE) != 0)
            return -1;
        pos = stringlib_find(str, len, varg.buf, varg.len, 0);
...
_Py_bytes_startswith(
    const char *str, Py_ssize_t len, PyObject *subobj, Py_ssize_t start, Py_ssize_t end
) {
