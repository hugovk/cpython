...
static PyMethodDef ellipsis_methods[] = {
    {"__reduce__", ellipsis_reduce, METH_NOARGS, NULL}, {NULL, NULL}
};
...
PySlice_New(PyObject *start, PyObject *stop, PyObject *step) {
    ...
    return (PyObject *)_PyBuildSlice_Consume2(Py_NewRef(start), Py_NewRef(stop), step);
}
...
PySlice_Unpack(PyObject *_r, Py_ssize_t *start, Py_ssize_t *stop, Py_ssize_t *step) {
    ...
    static_assert(
        PY_SSIZE_T_MIN + 1 <= -PY_SSIZE_T_MAX, "-PY_SSIZE_T_MAX < PY_SSIZE_T_MIN + 1"
    );
...
slice_indices(PySliceObject *self, PyObject *len) {
    ...
    if (_PyLong_IsNegative((PyLongObject *)length)) {
        PyErr_SetString(PyExc_ValueError, "length should not be negative");
        Py_DECREF(length);
        return NULL;
    }
