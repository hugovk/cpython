...
PyFile_GetLine(PyObject *f, int n) {
    ...
    if (result != NULL && !PyBytes_Check(result) && !PyUnicode_Check(result)) {
...
PyObject_AsFileDescriptor(PyObject *o) {
    ...
    if (PyLong_Check(o)) {
        ...
        PyErr_SetString(
            PyExc_TypeError, "argument must be an int, or have a fileno() method."
        );
        return -1;
    }
    ...
    if (fd < 0) {
        PyErr_Format(
            PyExc_ValueError, "file descriptor cannot be a negative integer (%i)", fd
        );
        return -1;
    }
    return fd;
...
_Py_UniversalNewlineFgetsWithSize(
    char *buf, int n, FILE *stream, PyObject *fobj, size_t *size
) {
    char *p = buf;
    int c;
...
PyFile_NewStdPrinter(int fd) {
    ...
    self = PyObject_New(PyStdPrinter_Object, &PyStdPrinter_Type);
    if (self != NULL) {
        self->fd = fd;
    }
...
stdprinter_repr(PyStdPrinter_Object *self) {
    return PyUnicode_FromFormat("<stdprinter(fd=%d) object at %p>", self->fd, self);
}
...
stdprinter_isatty(PyStdPrinter_Object *self, PyObject *Py_UNUSED(ignored)) {
    ...
    Py_BEGIN_ALLOW_THREADS res = isatty(self->fd);
...
PyFile_SetOpenCodeHook(Py_OpenCodeHookFunction hook, void *userData) {
    ...
    if (_PyRuntime.open_code_hook) {
        if (Py_IsInitialized()) {
            PyErr_SetString(
                PyExc_SystemError, "failed to change existing open_code hook"
            );
        }
        return -1;
    }
