...
mi_aligned_alloc(size_t alignment, size_t size) mi_attr_noexcept {
    ...
    // <https://en.cppreference.com/w/c/memory/aligned_alloc>. unfortunately, it turns
    // out quite some programs pass a size that is not an integral multiple so skip this
    // check..
    /* if mi_unlikely((size & (alignment - 1)) != 0) { // C11 requires alignment>0 &&
      integral multiple, see <https://en.cppreference.com/w/c/memory/aligned_alloc> #if
      MI_DEBUG > 0 _mi_error_message(EOVERFLOW, "(mi_)aligned_alloc requires the size to
      be an integral multiple of the alignment (size %zu, alignment %zu)\n", size,
      alignment); #endif return NULL;
      }
    */
...
mi_reallocarray(void *p, size_t count, size_t size) mi_attr_noexcept {  // BSD
...
mi_reallocarr(void *p, size_t count, size_t size) mi_attr_noexcept {  // NetBSD
    mi_assert(p != NULL);
    if (p == NULL) {
        errno = EINVAL;
        return EINVAL;
...
void *
mi__expand(void *p, size_t newsize) mi_attr_noexcept {  // Microsoft
...
#else
    unsigned short *p =
        (unsigned short *)_wgetenv((const wchar_t *)name);  // msvc warning 4996
    if (p == NULL) {
        *buf = NULL;
    } else {
        *buf = mi_wcsdup(p);
