...
#include <sys/sysctl.h>
    ...
    !defined(_AIX) && !defined(__OpenBSD__) && !defined(__FreeBSD__) &&   \
...
//------------------------------------------------------------------------------------
// Use syscalls for some primitives to allow for libraries that override open/read/close
// etc. and do allocation themselves; using syscalls prevents recursion when mimalloc is
// still initializing (issue #713)
//------------------------------------------------------------------------------------

#if defined(MI_HAS_SYSCALL_H) && defined(SYS_open) && defined(SYS_close) && \
    defined(SYS_read) && defined(SYS_access)
...
#elif !defined(__APPLE__) && !defined(_AIX) && !defined(__OpenBSD__) && \
    !defined(__FreeBSD__) && !defined(__sun) &&                         \
    !defined(__NetBSD__)  // avoid unused warnings
...
unix_detect_overcommit(void) {
    ...
    if (fd >= 0) {
        ...
        // 0: heuristic overcommit, 1: always overcommit, 2: never overcommit (ignore
        // NORESERVE)
        if (nread >= 1) {
...
_mi_prim_mem_init(mi_os_mem_config_t *config) {
    ...
    config->has_virtual_reserve = true;  // todo: check if this true for NetBSD?  (for
                                         // anonymous mmap with PROT_NONE)
}
...
static void *
unix_mmap_prim(
    void *addr, size_t size, size_t try_alignment, int protect_flags, int flags, int fd
) {
    ...
    if (addr == NULL && try_alignment > 1 &&
        ...
        if (((size_t)1 << n) == try_alignment && n >= 12 &&
            ...
            if (p == MAP_FAILED || !_mi_is_aligned(p, try_alignment)) {
                int err = errno;
                _mi_verbose_message(
                    "unable to directly request aligned OS memory (error: %d (0x%x), "
    ...
    if (addr == NULL && try_alignment > 1 &&
        (try_alignment % _mi_os_page_size()) == 0) {
        p = mmap(
            (void *)try_alignment, size, protect_flags, flags | MAP_ALIGN, fd, 0
        );  // addr parameter is the required alignment
        if (p != MAP_FAILED)
            return p;
        // fall back to regular mmap
    }
    ...
    // on 64-bit systems, use the virtual address area after 2TiB for 4MiB aligned
    // allocations
    if (addr == NULL) {
...
#endif
                _mi_verbose_message(
                    "unable to directly request hinted aligned OS memory (error: %d "
...
unix_mmap_fd(void) {
    ...
    // macOS: tracking anonymous page with a specific ID. (All up to 98 are taken
    // officially but LLVM sanitizers had taken 99)
...
#define MAP_NORESERVE 0
    ...
    if ((large_only || _mi_os_use_large_page(size, try_alignment)) && allow_large) {
        ...
        if (!large_only && try_ok > 0) {
            ...
            // enough permission, the `mmap` will always fail (but it might also fail
            // for other reasons). Therefore, once a large page allocation failed, we
            // don't try again for `large_page_try_ok` times to avoid too many failing
            // calls to mmap.
            mi_atomic_cas_strong_acq_rel(&large_page_try_ok, &try_ok, try_ok - 1);
        } else {
            int lflags =
                flags &
                ~MAP_NORESERVE;  // using NORESERVE on huge pages seems to fail on Linux
            int lfd = fd;
            ...
            if (large_only || lflags != flags) {
                ...
                *is_large = true;
                p = unix_mmap_prim(
                    addr, size, try_alignment, protect_flags, lflags, lfd
                );
#ifdef MAP_HUGE_1GB
                if (p == NULL && (lflags & MAP_HUGE_1GB) != 0) {
                    mi_huge_pages_available = false;  // don't try huge 1GiB pages again
                    _mi_warning_message(
                        "unable to allocate huge (1GiB) page, trying large (2MiB) "
    ...
    if (p == NULL) {
        *is_large = false;
        ...
        if (p != NULL) {
            ...
            // transparent huge pages (THP). Generally, it is not required to call
            // `madvise` with MADV_HUGE though since properly aligned allocations will
            // already use large pages if available in that case -- in particular for
            // our large regions (in `memory.c`). However, some systems only allow THP
            // if called with explicit `madvise`, so when large OS pages are enabled for
            // mimalloc, we call `madvise` anyways.
...
// Note: the `try_alignment` is just a hint and the returned pointer is not guaranteed
// to be aligned.
int
_mi_prim_alloc(
    size_t size,
    ...
    *addr = unix_mmap(
        NULL, size, try_alignment, protect_flags, false, allow_large, is_large
    );
...
unix_mprotect_hint(int err) {
    ...
    if (err == ENOMEM) {
        _mi_warning_message(
            ...
            "  On Linux this is controlled by the vm.max_map_count -- maybe increase "
            "it?\n"
...
#endif
    /*
    // decommit: use mmap with MAP_FIXED and PROT_NONE to discard the existing memory
    (and reduce rss) *needs_recommit = true; const int fd = unix_mmap_fd(); void* p =
    mmap(start, size, PROT_NONE, (MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS |
    MAP_NORESERVE), fd, 0); if (p != start) { err = errno; }
    */
    return err;
}
...
// will not reduce the `rss` stats in tools like `top` even though the memory is
// available to other processes. With the default `MIMALLOC_PURGE_DECOMMITS=1` we ensure
// that by default `MADV_DONTNEED` is used though.
...
_mi_prim_alloc_huge_os_pages(
    ...
    *is_zero = true;
    *addr = unix_mmap(
        hint_addr, size, MI_SEGMENT_SIZE, PROT_READ | PROT_WRITE, true, true, &is_large
    );
    if (*addr != NULL && numa_node >= 0 &&
        numa_node < 8 * MI_INTPTR_SIZE) {  // at most 64 nodes
        unsigned long numa_mask = (1UL << numa_node);
        // TODO: does `mbind` work correctly for huge OS pages? should we
        // use `set_mempolicy` before calling mmap instead?
        // see: <https://lkml.org/lkml/2017/2/9/875>
        long err = mi_prim_mbind(
            *addr, size, MPOL_PREFERRED, &numa_mask, 8 * MI_INTPTR_SIZE, 0
        );
        if (err != 0) {
            err = errno;
            _mi_warning_message(
...
_mi_prim_numa_node_count(void) {
    ...
    for (node = 0; node < 256; node++) {
        // enumerate node entries -- todo: it there a more efficient way to do this?
        // (but ensure there is no allocation)
...
_mi_prim_numa_node(void) {
    ...
    int policy;
    if (cpuset_getdomain(
            CPU_LEVEL_CPUSET, CPU_WHICH_PID, -1, sizeof(dom), &dom, &policy
        ) == -1)
        return 0ul;
...
size_t
_mi_prim_numa_node(void) {
    // TODO: DragonFly does not seem to provide any userland means to get this
    // information.
    return 0ul;
}
...
#else
// fallback: use standard C `getenv` but this cannot be used while initializing the C
// runtime
bool
_mi_prim_getenv(const char *name, char *result, size_t result_size) {
...
bool
_mi_prim_random_buf(void *buf, size_t buf_len) {
// Modern Linux provides `getrandom` but different distributions either use
// `sys/random.h` or `linux/random.h` and for the latter the actual `getrandom` call is
// not always defined. (see
// <https://stackoverflow.com/questions/45237324/why-doesnt-getrandom-compile>) We
// therefore use a syscall directly and fall back dynamically to /dev/urandom when
// needed.
...
#define GRND_NONBLOCK (1)
    ...
    if (mi_atomic_load_acquire(&no_getrandom) == 0) {
        ...
        if (errno != ENOSYS)
            return false;
        mi_atomic_store_release(
            &no_getrandom, (uintptr_t)1
        );  // don't call again, and fall back to /dev/urandom
    }
#endif
    int flags = O_RDONLY;
