...
static long mi_max_error_count =
    16;  // stop outputting errors after this (use < 0 for no limit)
static long mi_max_warning_count =
    16;  // stop outputting warnings after this (use < 0 for no limit)
...
// stable options
    ...
    {1, UNINIT, MI_OPTION(eager_commit)
    ...
    },  // eager commit arena's? 2 is used to enable this only on an OS that has
        // overcommit (i.e. linux)
    ...
    {0, UNINIT, MI_OPTION_LEGACY(allow_large_os_pages, large_os_pages)
    },  // use large OS pages, use only with eager commit to prevent fragmentation of
        // VMA's
    ...
    {0, UNINIT, MI_OPTION_LEGACY(abandoned_page_purge, abandoned_page_reset)
    ...
    },  // the first N segments per thread are not eagerly committed (but per page in
    ...
    {0, UNINIT, MI_OPTION(limit_os_alloc)
    ...
    {8, UNINIT, MI_OPTION(max_segment_reclaim)
    },  // max. number of segment reclaims from the abandoned segments per try.
    {0, UNINIT, MI_OPTION(destroy_on_exit)
    },  // release all OS memory on process exit; careful with dangling pointer or
        // after-exit frees!
#if (MI_INTPTR_SIZE > 4)
    {1024L * 1024L, UNINIT, MI_OPTION(arena_reserve)
...
_mi_options_init(void) {
    ...
    // (e.g. do not call this from process_init as that may run before CRT
    // initialization)
...
mi_option_get_size(mi_option_t option) {
    mi_assert_internal(
        option == mi_option_reserve_os_memory || option == mi_option_arena_reserve
    );
    long x = mi_option_get(option);
...
mi_out_buf_flush(mi_output_fun *out, bool no_more_buf, void *arg) {
    ...
    size_t count =
        mi_atomic_add_acq_rel(&out_len, (no_more_buf ? MI_MAX_DELAY_OUTPUT : 1));
...
// Should be atomic but gives errors on many platforms as generally we cannot cast a
// function pointer to a uintptr_t. For now, don't register output from multiple
// threads.
...
mi_register_output(mi_output_fun *out, void *arg) mi_attr_noexcept {
    mi_out_default =
        (out == NULL ? &mi_out_stderr : out);  // stop using the delayed output buffer
...
) error_count;  // = 0;  // when >= max_error_count stop emitting errors
static _Atomic(size_t
) warning_count;  // = 0;  // when >= max_warning_count stop emitting warnings
...
static void
mi_vfprintf(
    mi_output_fun *out, void *arg, const char *prefix, const char *fmt, va_list args
) {
    char buf[512];
    if (fmt == NULL)
        return;
...
void
_mi_assert_fail(
    const char *assertion, const char *fname, unsigned line, const char *func
) {
    _mi_fprintf(
        NULL,
        NULL,
...
#if (MI_SECURE > 0)
    if (err ==
        EFAULT) {  // abort on serious errors in secure mode (corrupted meta-data)
        abort();
    }
#endif
#if defined(MI_XMALLOC)
    if (err == ENOMEM ||
        err == EOVERFLOW) {  // abort on memory allocation fails in xmalloc mode
        abort();
    }
#endif
}
...
mi_option_init(mi_option_desc_t *desc) {
    ...
    if (!found && desc->legacy_name != NULL) {
        ...
        if (found) {
            _mi_warning_message(
                "environment option \"mimalloc_%s\" is deprecated -- use "
                "\"mimalloc_%s\" instead.\n",
                desc->legacy_name,
                desc->name
            );
        ...
        if (buf[0] == 0 || strstr("1;TRUE;YES;ON", buf) != NULL) {
            ...
            if (*end == 0) {
                ...
                if (desc->option == mi_option_verbose && desc->value == 0) {
                    // if the 'mimalloc_verbose' env var has a bogus value we'd never
                    // know (since the value defaults to 'off') so in that case briefly
                    // enable verbose
                    desc->value = 1;
                    _mi_warning_message(
