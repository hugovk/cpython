...
mi_heap_queue_first_update(mi_heap_t *heap, const mi_page_queue_t *pq) {
    ...
    if (idx <= 1) {
        ...
        // find previous size; due to minimal alignment upto 3 previous bins may need to
        // be skipped
...
mi_page_queue_remove(mi_page_queue_t *queue, mi_page_t *page) {
    ...
    mi_assert_internal(
        page->xblock_size == queue->block_size ||
...
mi_page_queue_enqueue_from(
    mi_page_queue_t *to, mi_page_queue_t *from, mi_page_t *page
) {
    mi_assert_internal(page != NULL);
...
_mi_page_queue_append(mi_heap_t *heap, mi_page_queue_t *pq, mi_page_queue_t *append) {
    ...
    for (mi_page_t *page = append->first; page != NULL; page = page->next) {
        ...
        // in this case it is ok to be delayed freeing since both "to" and "from" heap
        // are still alive.
        mi_atomic_store_release(&page->xheap, (uintptr_t)heap);
        // set the flag to delayed free (not overriding NEVER_DELAYED_FREE) which has as
        // a side effect that it spins until any DELAYED_FREEING is finished. This
        // ensures that after appending only the new heap will be used for delayed free
        // operations.
