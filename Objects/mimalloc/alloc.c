...
#if (MI_DEBUG > 0)
static inline void
mi_debug_fill(mi_page_t *page, mi_block_t *block, int c, size_t size) {
...
extern inline void *
_mi_page_malloc(mi_heap_t *heap, mi_page_t *page, size_t size, bool zero)
    mi_attr_noexcept {
    ...
    mi_assert_internal(
        delta >= 0 &&
        mi_page_usable_block_size(page) >= (size - MI_PADDING_SIZE + delta)
    );
#endif
    mi_track_mem_defined(
        padding, sizeof(mi_padding_t)
    ...
    if (!mi_page_is_huge(page)) {
        ...
        const size_t maxpad =
            (delta > MI_MAX_ALIGN_SIZE ? MI_MAX_ALIGN_SIZE : delta
...
extern inline void *
_mi_heap_malloc_zero_ex(mi_heap_t *heap, size_t size, bool zero, size_t huge_alignment)
    mi_attr_noexcept {
    if mi_likely (size <= MI_SMALL_SIZE_MAX) {
        ...
        void *const p = _mi_malloc_generic(
            heap, size + MI_PADDING_SIZE, zero, huge_alignment
...
static bool
mi_list_contains(
    const mi_page_t *page, const mi_block_t *list, const mi_block_t *elem
) {
    while (list != NULL) {
        if (elem == list)
            return true;
...
mi_check_is_double_free(const mi_page_t *page, const mi_block_t *block) {
    ...
    mi_block_t *n = mi_block_nextx(
        page, block, page->keys
    );  // pretend it is freed, and get the decoded first field
    if (((uintptr_t)n & (MI_INTPTR_SIZE - 1)) == 0 &&  // quick check: aligned pointer?
        (n == NULL || mi_is_in_same_page(block, n)
        ))  // quick check: in same page or NULL?
    {
        // Suspicious: decoded value a in block is in the same page (or NULL) -- maybe a
        // double free? (continue in separate function to improve code generation)
...
mi_page_decode_padding(
    const mi_page_t *page, const mi_block_t *block, size_t *delta, size_t *bsize
) {
    ...
    *delta = padding->delta;
    ...
    keys[1] = page->keys[1];
    bool ok =
        ((uint32_t)mi_ptr_encode(page, block, keys) == canary && *delta <= *bsize);
...
// contain the pointer for the delayed list, then shrink the padding (by decreasing
// delta) so it will later not trigger an overflow error in `mi_free_block`.
void
_mi_padding_shrink(
    const mi_page_t *page, const mi_block_t *block, const size_t min_size
) {
    size_t bsize;
    size_t delta;
    bool ok = mi_page_decode_padding(page, block, &delta, &bsize);
...
static bool
mi_verify_padding(
    const mi_page_t *page, const mi_block_t *block, size_t *size, size_t *wrong
) {
    size_t bsize;
    size_t delta;
    bool ok = mi_page_decode_padding(page, block, &delta, &bsize);
...
mi_stat_huge_free(const mi_page_t *page) {
    mi_heap_t *const heap = mi_heap_get_default();
    const size_t bsize =
        mi_page_block_size(page);  // to match stats in `page.c:mi_page_huge_alloc`
...
_mi_free_block_mt(mi_page_t *page, mi_block_t *block) {
    ...
    // that is safe as these are constant and the page won't be freed (as the block is
    // not freed yet).
    mi_check_padding(page, block);
    _mi_padding_shrink(
        page, block, sizeof(mi_block_t)
    );  // for small size, ensure we can fit the delayed thread pointers without
    ...
    if (segment->kind == MI_SEGMENT_HUGE) {
        ...
        // as these are large we reset the memory occupied by the page so it is
        // available to other threads (as the owning thread needs to actually free the
        // memory later).
    ...
    if (segment->kind !=
        MI_SEGMENT_HUGE) {  // not for huge segments as we just reset the content
        mi_debug_fill(page, block, MI_DEBUG_FREED, mi_usable_size(block));
    }
#endif

    // Try to put the block on either the page-local thread free list, or the heap
    // delayed free list.
    ...
    do {
        ...
        if mi_unlikely (use_delayed) {
            // unlikely: this only happens on the first concurrent free in a page that
            // is in the full list
    ...
    if mi_unlikely (use_delayed) {
        // racy read on `heap`, but ok because MI_DELAYED_FREEING is set (see
        // `mi_heap_delete` and `mi_heap_collect_abandon`)
        mi_heap_t *const heap =
            (mi_heap_t *)(mi_atomic_load_acquire(&page->xheap));  // mi_page_heap(page);
        mi_assert_internal(heap != NULL);
        if (heap != NULL) {
            // add to the delayed free list of this heap. (do this atomically as the
            // lock only protects heap memory validity)
            mi_block_t *dfree =
                mi_atomic_load_ptr_relaxed(mi_block_t, &heap->thread_delayed_free);
            do {
                mi_block_set_nextx(heap, block, dfree, heap->keys);
            } while (!mi_atomic_cas_ptr_weak_release(
                mi_block_t, &heap->thread_delayed_free, &dfree, block
            ));
        }
...
// Adjust a block that was allocated aligned, to the actual start of the block in the
// page.
mi_block_t *
_mi_page_ptr_unalign(
    const mi_segment_t *segment, const mi_page_t *page, const void *p
) {
...
_mi_free_generic(const mi_segment_t *segment, mi_page_t *page, bool is_local, void *p)
    mi_attr_noexcept {
    mi_block_t *const block =
        (mi_page_has_aligned(page) ? _mi_page_ptr_unalign(segment, page, p)
                                   : (mi_block_t *)p);
...
mi_checked_ptr_segment(const void *p, const char *msg) {
  ...
  if mi_unlikely(!mi_is_in_heap_region(p)) {
    ...
    if mi_unlikely (_mi_ptr_cookie(segment) != segment->cookie) {
        _mi_error_message(
            EINVAL, "%s: pointer does not point to a valid heap space: %p\n", msg, p
        );
        return NULL;
    }
#endif
...
mi_free(void *p) mi_attr_noexcept {
    ...
    const bool is_local =
        (_mi_prim_thread_id() == mi_atomic_load_relaxed(&segment->thread_id));
    mi_page_t *const page = _mi_segment_page_of(segment, p);

    if mi_likely (is_local) {  // thread-local free?
        if mi_likely (page->flags.full_aligned ==
                      0)  // and it is not a full page (full pages need to move from the
                          // full bin), nor has aligned blocks (aligned blocks need to
                          // be unaligned)
        {
...
#endif
            mi_track_free_size(
                p, mi_page_usable_size_of(page, block)
            );  // faster then mi_usable_size as we already know the page and that p is
                // unaligned
            mi_block_set_next(page, block, page->local_free);
            page->local_free = block;
            if mi_unlikely (--page->used ==
                            0) {  // using this expression generates better code than:
                                  // page->used--; if (mi_page_all_free(page))
                _mi_page_retire(page);
            }
        } else {
...
_mi_free_delayed_block(mi_block_t *block) {
    ...
    if (!_mi_page_try_use_delayed_free(
            page, MI_USE_DELAYED_FREE, false /* dont overwrite never delayed */
        )) {
        return false;
    }
...
void *
_mi_heap_realloc_zero(mi_heap_t *heap, void *p, size_t newsize, bool zero)
    mi_attr_noexcept {
    // if p == NULL then behave as malloc.
    // else if size == 0 then reallocate to a zero-sized block (and don't return NULL,
    // just as mi_malloc(0)). (this means that returning NULL always indicates an error,
    // and `p` will not have been freed in that case.)
    const size_t size =
        _mi_usable_size(p, "mi_realloc");  // also works if p == NULL (with size 0)
    if mi_unlikely (newsize <= size && newsize >= (size / 2) &&
                    newsize > 0) {  // note: newsize must be > 0 or otherwise we return
                                    // NULL for realloc(NULL,0)
        mi_assert_internal(p != NULL);
        // todo: do not track as the usable size is still the same in the free; adjust
        // potential padding? mi_track_resize(p,size,newsize) if (newsize < size) {
    ...
    if mi_likely (newp != NULL) {
        ...
        if mi_likely (p != NULL) {
            ...
            mi_track_mem_defined(
                p, copysize
            );  // _mi_useable_size may be too large for byte precise memory tracking..
...
mi_heap_realpath(mi_heap_t *heap, const char *fname, char *resolved_name)
    ...
    if (resolved_name != NULL) {
        ...
        free(rname
        );  // use regular free! (which may be redirected to our free but that's ok)
        return result;
    }
    /*
...
std_new_handler_t __attribute__((weak))
_ZSt15get_new_handlerv(void) {
    return NULL;
}
...
mi_heap_alloc_new_n(mi_heap_t *heap, size_t count, size_t size) {
    ...
    if mi_unlikely (mi_count_size_overflow(count, size, &total)) {
        mi_try_new_handler(false);  // on overflow we invoke the try_new_handler once to
                                    // potentially throw std::bad_alloc
        return NULL;
    } else {
