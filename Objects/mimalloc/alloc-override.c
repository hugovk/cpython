...
#error "this file should be included from 'alloc.c' (so aliases can work)"
    ...
    !(defined(MI_SHARED_LIB) && defined(_DLL))
...
#define MI_FORWARD(fun) \
    __attribute__((alias(#fun), used, visibility("default"), copy(fun)));
#else
...
// use interposing so `DYLD_INSERT_LIBRARIES` works without
// `DYLD_FORCE_FLAT_NAMESPACE=1` See:
// <https://books.google.com/books?id=K8vUkpOXhN4C&pg=PA73>
...
#endif
#ifdef MI_OSX_ZONE
        // we interpose malloc_default_zone in alloc-override-osx.c so we can use
        // mi_free safely
        MI_INTERPOSE_MI(free),
        MI_INTERPOSE_FUN(vfree, mi_free),
#else
        // sometimes code allocates from default zone but deallocates using plain free
        // :-( (like NxHashResizeToCapacity
        // <https://github.com/nneonneo/osx-10.9-opensource/blob/master/objc4-551.1/runtime/hashtable2.mm>)
        MI_INTERPOSE_FUN(
            free, mi_cfree
        ),  // use safe free that checks if pointers are from us
...
#endif
void
_ZdlPv(void *p);  // delete
void
_ZdaPv(void *p);  // delete[]
void
_ZdlPvm(void *p, size_t n);  // delete
void
_ZdaPvm(void *p, size_t n);  // delete[]
void *
_Znwm(size_t n);  // new
void *
_Znam(size_t n);  // new[]
void *
_ZnwmRKSt9nothrow_t(size_t n, mi_nothrow_t tag);  // new nothrow
void *
_ZnamRKSt9nothrow_t(size_t n, mi_nothrow_t tag);  // new[] nothrow
#ifdef __cplusplus
}
#endif
...
mi_decl_export void *
malloc(size_t size) MI_FORWARD1(mi_malloc, size) mi_decl_export
    void *calloc(size_t size, size_t n) MI_FORWARD2(mi_calloc, size, n) mi_decl_export
    void *realloc(void *p, size_t newsize)
        MI_FORWARD2(mi_realloc, p, newsize) mi_decl_export void free(void *p)
            MI_FORWARD0(mi_free, p)
#endif
...
void
operator delete(void *p) noexcept MI_FORWARD0(mi_free, p) void operator delete[](void *p
) noexcept MI_FORWARD0(mi_free, p)

    void *operator new(std::size_t n) noexcept(false)
        MI_FORWARD1(mi_new, n) void *operator new[](std::size_t n) noexcept(false)
            MI_FORWARD1(mi_new, n)
...
_ZdlPv(void *p) MI_FORWARD0(mi_free, p)           // delete
    ...
    void _ZdlPvm(void *p, size_t n) MI_FORWARD02(mi_free_size, p, n) void _ZdaPvm(
        void *p, size_t n
...
_ZnwmSt11align_val_t(size_t n, size_t al)
    MI_FORWARD2(mi_new_aligned, n, al) void *_ZnamSt11align_val_t(size_t n, size_t al)
        MI_FORWARD2(mi_new_aligned, n, al) void *_ZnwmSt11align_val_tRKSt9nothrow_t(
            size_t n, size_t al, mi_nothrow_t tag
        ) {
    MI_UNUSED(tag);
...
_ZnwjSt11align_val_t(size_t n, size_t al)
    MI_FORWARD2(mi_new_aligned, n, al) void *_ZnajSt11align_val_t(size_t n, size_t al)
        MI_FORWARD2(mi_new_aligned, n, al) void *_ZnwjSt11align_val_tRKSt9nothrow_t(
            size_t n, size_t al, mi_nothrow_t tag
        ) {
    MI_UNUSED(tag);
    return mi_new_aligned_nothrow(n, al);
...
#else
#error \
    "define overloads for new/delete for this platform (just for performance, can be skipped)"
#endif
#endif  // __cplusplus
...
void *
reallocf(void *p, size_t newsize) MI_FORWARD2(mi_reallocf, p, newsize) size_t
...
// Note: it seems __USE_ISOC11 is not defined in musl (and perhaps other libc's) so we
// only check for it if using glibc. Note: Conda has a custom glibc where
// `aligned_alloc` is declared `static inline` and we cannot override it, but both
// _ISOC11_SOURCE and __USE_ISOC11 are undefined in Conda GCC7 or GCC9. Fortunately, in
// the case where `aligned_alloc` is declared as `static inline` it uses internally
// `memalign`, `posix_memalign`, or `_aligned_malloc` so we  can avoid overriding it
// ourselves.
...
int
reallocarr(void *p, size_t count, size_t size) {
...
void *
_aligned_malloc(size_t alignment, size_t size) {
    return mi_aligned_alloc(alignment, size);
}
...
__libc_malloc(size_t size)
    MI_FORWARD1(mi_malloc, size) void *__libc_calloc(size_t count, size_t size)
        MI_FORWARD2(mi_calloc, count, size) void *__libc_realloc(void *p, size_t size)
            MI_FORWARD2(mi_realloc, p, size) void __libc_free(void *p)
                MI_FORWARD0(mi_free, p) void __libc_cfree(void *p)
                    MI_FORWARD0(mi_free, p)
