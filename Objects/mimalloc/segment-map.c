...
mi_segment_map_index_of(const mi_segment_t *segment, size_t *bitidx) {
    mi_assert_internal(
        _mi_ptr_segment(segment + 1) == segment
    );  // is it aligned on MI_SEGMENT_SIZE?
...
_mi_segment_of(const void *p) {
    ...
    size_t bitidx;
    size_t index = mi_segment_map_index_of(segment, &bitidx);
    // fast path: for any pointer to valid small/medium/large object or first
    // MI_SEGMENT_SIZE in huge
    ...
    // could be slow but searches in MI_INTPTR_SIZE * MI_SEGMENT_SIZE (512MiB) steps
    // through valid huge objects note: we could maintain a lowest index to speed up the
    // path for invalid pointers?
    size_t lobitidx;
    size_t loindex;
    ...
    size_t diff = (((index - loindex) * (8 * MI_INTPTR_SIZE)) + bitidx - lobitidx) *
                  MI_SEGMENT_SIZE;
    ...
    mi_assert_internal(
        p >= (void *)segment &&
        (uint8_t *)p < (uint8_t *)segment + mi_segment_size(segment)
    );
    return segment;
}
...
static void* mi_segment_range_of(const void* p, size_t* size) {
  ...
  mi_assert_internal(page == NULL || (mi_segment_page_size(_mi_page_segment(page)) -
(MI_SECURE == 0 ? 0 : _mi_os_page_size())) >= block_size); mi_reset_delayed(tld);
