...
_mi_bitmap_try_find_claim_field(
    mi_bitmap_t bitmap, size_t idx, const size_t count, mi_bitmap_index_t *bitmap_idx
) {
    ...
    while (bitidx <= bitidx_max) {
        ...
        if (mapm == 0) {  // are the mask bits free at bitidx?
            ...
            if (!mi_atomic_cas_strong_acq_rel(
                    field, &map, newmap
                )) {  // TODO: use weak cas here?
                // no success, another thread claimed concurrently.. keep going (with
                // updated `map`)
                continue;
            } else {
            ...
            const size_t shift =
                (count == 1 ? 1 : (MI_INTPTR_BITS - mi_clz(mapm) - bitidx));
...
// fulfilled
bool
_mi_bitmap_try_find_from_claim_pred(
    mi_bitmap_t bitmap,
    const size_t bitmap_fields,
    const size_t start_field_idx,
    const size_t count,
...
// Returns `true` if all `count` bits were 0 previously. `any_zero` is `true` if there
// was at least one zero bit.
bool
_mi_bitmap_claim(
    mi_bitmap_t bitmap,
...
// Returns `true` if all `count` bits were 1. `any_ones` is `true` if there was at least
// one bit set to one.
static bool
...
// Only needs to consider crossing into the next fields (see
// `mi_bitmap_try_find_from_claim_across`)
static bool
mi_bitmap_try_find_claim_field_across(
    ...
    if (initial == 0)
        return false;
    if (initial >= count)
        return _mi_bitmap_try_find_claim_field(
            bitmap, idx, count, bitmap_idx
        );  // no need to cross fields (this case won't happen for us)
    ...
    while (found < count) {
        ...
        const size_t mask_bits =
            (found + MI_BITMAP_FIELD_BITS <= count ? MI_BITMAP_FIELD_BITS
                                                   : (count - found));
    ...
    // we found a range of contiguous zeros up to the final field; mask contains mask in
    // the final field now try to claim the range atomically
...
rollback:
    ...
    if (field == initial_field) {  // (if we failed on the initial field, `field + 1 ==
                                   // initial_field`)
        map = mi_atomic_load_relaxed(field);
        do {
    ...
    // retry? (we make a recursive call instead of goto to be able to use const
    // declarations)
    if (retries <= 2) {
        return mi_bitmap_try_find_claim_field_across(
            bitmap, bitmap_fields, idx, count, retries + 1, bitmap_idx
        );
    } else {
        return false;
    }
...
mi_bitmap_mask_across(
    ...
    if mi_likely (bitidx + count <= MI_BITMAP_FIELD_BITS) {
        ...
        *post_mask = (count == 0 ? 0 : mi_bitmap_mask_(count, 0));
        mi_assert_internal(
            mi_bitmap_index_field(bitmap_idx) + mid_count + (count == 0 ? 0 : 1) <
            bitmap_fields
        );
        return mid_count;
    }
...
_mi_bitmap_unclaim_across(
    ...
    size_t mid_mask;
    size_t post_mask;
    size_t mid_count = mi_bitmap_mask_across(
        bitmap_idx, bitmap_fields, count, &pre_mask, &mid_mask, &post_mask
    );
    bool all_one = true;
    mi_bitmap_field_t *field = &bitmap[idx];
...
bool
_mi_bitmap_is_any_claimed_across(
    mi_bitmap_t bitmap, size_t bitmap_fields, size_t count, mi_bitmap_index_t bitmap_idx
) {
    bool any_ones;
