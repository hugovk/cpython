...
mi_commit_mask_create_intersect(
    const mi_commit_mask_t *commit, const mi_commit_mask_t *cm, mi_commit_mask_t *res
) {
    for (size_t i = 0; i < MI_COMMIT_MASK_FIELD_COUNT; i++) {
...
mi_span_queue_delete(mi_span_queue_t *sq, mi_slice_t *slice) {
    ...
    // should work too if the queue does not contain slice (which can happen during
    // reclaim)
    if (slice->prev != NULL)
...
mi_segment_is_valid(mi_segment_t *segment, mi_segments_tld_t *tld) {
    ...
    mi_assert_internal(
        segment->thread_id == 0 || segment->thread_id == _mi_thread_id()
    );
    mi_assert_internal(
        mi_commit_mask_all_set(&segment->commit_mask, &segment->purge_mask)
    );  // can only decommit committed blocks
    ...
    while (slice < end) {
        ...
        if (mi_slice_is_used(slice
            )) {  // a page in use, we need at least MAX_SLICE_OFFSET valid back offsets
            used_count++;
            for (size_t i = 0; i <= MI_MAX_SLICE_OFFSET && index + i <= maxindex; i++) {
                ...
                mi_assert_internal(
                    i == 0 || segment->slices[index + i].slice_count == 0
                );
                mi_assert_internal(
                    i == 0 || segment->slices[index + i].xblock_size == 1
                );
            }
            ...
            if (segment->kind != MI_SEGMENT_HUGE ||
                slice->slice_count <=
                    (segment->slice_entries - segment->segment_info_slices)) {
                mi_assert_internal(
                    (uint8_t *)slice == (uint8_t *)last - last->slice_offset
                );
            }
...
_mi_segment_page_start_from_slice(
    ...
    // note: the offset must always be an xblock_size multiple since we assume small
    // allocations are aligned (see `mi_heap_malloc_aligned`).
...
_mi_segment_page_start(
    const mi_segment_t *segment, const mi_page_t *page, size_t *page_size
) {
    const mi_slice_t *slice = mi_page_to_slice((mi_page_t *)page);
    uint8_t *p =
        _mi_segment_page_start_from_slice(segment, slice, page->xblock_size, page_size);
...
mi_segment_os_free(mi_segment_t *segment, mi_segments_tld_t *tld) {
    ...
    if (MI_SECURE > 0) {
        // _mi_os_unprotect(segment, mi_segment_size(segment)); // ensure no more guard
        // pages are set unprotect the guard pages; we cannot just unprotect the whole
        // segment size as part may be decommitted
        size_t os_pagesize = _mi_os_page_size();
        _mi_os_unprotect(
            (uint8_t *)segment + mi_segment_info_size(segment) - os_pagesize,
            os_pagesize
        );
    ...
    _mi_arena_free(
        segment, mi_segment_size(segment), csize, segment->memid, tld->stats
    );
}
...
mi_segment_commit_mask(
    ...
    if (pstart >= segstart &&
        start < segstart) {  // note: the mask is also calculated for an initial commit
                             // of the info area
        start = segstart;
    }
    if (end > segsize) {
    ...
    *full_size = (end > start ? end - start : 0);
    ...
    if (bitidx + bitcount > MI_COMMIT_MASK_BITS) {
        _mi_warning_message(
            "commit mask overflow: idx=%zu count=%zu start=%zx end=%zx p=0x%p size=%zu "
            "fullsize=%zu\n",
            bitidx,
            bitcount,
            start,
...
mi_segment_commit(mi_segment_t *segment, uint8_t *p, size_t size, mi_stats_t *stats) {
    ...
    mi_segment_commit_mask(
        segment, false /* conservative? */, p, size, &start, &full_size, &mask
    );
    ...
    // increase purge expiration when using part of delayed purges -- we assume more
    // allocations are coming soon.
...
mi_segment_ensure_committed(
    mi_segment_t *segment, uint8_t *p, size_t size, mi_stats_t *stats
) {
    mi_assert_internal(
        mi_commit_mask_all_set(&segment->commit_mask, &segment->purge_mask)
    );
    // note: assumes commit_mask is always full for huge segments as otherwise the
    // commit mask bits can overflow
...
mi_segment_purge(mi_segment_t *segment, uint8_t *p, size_t size, mi_stats_t *stats) {
    ...
    mi_commit_mask_t mask;
    mi_segment_commit_mask(
        segment, true /* conservative? */, p, size, &start, &full_size, &mask
    );
    if (mi_commit_mask_is_empty(&mask) || full_size == 0)
        return true;
    ...
    if (mi_commit_mask_any_set(&segment->commit_mask, &mask)) {
        ...
        const bool decommitted =
            _mi_os_purge(start, full_size, stats);  // reset or decommit
        if (decommitted) {
            mi_commit_mask_t cmask;
            mi_commit_mask_create_intersect(&segment->commit_mask, &mask, &cmask);
            _mi_stat_increase(
...
mi_segment_schedule_purge(
    ...
    if (mi_option_get(mi_option_purge_delay) == 0) {
        ...
        mi_commit_mask_t cmask;
        mi_commit_mask_create_intersect(
            &segment->commit_mask, &mask, &cmask
        );  // only purge what is committed; span_free may try to decommit more
        ...
        if (segment->purge_expire == 0) {
            ...
            if (segment->purge_expire + mi_option_get(mi_option_purge_extend_delay) <=
                now) {
                mi_segment_try_purge(segment, true, stats);
            } else {
                segment->purge_expire =
                    now + mi_option_get(mi_option_purge_extend_delay
                          );  // (mi_option_get(mi_option_purge_delay) / 8); // wait a
                              // tiny bit longer in case there is a series of free's
            }
        } else {
...
mi_segment_span_free(
    ...
    mi_span_queue_t *sq =
        (segment->kind == MI_SEGMENT_HUGE || mi_segment_is_abandoned(segment)
             ? NULL
...
static void mi_segment_span_add_free(mi_slice_t* slice, mi_segments_tld_t* tld) {
  mi_segment_t* segment = _mi_ptr_segment(slice);
  mi_assert_internal(slice->xblock_size==0 && slice->slice_count>0 &&
slice->slice_offset==0); size_t slice_index = mi_slice_index(slice);
...
mi_segment_span_free_coalesce(mi_slice_t *slice, mi_segments_tld_t *tld) {
    mi_assert_internal(
        slice != NULL && slice->slice_count > 0 && slice->slice_offset == 0
    );
    mi_segment_t *segment = _mi_ptr_segment(slice);
    ...
    if (segment->kind == MI_SEGMENT_HUGE) {
        // issue #691: segment->used can be 0 if the huge page block was freed while
        // abandoned (reclaim will get here in that case)
        mi_assert_internal(
            (segment->used == 0 && slice->xblock_size == 0) || segment->used == 1
        );  // decreased right after this call in `mi_segment_page_clear`
        slice->xblock_size = 0;  // mark as free anyways
        // we should mark the last slice `xblock_size=0` now to maintain invariants but
        // we skip it to avoid a possible cache miss (and the segment is about to be
        // freed)
        return slice;
    }
...
mi_segment_span_allocate(
    ...
    if (!mi_segment_ensure_committed(
            segment,
    ...
    slice->xblock_size =
        (uint32_t)(bsize >= MI_HUGE_BLOCK_SIZE ? MI_HUGE_BLOCK_SIZE : bsize);
    ...
    if (slice_index + extra >= segment->slice_entries)
        extra = segment->slice_entries - slice_index -
                1;  // huge objects may have more slices than available entries in the
                    // segment->slices
    ...
    // coalescing and for large alignments) note: the cast is needed for ubsan since the
    // index can be larger than MI_SLICES_PER_SEGMENT for huge allocations (see #543)
...
mi_segment_slice_split(
    mi_segment_t *segment, mi_slice_t *slice, size_t slice_count, mi_segments_tld_t *tld
) {
    ...
    mi_segment_span_free(
        segment, next_index, next_count, false /* don't purge left-over part */, tld
    );
    slice->slice_count = (uint32_t)slice_count;
}
        ...
        for (mi_slice_t *slice = sq->first; slice != NULL; slice = slice->next) {
            if (slice->slice_count >= slice_count) {
                ...
                if (_mi_arena_memid_is_suitable(segment->memid, req_arena_id)) {
                    ...
                    mi_assert_internal(
                        slice != NULL && slice->slice_count == slice_count &&
                        slice->xblock_size > 0
                    );
                    mi_page_t *page = mi_segment_span_allocate(
                        segment, mi_slice_index(slice), slice->slice_count, tld
                    );
                    if (page == NULL) {
...

{
    ...
    if (page_alignment > 0) {
        ...
        *psegment_slices =
        ...
        if (*psegment_slices > (size_t)UINT32_MAX)
            return NULL;
    }
...
mi_segment_alloc(
    ...
    size_t segment_slices =
        mi_segment_calculate_slices(required, &pre_size, &info_slices);

    // mi_page_t.slice_count type is uint32_t
    if (segment_slices > (size_t)UINT32_MAX)
        return NULL;

    // Commit eagerly only if not the first N lazy segments (to reduce impact of many
    // threads that allocate just a little)
    const bool eager_delay = (  // !_mi_os_has_overcommit() &&             // never
                                // delay on overcommit systems
    ...
    mi_segment_t *segment = mi_segment_os_alloc(
        required,
        page_alignment,
        eager_delay,
        req_arena_id,
    ...
    // zero the segment info? -- not always needed as it may be zero initialized from
    // the OS
    if (!segment->memid.initially_zero) {
        ptrdiff_t ofs = offsetof(mi_segment_t, next);
        size_t prefix = offsetof(mi_segment_t, slices) - ofs;
        size_t zsize =
            prefix + (sizeof(mi_slice_t) * (segment_slices + 1));  // one more
    ...
    if (segment->kind == MI_SEGMENT_NORMAL) {  // not a huge page
        ...
        mi_segment_span_free(
            segment,
            info_slices,
            segment->slice_entries - info_slices,
            false /* don't purge */,
            tld
        );
    } else {
        ...
        *huge_page = mi_segment_span_allocate(
            segment, info_slices, segment_slices - info_slices - guard_slices, tld
        );
...
mi_segment_free(mi_segment_t *segment, bool force, mi_segments_tld_t *tld) {
    ...
    while (slice < end) {
        ...
        mi_assert_internal(slice->slice_offset == 0);
        mi_assert_internal(
            mi_slice_index(slice) == 0 || slice->xblock_size == 0
        );  // no more used pages ..
    ...
    mi_assert_internal(
        page_count == 2
    );  // first page is allocated by the segment itself
...
mi_segment_page_clear(mi_page_t *page, mi_segments_tld_t *tld) {
    ...
    if (segment->allow_decommit &&
        mi_option_is_enabled(mi_option_deprecated_page_reset)) {
        size_t psize;
...
static void
mi_abandoned_visited_push(mi_abandoned_pool_t *pool, mi_segment_t *segment) {
    mi_assert_internal(segment->thread_id == 0);
    mi_assert_internal(
        mi_atomic_load_ptr_relaxed(mi_segment_t, &segment->abandoned_next) == NULL
    );
    mi_assert_internal(segment->next == NULL);
    mi_assert_internal(segment->used > 0);
    mi_segment_t *anext =
        mi_atomic_load_ptr_relaxed(mi_segment_t, &pool->abandoned_visited);
    do {
        mi_atomic_store_ptr_release(mi_segment_t, &segment->abandoned_next, anext);
    } while (!mi_atomic_cas_ptr_weak_release(
        mi_segment_t, &pool->abandoned_visited, &anext, segment
    ));
...
static bool
mi_abandoned_visited_revisit(mi_abandoned_pool_t *pool) {
    // quick check if the visited list is empty
    if (mi_atomic_load_ptr_relaxed(mi_segment_t, &pool->abandoned_visited) == NULL)
        return false;

    // grab the whole visited list
    mi_segment_t *first =
        mi_atomic_exchange_ptr_acq_rel(mi_segment_t, &pool->abandoned_visited, NULL);
    if (first == NULL)
        return false;
    ...
    mi_segment_t *next;
    while ((next = mi_atomic_load_ptr_relaxed(mi_segment_t, &last->abandoned_next)) !=
           NULL) {
        last = next;
    }
    ...
    do {
        ...
        mi_atomic_store_ptr_release(
            mi_segment_t, &last->abandoned_next, mi_tagged_segment_ptr(anext)
        );
...
static void
mi_abandoned_push(mi_abandoned_pool_t *pool, mi_segment_t *segment) {
    mi_assert_internal(segment->thread_id == 0);
    mi_assert_internal(
        mi_atomic_load_ptr_relaxed(mi_segment_t, &segment->abandoned_next) == NULL
    );
    ...
    do {
        mi_atomic_store_ptr_release(
            mi_segment_t, &segment->abandoned_next, mi_tagged_segment_ptr(ts)
        );
...
// Wait until there are no more pending reads on segments that used to be in the
// abandoned list called for example from `arena.c` before decommitting
void
_mi_abandoned_await_readers(mi_abandoned_pool_t *pool) {
    size_t n;
    do {
...
static mi_segment_t *
mi_abandoned_pop(mi_abandoned_pool_t *pool) {
    mi_segment_t *segment;
    // Check efficiently if it is empty (or if the visited list needs to be moved)
    mi_tagged_segment_t ts = mi_atomic_load_relaxed(&pool->abandoned);
    segment = mi_tagged_segment_ptr(ts);
    if mi_likely (segment == NULL) {
        if mi_likely (!mi_abandoned_visited_revisit(pool
    ...
    mi_atomic_increment_relaxed(&pool->abandoned_readers
    ...
    do {
        ...
        if (segment != NULL) {
            ...
            next = mi_tagged_segment(
                anext, ts
            );  // note: reads the segment's `abandoned_next` field so should not be
                // decommitted
        }
...
mi_segment_abandon(mi_segment_t *segment, mi_segments_tld_t *tld) {
    ...
    mi_segment_try_purge(
        segment,
        mi_option_is_enabled(mi_option_abandoned_page_purge) /* force? */,
        tld->stats
    );
...
mi_slices_start_iterate(mi_segment_t *segment, const mi_slice_t **end) {
    ...
    *end = mi_segment_slices_end(segment);
    mi_assert_internal(
        slice->slice_count > 0 && slice->xblock_size > 0
    );                                   // segment allocated page
    ...
    while (slice < end) {
        ...
        if (mi_slice_is_used(slice)) {  // used page
            ...
            if (mi_page_all_free(page) && _PyMem_mi_page_is_safe_to_free(page)) {
                // if this page is all free now, free it without adding to any queues
                // (yet)
...
#endif
                segment->abandoned--;
                slice = mi_segment_page_clear(
                    page, tld
                );  // re-assign slice due to coalesce!
                mi_assert_internal(!mi_slice_is_used(slice));
                if (slice->slice_count >= slices_needed) {
                    has_page = true;
                }
            } else {
                if (page->xblock_size == block_size &&
                    mi_page_has_any_available(page)) {
...
static mi_heap_t *
mi_heap_by_tag(mi_heap_t *heap, uint8_t tag) {
    if (heap->tag == tag) {
        return heap;
    }
...
// set `right_page_reclaimed` to `true` if it reclaimed a page of the right `block_size`
// that was not full.
static mi_segment_t *
mi_segment_reclaim(
    mi_segment_t *segment,
    ...
    while (slice < end) {
        ...
        if (mi_slice_is_used(slice)) {
            ...
            if (mi_page_all_free(page) && _PyMem_mi_page_is_safe_to_free(page)) {
                ...
                slice = mi_segment_page_clear(
                    page, tld
                );  // set slice again due to coalesceing
            } else {
                // otherwise reclaim it into the heap
                _mi_page_reclaim(target_heap, page);
                if (requested_block_size == page->xblock_size &&
                    mi_page_has_any_available(page) &&
                    requested_block_size <= MI_MEDIUM_OBJ_SIZE_MAX &&
                    heap == target_heap) {
                    if (right_page_reclaimed != NULL) {
...
mi_segment_try_reclaim(
    ...
    *reclaimed = false;
    mi_segment_t *segment;
    long max_tries = mi_option_get_clamp(
        mi_option_max_segment_reclaim, 8, 1024
    );  // limit the work to bound allocation times
    while ((max_tries-- > 0) && ((segment = mi_abandoned_pop(tld->abandoned)) != NULL)
    ) {
        segment->abandoned_visits++;
        // todo: an arena exclusive heap will potentially visit many abandoned
        // unsuitable segments and push them into the visited list and use many tries.
        // Perhaps we can skip non-suitable ones in a better way?
        bool is_suitable = _mi_heap_memid_is_suitable(heap, segment->memid);
        bool has_page = mi_segment_check_free(
            segment, needed_slices, block_size, tld
        );  // try to free up pages (due to concurrent frees)
        if (segment->used == 0) {
            // free the segment (by forced reclaim) to make it available to other
            // threads. note1: we prefer to free a segment as that might lead to
            // reclaiming another segment that is still partially used. note2: we could
            // in principle optimize this by skipping reclaim and directly freeing but
            // that would violate some invariants temporarily)
            mi_segment_reclaim(segment, heap, 0, NULL, tld);
        } else if (has_page && is_suitable) {
            // found a large enough free span, or a page of the right block_size with
            // free space we return the result of reclaim (which is usually `segment`)
            // as it might free the segment due to concurrent frees (in which case
            // `NULL` is returned).
            ...
            // otherwise, push on the visited list so it gets not looked at too quickly
            // again
            mi_segment_try_purge(
                segment, true /* force? */, tld->stats
            );  // force purge if needed as we may not visit soon again
...
_mi_abandoned_collect(mi_heap_t *heap, bool force, mi_segments_tld_t *tld) {
    ...
    while ((max_tries-- > 0) && ((segment = mi_abandoned_pop(pool)) != NULL)) {
        ...
        if (segment->used == 0) {
            // free the segment (by forced reclaim) to make it available to other
            // threads. note: we could in principle optimize this by skipping reclaim
            // and directly freeing but that would violate some invariants temporarily)
            mi_segment_reclaim(segment, heap, 0, NULL, tld);
        } else {
            // otherwise, purge if needed and push on the visited list
            // note: forced purge can be expensive if many threads are destroyed/created
            // as in mstress.
...
mi_segment_reclaim_or_alloc(
    ...
    if (reclaimed) {
        ...
        mi_assert_internal(segment != NULL);
        return NULL;  // pretend out-of-memory as the page will be in the page queue of
                      // the heap with available blocks
...
mi_segments_page_alloc(
    ...
    // find a free page
    size_t page_size = _mi_align_up(
        required,
        (required > MI_MEDIUM_PAGE_SIZE ? MI_MEDIUM_PAGE_SIZE : MI_SEGMENT_SLICE_SIZE)
    );
    size_t slices_needed = page_size / MI_SEGMENT_SLICE_SIZE;
    mi_assert_internal(slices_needed * MI_SEGMENT_SLICE_SIZE == page_size);
    mi_page_t *page = mi_segments_page_find_and_allocate(
        slices_needed, heap->arena_id, tld
    );  //(required <= MI_SMALL_SIZE_MAX ? 0 : slices_needed), tld);
    if (page == NULL) {
        // no free page, allocate a new segment and try again
        if (mi_segment_reclaim_or_alloc(heap, slices_needed, block_size, tld, os_tld) ==
            NULL) {
            ...
            return mi_segments_page_alloc(
                heap, page_kind, required, block_size, tld, os_tld
            );
        }
    }
    mi_assert_internal(
...
mi_segment_huge_page_alloc(
    ...
    page->xblock_size =
        (psize > MI_HUGE_BLOCK_SIZE ? MI_HUGE_BLOCK_SIZE : (uint32_t)psize);

    // decommit the part of the prefix of a page that will not be used; this can be
    // quite large (close to MI_SEGMENT_SIZE)
    if (page_alignment > 0 && segment->allow_decommit) {
        ...
        _mi_os_reset(
            decommit_start, decommit_size, &_mi_stats_main
        );  // note: cannot use segment_decommit on huge segments
    }
...
_mi_segment_huge_page_free(mi_segment_t *segment, mi_page_t *page, mi_block_t *block) {
    // huge page segments are always abandoned and can be freed immediately by any
    // thread
    mi_assert_internal(segment->kind == MI_SEGMENT_HUGE);
    ...
    // claim it and free
    mi_heap_t *heap =
        mi_heap_get_default();  // issue #221; don't use the internal get_default_heap
                                // as we need to ensure the thread is initialized.
    // paranoia: if this it the last reference, the cas should always succeed
    size_t expected_tid = 0;
    if (mi_atomic_cas_strong_acq_rel(
            &segment->thread_id, &expected_tid, heap->thread_id
        )) {
...
_mi_segment_page_alloc(
    ...
    if mi_unlikely (page_alignment > MI_ALIGNMENT_MAX) {
        ...
        page = mi_segment_huge_page_alloc(
            block_size, page_alignment, heap->arena_id, tld, os_tld
        );
    } else if (block_size <= MI_SMALL_OBJ_SIZE_MAX) {
        page = mi_segments_page_alloc(
            heap, MI_PAGE_SMALL, block_size, block_size, tld, os_tld
        );
    } else if (block_size <= MI_MEDIUM_OBJ_SIZE_MAX) {
        page = mi_segments_page_alloc(
            heap, MI_PAGE_MEDIUM, MI_MEDIUM_PAGE_SIZE, block_size, tld, os_tld
        );
    } else if (block_size <= MI_LARGE_OBJ_SIZE_MAX) {
        page = mi_segments_page_alloc(
            heap, MI_PAGE_LARGE, block_size, block_size, tld, os_tld
        );
    } else {
        page = mi_segment_huge_page_alloc(
            block_size, page_alignment, heap->arena_id, tld, os_tld
    ...
    mi_assert_expensive(
        page == NULL || mi_segment_is_valid(_mi_page_segment(page), tld)
    );
    return page;
}
    ...
    while (slice < end) {
        if (mi_slice_is_used(slice)) {
            ...
            if (page->tag == page_tag) {
                if (!mi_segment_visit_page(segment, page, visit_blocks, visitor, arg))
                    return false;
            }
        }
        slice = slice + slice->slice_count;
...
_mi_abandoned_pool_visit_blocks(
    mi_abandoned_pool_t *pool,
    uint8_t page_tag,
    bool visit_blocks,
    mi_block_visit_fun *visitor,
    void *arg
) {
    // Note: this is not safe in any other thread is abandoning or claiming segments
    // from the pool
    mi_segment_t *segment = mi_tagged_segment_ptr(pool->abandoned);
    while (segment != NULL) {
        if (!mi_segment_visit_pages(segment, page_tag, visit_blocks, visitor, arg))
            return false;
