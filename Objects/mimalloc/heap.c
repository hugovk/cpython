...
typedef bool(heap_page_visitor_fun)(
...
static bool
mi_heap_visit_pages(
    mi_heap_t *heap, heap_page_visitor_fun *fn, void *arg1, void *arg2
) {
...
// visit all pages
    ...
    for (size_t i = 0; i <= MI_BIN_FULL; i++) {
        ...
        while (page != NULL) {
            mi_page_t *next =
                page->next;  // save next in case the page gets removed from the queue
...
mi_heap_collect_ex(mi_heap_t *heap, mi_collect_t collect) {
    ...
    // note: never reclaim on collect but leave it to threads that need storage to
    // reclaim
    ...
    if (force_main) {
        // the main thread is abandoned (end-of-program), try to reclaim all abandoned
        // segments. if all memory is freed by now, all segments should be freed.
    ...
    // (if abandoning, after this there are no more thread-delayed references into the
    // pages.)
    ...
    // collect abandoned segments (in particular, purge expired parts of segments in the
    // abandoned segment list) note: forced purge can be quite expensive if many threads
    // are created/destroyed so we do not force on abandonment
...
mi_heap_new_in_arena(mi_arena_id_t arena_id) {
    ...
    mi_heap_t *heap =
        mi_heap_malloc_tp(bheap, mi_heap_t);  // todo: OS allocate in secure mode?
    if (heap == NULL)
        return NULL;
...
// called from `mi_heap_destroy` and `mi_heap_delete` to free the internal heap
// resources.
static void
...
mi_heap_absorb(mi_heap_t *heap, mi_heap_t *from) {
    ...
    // note: be careful here as the `heap` field in all those pages no longer point to
    // `from`, turns out to be ok as `_mi_heap_delayed_free` only visits the list and
    // calls a the regular `_mi_free_delayed_block` which is safe.
    _mi_heap_delayed_free_all(from);
#if !defined(_MSC_VER) || \
    (_MSC_VER > 1900      \
    )  // somehow the following line gives an error in VS2015, issue #353
    mi_assert_internal(
        mi_atomic_load_ptr_relaxed(mi_block_t, &from->thread_delayed_free) == NULL
    );
#endif
...
static void
mi_fast_divisor(size_t divisor, size_t *magic, size_t *shift) {
...
static size_t
mi_fast_divide(size_t n, size_t magic, size_t shift) {
    ...
    for (size_t i = 0; i < bmapsize; i++) {
        if (free_map[i] == 0) {
            ...
            while (m) {
                ...
                if (!visitor(heap, area, block + (bitidx * bsize), ubsize, arg))
                    return false;
                m &= m - 1;
            }
...
typedef bool(mi_heap_area_visit_fun)(
    const mi_heap_t *heap, const mi_heap_area_ex_t *area, void *arg
);
...
mi_heap_visit_areas_page(
    mi_heap_t *heap, mi_page_queue_t *pq, mi_page_t *page, void *vfun, void *arg
) {
    MI_UNUSED(heap);
    MI_UNUSED(pq);
...
mi_heap_visit_areas(const mi_heap_t *heap, mi_heap_area_visit_fun *visitor, void *arg) {
    ...
    return mi_heap_visit_pages(
        (mi_heap_t *)heap, &mi_heap_visit_areas_page, (void *)(visitor), arg
...
mi_heap_area_visitor(const mi_heap_t *heap, const mi_heap_area_ex_t *xarea, void *arg) {
    ...
    if (args->visit_blocks) {
        return _mi_heap_area_visit_blocks(
            &xarea->area, xarea->page, args->visitor, args->arg
        );
    } else {
        return true;
    }
...
mi_heap_visit_blocks(
    const mi_heap_t *heap, bool visit_blocks, mi_block_visit_fun *visitor, void *arg
) {
