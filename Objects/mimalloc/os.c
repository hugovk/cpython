...
static mi_os_mem_config_t mi_os_mem_config = {
    ...
    false,  // must free whole? (on mmap systems we can free anywhere in a mapped range,
            // but on Windows we must free the entire span)
    true  // has virtual reserve? (if true we can reserve virtual address space without
          // using commit or physical memory)
};
...
// if large OS pages are supported (2 or 4MiB), then return the size, otherwise return
// the small page size (4KiB)
size_t
_mi_os_large_page_size(void) {
    return (
        mi_os_mem_config.large_page_size != 0 ? mi_os_mem_config.large_page_size
                                              : _mi_os_page_size()
    );
}
...
_mi_os_use_large_page(size_t size, size_t alignment) {
    ...
    if (mi_os_mem_config.large_page_size == 0 ||
        ...
        return false;
    return (
        (size % mi_os_mem_config.large_page_size) == 0 &&
...
// If this returns NULL, the OS will determine the address but on some OS's that may not
// be properly aligned which can be more costly as it needs to be adjusted afterwards.
// For a size > 1GiB this always returns NULL in order to guarantee good ASLR
// randomization; (otherwise an initial large allocation of say 2TiB has a 50% chance to
...
#define MI_HINT_AREA    \
    ((uintptr_t)4 << 40 \
    )  // upto 6TiB   (since before win8 there is "only" 8TiB available to processes)
#define MI_HINT_MAX      \
    ((uintptr_t)30 << 40 \
    )  // wrap after 30TiB (area after 32TiB is used for huge OS pages)
...
_mi_os_get_aligned_hint(size_t try_alignment, size_t size) {
    ...
    if (size > 1 * MI_GiB)
        return NULL;  // guarantee the chance of fixed valid address is at most
                      // 1/(MI_HINT_AREA / 1<<30) = 1/4096.
#if (MI_SECURE > 0)
    size += MI_SEGMENT_SIZE;  // put in `MI_SEGMENT_SIZE` virtual gaps between hinted
                              // blocks; this splits VLA's but increases guarded areas.
#endif
...
#if (MI_SECURE > 0 || \
     MI_DEBUG ==      \
         0)  // security: randomize start of aligned allocations unless in debug mode
        ...
        hint = mi_atomic_add_acq_rel(
            &aligned_base, size
        );  // this may still give 0 or > MI_HINT_MAX but that is ok, it is a hint after
            // all
    }
...
mi_os_prim_free(void *addr, size_t size, bool still_committed, mi_stats_t *tld_stats) {
    ...
    if (err != 0) {
        _mi_warning_message(
            "unable to free OS memory (error: %d (0x%x), size: 0x%zx bytes, address: "
            "%p)\n",
            err,
            err,
            size,
...
_mi_os_free_ex(
    ...
    if (mi_memkind_is_os(memid.memkind)) {
        ...
        if (memid.mem.os.base != NULL) {
            ...
            mi_assert(
                (uint8_t *)memid.mem.os.base + memid.mem.os.alignment >= (uint8_t *)addr
            );
...
mi_os_prim_alloc(
    ...
    *is_zero = false;
    void *p = NULL;
    int err =
        _mi_prim_alloc(size, try_alignment, commit, allow_large, is_large, is_zero, &p);
    if (err != 0) {
        _mi_warning_message(
            "unable to allocate OS memory (error: %d (0x%x), size: 0x%zx bytes, align: "
            "0x%zx, commit: %d, allow large: %d)\n",
            err,
            err,
            size,
            try_alignment,
...
mi_os_prim_alloc_aligned(
    ...
    void *p = mi_os_prim_alloc(
        size, alignment, commit, allow_large, is_large, is_zero, stats
    );
    if (p == NULL)
        return NULL;
    ...
    if (((uintptr_t)p % alignment) == 0) {
        *base = p;
        ...
        // _mi_warning_message("unable to allocate aligned OS memory directly, fall back
        // to over-allocation (size: 0x%zx bytes, address: %p, alignment: 0x%zx, commit:
        ...
        if (mi_os_mem_config.must_free_whole) {  // win32 virtualAlloc cannot free parts
                                                 // of an allocate block
            ...
            // note: this is dangerous on Windows as VirtualFree needs the actual base
            // pointer this is handled though by having the `base` field in the memid's
            *base = p;  // remember the base
            ...
            // and selectively unmap parts around the over-allocated area. (noop on
            // sbrk)
            ...
            if (post_size > 0) {
                mi_os_prim_free(
                    (uint8_t *)aligned_p + mid_size, post_size, commit, stats
                );
            }
...
_mi_os_alloc_aligned(
    ...
    *memid = _mi_memid_none();
    ...
    bool os_is_zero = false;
    void *os_base = NULL;
    void *p = mi_os_prim_alloc_aligned(
        size,
        alignment,
        commit,
        allow_large,
    ...
    *memid = _mi_memid_none();
    ...
    if (offset == 0) {
        ...
        return _mi_os_alloc_aligned(
            size, alignment, commit, allow_large, memid, tld_stats
        );
    } else {
        // overallocate to align at an offset
        const size_t extra = _mi_align_up(offset, alignment) - offset;
        const size_t oversize = size + extra;
        void *const start = _mi_os_alloc_aligned(
            oversize, alignment, commit, allow_large, memid, tld_stats
        );
        if (start == NULL)
            return NULL;
...
mi_os_page_align_areax(bool conservative, void *addr, size_t size, size_t *newsize) {
    ...
    void *start =
        (conservative ? mi_align_up_ptr(addr, _mi_os_page_size())
                      : mi_align_down_ptr(addr, _mi_os_page_size()));
    void *end =
        (conservative ? mi_align_down_ptr((uint8_t *)addr + size, _mi_os_page_size())
...
_mi_os_commit(void *addr, size_t size, bool *is_zero, mi_stats_t *tld_stats) {
    ...
    if (err != 0) {
        _mi_warning_message(
            "cannot commit OS memory (error: %d (0x%x), address: %p, size: 0x%zx "
            "bytes)\n",
            err,
            err,
            start,
...
static bool
mi_os_decommit_ex(
    void *addr, size_t size, bool *needs_recommit, mi_stats_t *tld_stats
) {
    MI_UNUSED(tld_stats);
    mi_stats_t *stats = &_mi_stats_main;
    ...
    *needs_recommit = true;
    ...
    if (err != 0) {
        _mi_warning_message(
            "cannot decommit OS memory (error: %d (0x%x), address: %p, size: 0x%zx "
            "bytes)\n",
            err,
            err,
            start,
...
_mi_os_reset(void *addr, size_t size, mi_stats_t *stats) {
    ...
    if (err != 0) {
        _mi_warning_message(
            "cannot reset OS memory (error: %d (0x%x), address: %p, size: 0x%zx "
            "bytes)\n",
            err,
            err,
            start,
...
_mi_os_purge_ex(void *p, size_t size, bool allow_reset, mi_stats_t *stats) {
        ...
        !_mi_preloading())  // don't decommit during preloading (unsafe)
    {
        ...
        if (allow_reset) {  // this can sometimes be not allowed if the range is not
                            // fully committed
...
#if (MI_SECURE > 0 || \
     MI_DEBUG == 0)  // security: randomize start of huge pages unless in debug mode
            mi_heap_t *heap = mi_prim_get_default_heap();
            // gh-123022: default heap may not be initialized in CPython in background
            // threads
            if (mi_heap_is_initialized(heap)) {
...
_mi_os_alloc_huge_os_pages(
    ...
    *memid = _mi_memid_none();
    ...
    while (page < pages) {
        ...
        void *p = NULL;
        int err = _mi_prim_alloc_huge_os_pages(
            addr, MI_HUGE_OS_PAGE_SIZE, numa_node, &is_zero, &p
        );
        if (!is_zero) {
            all_zero = false;
        }
        if (err != 0) {
            _mi_warning_message(
                "unable to allocate huge OS page (error: %d (0x%x), address: %p, size: "
                "%zx bytes)\n",
                err,
                err,
                addr,
        ...
        if (p != addr) {
            ...
            if (p != NULL) {
                _mi_warning_message(
                    "could not allocate contiguous huge OS page %zu at %p\n", page, addr
                );
        ...
        if (max_msecs > 0) {
            ...
            if (page >= 1) {
                ...
                if (estimate >
                    2 * max_msecs) {  // seems like we are going to timeout, break
                    elapsed = max_msecs + 1;
                }
            }
            if (elapsed > max_msecs) {
                _mi_warning_message(
                    "huge OS page allocation timed out (after allocating %zu "
                    "page(s))\n",
                    page
                );
                break;
    ...
    if (page != 0) {
        ...
        *memid =
            _mi_memid_create_os(true /* is committed */, all_zero, true /* is_large */);
...
// note: needed with VirtualAlloc but could potentially be done in one go on mmap'd
// systems.
static void
mi_os_free_huge_os_pages(void *p, size_t size, mi_stats_t *stats) {
    if (p == NULL || size == 0)
