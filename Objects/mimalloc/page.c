...
static void
mi_page_init(mi_heap_t *heap, mi_page_t *page, size_t size, mi_tld_t *tld);
static void
...
// mi_assert_internal(tfree_count <= page->thread_freed + 1);
    ...
    size_t free_count = mi_page_list_count(page, page->free) +
                        mi_page_list_count(page, page->local_free);
...
_mi_page_is_valid(mi_page_t *page) {
    ...
    if (mi_page_heap(page) != NULL) {
        ...
        mi_assert_internal(
            !_mi_process_is_initialized || segment->thread_id == 0 ||
...
_mi_page_try_use_delayed_free(
    mi_page_t *page, mi_delayed_t delay, bool override_never
) {
    ...
    do {
        ...
        if mi_unlikely (old_delay == MI_DELAYED_FREEING) {
            ...
            // tfree = mi_tf_set_delayed(tfree, MI_NO_DELAYED_FREE); // will cause CAS
            // to busy fail
...
_mi_page_thread_free_collect(mi_page_t *page) {
    ...
    // if `count > max_count` there was a memory corruption (possibly infinite list due
    // to double multi-threaded free)
    if (count > max_count) {
...
_mi_page_free_collect(mi_page_t *page, bool force) {
    ...
    if (force ||
        mi_page_thread_free(page) != NULL) {  // quick test to avoid an atomic operation
...
static mi_page_t *
mi_page_fresh_alloc(
    mi_heap_t *heap, mi_page_queue_t *pq, size_t block_size, size_t page_alignment
) {
#if !MI_HUGE_PAGE_ABANDON
    mi_assert_internal(pq != NULL);
    mi_assert_internal(mi_heap_contains_queue(heap, pq));
    mi_assert_internal(
        page_alignment > 0 || block_size > MI_MEDIUM_OBJ_SIZE_MAX ||
        block_size == pq->block_size
    );
#endif
    mi_page_t *page = _mi_segment_page_alloc(
        heap, block_size, page_alignment, &heap->tld->segments, &heap->tld->os
    );
    if (page == NULL) {
        // this may be out-of-memory, or an abandoned page was reclaimed (and in our
        // queue)
        return NULL;
    }
    mi_assert_internal(
    ...
    const size_t full_block_size =
        ((pq == NULL || mi_page_queue_is_huge(pq))
...
_mi_heap_delayed_free_partial(mi_heap_t *heap) {
    ...
    while (block != NULL && !mi_atomic_cas_ptr_weak_acq_rel(
                                mi_block_t, &heap->thread_delayed_free, &block, NULL
                            )) { /* nothing */
    };
    bool all_freed = true;
    ...
    while (block != NULL) {
        ...
        if (!_mi_free_delayed_block(block)) {
            ...
            // reinserting the current block into the delayed free list
            all_freed = false;
            mi_block_t *dfree =
                mi_atomic_load_ptr_relaxed(mi_block_t, &heap->thread_delayed_free);
            do {
...
_mi_page_retire(mi_page_t *page) mi_attr_noexcept {
    ...
    if mi_likely (page->xblock_size <= MI_MAX_RETIRE_SIZE &&
                  !mi_page_queue_is_special(pq
                  )) {  // not too large && not full or huge queue?
        if (pq->last == page && pq->first == page) {  // the only page in the queue?
            mi_stat_counter_increase(_mi_stats_main.page_no_retire, 1);
            page->retire_expire =
                1 + (page->xblock_size <= MI_SMALL_OBJ_SIZE_MAX ? MI_RETIRE_CYCLES
                                                                : MI_RETIRE_CYCLES / 4);
            mi_heap_t *heap = mi_page_heap(page);
...
mi_page_free_list_extend_secure(
    ...
    for (size_t i = 0; i < slice_count; i++) {
        blocks[i] =
            mi_page_block_at(page, page_area, bsize, page->capacity + i * slice_extend);
    ...
    for (size_t i = 1; i < extend; i++) {
        ...
        blocks[current] =
            (mi_block_t *)((uint8_t *)block + bsize);  // bump to the following block
        mi_block_set_next(
            page, block, blocks[next]
...
mi_page_free_list_extend(
    ...
    mi_block_t *const last =
        mi_page_block_at(page, page_area, bsize, page->capacity + extend - 1);
...
mi_page_extend_free(mi_heap_t *heap, mi_page_t *page, mi_tld_t *tld) {
    ...
    const size_t bsize =
        (page->xblock_size < MI_HUGE_BLOCK_SIZE ? page->xblock_size : page_size);
    size_t extend = page->reserved - page->capacity;
    mi_assert_internal(extend > 0);

    size_t max_extend =
        (bsize >= MI_MAX_EXTEND_SIZE ? MI_MIN_EXTEND
                                     : MI_MAX_EXTEND_SIZE / (uint32_t)bsize);
    ...
    if (extend < MI_MIN_SLICES ||
        MI_SECURE == 0) {  //! mi_option_is_enabled(mi_option_secure)) {
...
mi_page_init(mi_heap_t *heap, mi_page_t *page, size_t block_size, mi_tld_t *tld) {
    ...
    page->xblock_size =
        (block_size < MI_HUGE_BLOCK_SIZE ? (uint32_t)block_size : MI_HUGE_BLOCK_SIZE
        );  // initialize before _mi_segment_page_start
    size_t page_size;
...
// search through the pages in "next fit" order
    ...
    if (page == NULL) {
        ...
        if (page == NULL && first_try) {
            // out-of-memory _or_ an abandoned page with free blocks was reclaimed, try
            // once again
...
_mi_deferred_free(mi_heap_t *heap, bool force) {
    ...
    if (deferred_free != NULL && !heap->tld->recurse) {
        ...
        deferred_free(
            force, heap->tld->heartbeat, mi_atomic_load_ptr_relaxed(void, &deferred_arg)
        );
...
// Huge pages are also use if the requested alignment is very large (>
// MI_ALIGNMENT_MAX).
static mi_page_t *
mi_large_huge_page_alloc(mi_heap_t *heap, size_t size, size_t page_alignment) {
...
#else
    mi_page_queue_t *pq = mi_page_queue(
        heap, is_huge ? MI_HUGE_BLOCK_SIZE : block_size
    );  // not block_size as that can be low if the page_alignment > 0
    ...
    if (page != NULL) {
...
#if MI_HUGE_PAGE_ABANDON
            mi_assert_internal(
                _mi_page_segment(page)->thread_id == 0
        ...
        );  // note: not `mi_page_block_size` to account for padding
...
mi_find_page(mi_heap_t *heap, size_t size, size_t huge_alignment) mi_attr_noexcept {
    ...
    const size_t req_size =
        size -
        MI_PADDING_SIZE;  // correct for padding_size in case of an overflow on `size`
    if mi_unlikely (req_size > (MI_MEDIUM_OBJ_SIZE_MAX - MI_PADDING_SIZE) ||
        ...
        if mi_unlikely (req_size >
            ...
            _mi_error_message(
                EOVERFLOW, "allocation request is too large (%zu bytes)\n", req_size
            );
            return NULL;
        } else {
...
// Generic allocation routine if the fast path (`alloc.c:mi_page_malloc`) does not
// succeed. Note: in debug mode the size includes MI_PADDING_SIZE and might have
// overflowed. The `huge_alignment` is normally 0 but is set to a multiple of
// MI_SEGMENT_SIZE for very large requested alignments in which case we use a huge
// segment.
void *
_mi_malloc_generic(mi_heap_t *heap, size_t size, bool zero, size_t huge_alignment)
    mi_attr_noexcept {
    ...
    if mi_unlikely (page == NULL) {  // first time out of memory, try to collect and
    ...
    if mi_unlikely (page == NULL) {  // out of memory
        const size_t req_size =
            size - MI_PADDING_SIZE;  // correct for padding_size in case of an overflow
                                     // on `size`
    ...
    // and try again, this time succeeding! (i.e. this should never recurse through
    // _mi_page_malloc)
    if mi_unlikely (zero && page->xblock_size == 0) {
        // note: we cannot call _mi_page_malloc with zeroing for huge blocks; we zero it
