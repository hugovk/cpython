...
#define MI_MAX_ARENAS \
    (112              \
    )  // not more than 126 (since we use 7 bits in the memid and an arena index + 1)
...
typedef struct mi_arena_s {
    ...
    mi_memid_t
        meta_memid;  // memid of the arena structure itself (OS or static allocation)
    int numa_node;   // associated NUMA node
    bool exclusive;  // only allow allocations if specifically for this arena
    bool
        is_large;  // memory area consists of large- or huge OS pages (always committed)
    _Atomic(size_t) search_idx;  // optimization to start the search for free blocks
    _Atomic(mi_msecs_t) purge_expire;     // expiration time when blocks should be
                                          // decommitted from `blocks_decommit`.
    mi_bitmap_field_t *blocks_dirty;      // are the blocks potentially non-zero?
    mi_bitmap_field_t *blocks_committed;  // are the blocks committed? (can be NULL for
                                          // memory that cannot be decommitted)
    mi_bitmap_field_t
        *blocks_purge;  // blocks that can be (reset) decommitted. (can be NULL for
                        // memory that cannot be (reset) decommitted)
    mi_bitmap_field_t
        blocks_inuse[1];  // in-place bitmap of in-use blocks (of size `field_count`)
} mi_arena_t;
...
// static bool mi_manage_os_memory_ex2(void* start, size_t size, bool is_large, int
// numa_node, bool exclusive, mi_memid_t memid, mi_arena_id_t* arena_id)
// mi_attr_noexcept;
...
mi_arena_id_is_suitable(
    mi_arena_id_t arena_id, bool arena_is_exclusive, mi_arena_id_t req_arena_id
) {
    return (
        (!arena_is_exclusive && req_arena_id == _mi_arena_id_none()) ||
...
mi_memid_create_arena(
    mi_arena_id_t id, bool is_exclusive, mi_bitmap_index_t bitmap_index
) {
...
mi_arena_memid_indices(
    mi_memid_t memid, size_t *arena_index, mi_bitmap_index_t *bitmap_index
) {
...
mi_arena_try_claim(mi_arena_t *arena, size_t blocks, mi_bitmap_index_t *bitmap_idx) {
    size_t idx = 0;  // mi_atomic_load_relaxed(&arena->search_idx);  // start from last
    ...
    if (_mi_bitmap_try_find_from_claim_across(
        ...
        mi_atomic_store_relaxed(
            &arena->search_idx, mi_bitmap_index_field(*bitmap_idx)
        );  // start search from found location next time around
        return true;
    };
    return false;
}
...
mi_arena_try_alloc_at(
    ...
    *memid = mi_memid_create_arena(arena->id, arena->exclusive, bitmap_index);
    ...
    if (arena->blocks_purge != NULL) {
        // this is thread safe as a potential purge only decommits parts that are not
        // yet claimed as used (in `blocks_inuse`).
        _mi_bitmap_unclaim_across(
            arena->blocks_purge, arena->field_count, needed_bcount, bitmap_index
        );
    }
    ...
    if (arena->memid.initially_zero && arena->blocks_dirty != NULL) {
        memid->initially_zero = _mi_bitmap_claim_across(
            arena->blocks_dirty, arena->field_count, needed_bcount, bitmap_index, NULL
        );
    }
    ...
    if (arena->blocks_committed == NULL) {
        ...
        // commit requested, but the range may not be committed as a whole: ensure it is
        // committed now
        memid->initially_committed = true;
        ...
        if (any_uncommitted) {
            ...
            if (!_mi_os_commit(
                    p, mi_arena_block_size(needed_bcount), &commit_zero, tld->stats
                )) {
        ...
        memid->initially_committed = _mi_bitmap_is_claimed_across(
            arena->blocks_committed, arena->field_count, needed_bcount, bitmap_index
        );
    }
...
mi_arena_try_alloc(
    ...
    if (req_arena_id != _mi_arena_id_none()) {
        ...
        if (numa_node >= 0) {  // if numa_node was < 0 (no specific affinity requested),
                               // all arena's have been tried already
            for (size_t i = 0; i < max_arena; i++) {
                void *p = mi_arena_try_alloc_at_id(
                    mi_arena_id_create(i),
                    false /* only proceed if not numa local */,
                    numa_node,
                    size,
                    alignment,
                    commit,
...
mi_arena_reserve(
    ...
    if (!_mi_os_has_virtual_reserve()) {
        arena_reserve =
            arena_reserve / 4;  // be conservative if virtual reserve is not supported
    ...
    return (
        mi_reserve_os_memory_ex(
            arena_reserve, arena_commit, allow_large, false /* exclusive */, arena_id
        ) == 0
    );
}
...
_mi_arena_alloc_aligned(
    ...
    *memid = _mi_memid_none();
    ...
    // try to allocate in an arena if the alignment is small enough and the object is
    // not too small (as for heap meta data)
    if (size >= MI_ARENA_MIN_OBJ_SIZE && alignment <= MI_SEGMENT_ALIGN &&
        align_offset == 0) {
        void *p = mi_arena_try_alloc(
            numa_node, size, alignment, commit, allow_large, req_arena_id, memid, tld
        );
        if (p != NULL)
            return p;
    ...
    if (align_offset > 0) {
        return _mi_os_alloc_aligned_at_offset(
            size, alignment, align_offset, commit, allow_large, memid, tld->stats
        );
    } else {
        return _mi_os_alloc_aligned(
            size, alignment, commit, allow_large, memid, tld->stats
        );
    }
}
...
_mi_arena_alloc(
    ...
    return _mi_arena_alloc_aligned(
        size, MI_ARENA_BLOCK_SIZE, 0, commit, allow_large, req_arena_id, memid, tld
    );
}
...
mi_arena_purge(mi_arena_t *arena, size_t bitmap_idx, size_t blocks, mi_stats_t *stats) {
    ...
    if (_mi_bitmap_is_claimed_across(
            arena->blocks_committed, arena->field_count, blocks, bitmap_idx
        )) {
        // all blocks are committed, we can purge freely
        needs_recommit = _mi_os_purge(p, size, stats);
    } else {
        // some blocks are not committed -- this can happen when a partially committed
        // block is freed in `_mi_arena_free` and it is conservatively marked as
        // uncommitted but still scheduled for a purge we need to ensure we do not try
        // to reset (as that may be invalid for uncommitted memory), and also undo the
        // decommit stats (as it was already adjusted)
    ...
    _mi_bitmap_unclaim_across(
        arena->blocks_purge, arena->field_count, blocks, bitmap_idx
    );
...
mi_arena_schedule_purge(
    mi_arena_t *arena, size_t bitmap_idx, size_t blocks, mi_stats_t *stats
) {
    mi_assert_internal(arena->blocks_purge != NULL);
    ...
    if (_mi_preloading() || delay == 0) {
        ...
        if (expire != 0) {
            mi_atomic_addi64_acq_rel(
                &arena->purge_expire, delay / 10
            );  // add smallish extra delay
        } else {
            mi_atomic_storei64_release(&arena->purge_expire, _mi_clock_now() + delay);
        }
        _mi_bitmap_claim_across(
            arena->blocks_purge, arena->field_count, blocks, bitmap_idx, NULL
        );
    }
}
...
mi_arena_try_purge(mi_arena_t *arena, mi_msecs_t now, bool force, mi_stats_t *stats) {
    ...
    if (!full_purge) {
        ...
        mi_atomic_casi64_strong_acq_rel(
            &arena->purge_expire, &expected, _mi_clock_now() + delay
        );
    }
    return any_purged;
}
...
void
_mi_arena_free(
    void *p, size_t size, size_t committed_size, mi_memid_t memid, mi_stats_t *stats
) {
    ...
    if (mi_memkind_is_os(memid.memkind)) {
        ...
        if (!all_committed && committed_size > 0) {
            // if partially committed, adjust the committed stats (as `_mi_os_free` will
            // increase decommit by the full size)
        ...
        if (arena->field_count <= mi_bitmap_index_field(bitmap_idx)) {
            _mi_error_message(
                EINVAL,
                "trying to free from non-existent arena block: %p, size %zu, memid: "
                "0x%zx\n",
                p,
                size,
                memid
        ...
        if (arena->memid.is_pinned || arena->blocks_committed == NULL) {
            ...
            if (!all_committed) {
                // mark the entire range as no longer committed (so we recommit the full
                // range when re-using)
                _mi_bitmap_unclaim_across(
                    arena->blocks_committed, arena->field_count, blocks, bitmap_idx
                );
                ...
                if (committed_size > 0) {
                    ...
                    // recommitted when re-using) in the delayed purge, we now need to
                    // not count a decommit if the range is not marked as committed.
                    _mi_stat_decrease(&stats->committed, committed_size);
                }
...
// `mi_option_destroy_on_exit` for dynamic libraries that are unloaded and need to
// release all their allocated memory.
static void
    ...
    for (size_t i = 0; i < max_arena; i++) {
        ...
        if (arena != NULL) {
            if (arena->start != NULL && mi_memkind_is_os(arena->memid.memkind)) {
                ...
                _mi_os_free(
                    arena->start, mi_arena_size(arena), arena->memid, &_mi_stats_main
                );
            } else {
                new_max_arena = i;
            }
            mi_arena_meta_free(
                arena, arena->meta_memid, arena->meta_size, &_mi_stats_main
            );
        }
    }
...
// Purge the arenas; if `force_purge` is true, amenable parts are purged even if not yet
// expired
void
...
mi_manage_os_memory_ex2(
    ...
    mi_arena_t *arena = (mi_arena_t *)mi_arena_meta_zalloc(
        asize, &meta_memid, &_mi_stats_main
    ...
    arena->numa_node = numa_node;  // TODO: or get the current numa node if -1? (now it
                                   // allows anyone to allocate on -1)
    arena->is_large = is_large;
    arena->purge_expire = 0;
    arena->search_idx = 0;
    arena->blocks_dirty = &arena->blocks_inuse[fields];  // just after inuse bitmap
    arena->blocks_committed =
        (arena->memid.is_pinned ? NULL : &arena->blocks_inuse[2 * fields]
        );  // just after dirty bitmap
    arena->blocks_purge =
        (arena->memid.is_pinned ? NULL : &arena->blocks_inuse[3 * fields]
        );  // just after committed bitmap
    // initialize committed bitmap?
    if (arena->blocks_committed != NULL && arena->memid.initially_committed) {
        memset(
            (void *)arena->blocks_committed, 0xFF, fields * sizeof(mi_bitmap_field_t)
        );  // cast to void* to avoid atomic warning
    }
...
int
mi_reserve_os_memory_ex(
    size_t size, bool commit, bool allow_large, bool exclusive, mi_arena_id_t *arena_id
) mi_attr_noexcept {
    if (arena_id != NULL)
        *arena_id = _mi_arena_id_none();
    size = _mi_align_up(size, MI_ARENA_BLOCK_SIZE);  // at least one block
    mi_memid_t memid;
    void *start = _mi_os_alloc_aligned(
        size, MI_SEGMENT_ALIGN, commit, allow_large, &memid, &_mi_stats_main
    );
    if (start == NULL)
        return ENOMEM;
    const bool is_large = memid.is_pinned;  // todo: use separate is_large field?
    if (!mi_manage_os_memory_ex2(
            start, size, is_large, -1 /* numa node */, exclusive, memid, arena_id
        )) {
...
mi_debug_show_bitmap(
    const char *prefix, mi_bitmap_field_t *fields, size_t field_count
) {
...
mi_debug_show_arenas(void) mi_attr_noexcept {
    ...
    for (size_t i = 0; i < max_arenas; i++) {
        ...
        size_t inuse_count = 0;
        _mi_verbose_message(
            "arena %zu: %zu blocks with %zu fields\n",
            i,
            arena->block_count,
            arena->field_count
        );
        inuse_count +=
            mi_debug_show_bitmap("  ", arena->blocks_inuse, arena->field_count);
...
mi_reserve_huge_os_pages_at_ex(
    ...
    mi_memid_t memid;
    void *p = _mi_os_alloc_huge_os_pages(
        pages, numa_node, timeout_msecs, &pages_reserved, &hsize, &memid
    );
    ...
    if (!mi_manage_os_memory_ex2(
            p, hsize, true, numa_node, exclusive, memid, arena_id
        )) {
...
// reserve huge pages evenly among the given number of numa nodes (or use the available
// ones as detected)
int
mi_reserve_huge_os_pages_interleave(
    size_t pages, size_t numa_nodes, size_t timeout_msecs
) mi_attr_noexcept {
    if (pages == 0)
        return 0;
...
mi_reserve_huge_os_pages(size_t pages, double max_secs, size_t *pages_reserved)
    ...
    MI_UNUSED(max_secs);
    _mi_warning_message(
        "mi_reserve_huge_os_pages is deprecated: use "
        "mi_reserve_huge_os_pages_interleave/at instead\n"
    );
    if (pages_reserved != NULL)
        *pages_reserved = 0;
    int err =
        mi_reserve_huge_os_pages_interleave(pages, 0, (size_t)(max_secs * 1000.0));
