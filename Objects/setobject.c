...
static int
set_table_resize(PySetObject *, Py_ssize_t);
...
set_repr_lock_held(PySetObject *so) {
    ...
    if (!PySet_CheckExact(so))
        result = PyUnicode_FromFormat("%s({%U})", Py_TYPE(so)->tp_name, listrepr);
    else
    ...
    if (so->fill == 0) {
        ...
        for (i = other->mask + 1; i > 0; i--, other_entry++) {
            ...
            if (key != NULL && key != dummy) {
                set_insert_clean(newtable, newmask, Py_NewRef(key), other_entry->hash);
            }
        }
        return 0;
    }
...
set_traverse(PyObject *self, visitproc visit, void *arg) {
    ...
    while (set_next(so, &pos, &entry)) Py_VISIT(entry->key);
    return 0;
}
...
typedef struct {
    PyObject_HEAD PySetObject *si_set; /* Set to NULL when iterator is exhausted */
    Py_ssize_t si_used;
    Py_ssize_t si_pos;
    Py_ssize_t len;
} setiterobject;
...
/*[clinic end generated code: output=fbb049c0806028de input=a50acf0365e1f0a5]*/
{
    ...
    if (PyDict_CheckExact(other)) {
        ...
        PySetObject *otherset =
            (PySetObject *)make_new_set_basetype(Py_TYPE(so), other);
        if (otherset == NULL) {
            return NULL;
        }
    ...
    switch (op) {
        case Py_EQ:
            if (PySet_GET_SIZE(v) != PySet_GET_SIZE(w))
                Py_RETURN_FALSE;
            if (v->hash != -1 && ((PySetObject *)w)->hash != -1 &&
...
static PyMethodDef set_methods[] = {
    SET_ADD_METHODDEF SET_CLEAR_METHODDEF SET___CONTAINS___METHODDEF SET_COPY_METHODDEF
        SET_DISCARD_METHODDEF SET_DIFFERENCE_MULTI_METHODDEF
            SET_DIFFERENCE_UPDATE_METHODDEF SET_INTERSECTION_MULTI_METHODDEF
                SET_INTERSECTION_UPDATE_MULTI_METHODDEF SET_ISDISJOINT_METHODDEF
                    SET_ISSUBSET_METHODDEF SET_ISSUPERSET_METHODDEF SET_POP_METHODDEF
                        SET___REDUCE___METHODDEF SET_REMOVE_METHODDEF
                            SET___SIZEOF___METHODDEF SET_SYMMETRIC_DIFFERENCE_METHODDEF
                                SET_SYMMETRIC_DIFFERENCE_UPDATE_METHODDEF
                                    SET_UNION_METHODDEF SET_UPDATE_METHODDEF{
                                        "__class_getitem__",
                                        Py_GenericAlias,
                                        METH_O | METH_CLASS,
...
static PyMethodDef frozenset_methods[] = {
    SET___CONTAINS___METHODDEF FROZENSET_COPY_METHODDEF SET_DIFFERENCE_MULTI_METHODDEF
        SET_INTERSECTION_MULTI_METHODDEF SET_ISDISJOINT_METHODDEF SET_ISSUBSET_METHODDEF
            SET_ISSUPERSET_METHODDEF SET___REDUCE___METHODDEF SET___SIZEOF___METHODDEF
                SET_SYMMETRIC_DIFFERENCE_METHODDEF SET_UNION_METHODDEF{
                    "__class_getitem__",
                    Py_GenericAlias,
                    METH_O | METH_CLASS,
                    PyDoc_STR("See PEP 585")
...
static PyTypeObject _PySetDummy_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type, 0) "<dummy key> type",
    0,
    0,
    dummy_dealloc,
