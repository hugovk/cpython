...
PyCMethod_New(PyMethodDef *ml, PyObject *self, PyObject *module, PyTypeObject *cls) {
    ...
    switch (ml->ml_flags & (METH_VARARGS | METH_FASTCALL | METH_NOARGS | METH_O |
        ...
        case METH_METHOD | METH_FASTCALL | METH_KEYWORDS:
            ...
            break;
        default:
            PyErr_Format(PyExc_SystemError, "%s() method: bad call flags", ml->ml_name);
            return NULL;
    }
...
meth_reduce(PyObject *self, PyObject *Py_UNUSED(ignored)) {
    ...
    return Py_BuildValue(
        "N(Os)", _PyEval_GetBuiltin(&_Py_ID(getattr)), m->m_self, m->m_ml->ml_name
    );
}

static PyMethodDef meth_methods[] = {
    {"__reduce__", meth_reduce, METH_NOARGS, NULL}, {NULL, NULL}
};
...
meth_get__text_signature__(PyObject *self, void *closure) {
    ...
    return _PyType_GetTextSignatureFromInternalDoc(
        m->m_ml->ml_name, m->m_ml->ml_doc, m->m_ml->ml_flags
    );
}
...
meth_get__qualname__(PyObject *self, void *closure) {
    ...
    PyObject *type =
        PyType_Check(m->m_self) ? m->m_self : (PyObject *)Py_TYPE(m->m_self);
...
static PyMemberDef meth_members[] = {
    {"__module__", _Py_T_OBJECT, OFF(m_module), 0}, {NULL}
};
...
meth_repr(PyObject *self) {
    PyCFunctionObject *m = _PyCFunctionObject_CAST(self);
    if (m->m_self == NULL || PyModule_Check(m->m_self)) {
        return PyUnicode_FromFormat("<built-in function %s>", m->m_ml->ml_name);
    }
...
meth_richcompare(PyObject *self, PyObject *other, int op) {
    ...
    if ((op != Py_EQ && op != Py_NE) || !PyCFunction_Check(self) ||
...
PyTypeObject PyCFunction_Type = {
    ...
    0,                                       /* tp_setattro */
    0,                                       /* tp_as_buffer */
    Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC | Py_TPFLAGS_HAVE_VECTORCALL, /* tp_flags */
    0,                                                                    /* tp_doc */
...
PyTypeObject PyCMethod_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type, 0).tp_name = "builtin_method",
...
cfunction_check_kwargs(PyThreadState *tstate, PyObject *func, PyObject *kwnames) {
    ...
    if (kwnames && PyTuple_GET_SIZE(kwnames)) {
        ...
        if (funcstr != NULL) {
            _PyErr_Format(
                tstate, PyExc_TypeError, "%U takes no keyword arguments", funcstr
            );
            Py_DECREF(funcstr);
        }
        return -1;
...
cfunction_vectorcall_FASTCALL(
    ...
    PyCFunctionFast meth = (PyCFunctionFast)cfunction_enter_call(tstate, func);
    if (meth == NULL) {
        return NULL;
    }
...
cfunction_vectorcall_FASTCALL_KEYWORDS(
    ...
    PyCFunctionFastWithKeywords meth =
        (PyCFunctionFastWithKeywords)cfunction_enter_call(tstate, func);
    if (meth == NULL) {
        return NULL;
    }
...
cfunction_vectorcall_NOARGS(
    ...
    PyObject *result =
        _PyCFunction_TrampolineCall(meth, PyCFunction_GET_SELF(func), NULL);
    _Py_LeaveRecursiveCallTstate(tstate);
    return result;
}
...
cfunction_vectorcall_O(
    ...
    PyObject *result =
        _PyCFunction_TrampolineCall(meth, PyCFunction_GET_SELF(func), args[0]);
    _Py_LeaveRecursiveCallTstate(tstate);
    return result;
}
...
cfunction_call(PyObject *func, PyObject *args, PyObject *kwargs) {
    ...
    if (flags & METH_KEYWORDS) {
        result = _PyCFunctionWithKeywords_TrampolineCall(
            (*(PyCFunctionWithKeywords)(void (*)(void))meth), self, args, kwargs
        );
    } else {
