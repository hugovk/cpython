...
PyPickleBuffer_GetBuffer(PyObject *obj) {
    ...
    if (self->view.obj == NULL) {
        PyErr_SetString(
            PyExc_ValueError, "operation forbidden on released PickleBuffer object"
        );
        return NULL;
    }
    return &self->view;
...
picklebuf_new(PyTypeObject *type, PyObject *args, PyObject *kwds) {
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O:PickleBuffer", keywords, &base)) {
        return NULL;
    }
...
picklebuf_raw(PyPickleBufferObject *self, PyObject *Py_UNUSED(ignored)) {
    ...
    if (self->view.suboffsets != NULL || !PyBuffer_IsContiguous(&self->view, 'A')) {
        PyErr_SetString(
            PyExc_BufferError, "cannot extract raw buffer from non-contiguous buffer"
        );
        return NULL;
    }
...
PyTypeObject PyPickleBuffer_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "pickle.PickleBuffer",
