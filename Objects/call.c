...
_Py_CheckFunctionResult(
    PyThreadState *tstate, PyObject *callable, PyObject *result, const char *where
) {
...
_PyObject_VectorcallDictTstate(
    ...
    if (kwargs == NULL || PyDict_GET_SIZE(kwargs) == 0) {
        ...
        newargs = _PyStack_UnpackDict(tstate, args, nargs, kwargs, &kwnames);
        if (newargs == NULL) {
            return NULL;
        }
        res = func(callable, newargs, nargs | PY_VECTORCALL_ARGUMENTS_OFFSET, kwnames);
...
object_is_not_callable(PyThreadState *tstate, PyObject *callable) {
    if (Py_IS_TYPE(callable, &PyModule_Type)) {
        ...
        // TypeError: 'module' object is not callable. Did you mean:
        // 'pprint.pprint(...)'?
...
_PyVectorcall_Call(
    ...
    PyObject *const *args;
    PyObject *kwnames;
    args = _PyStack_UnpackDict(tstate, _PyTuple_ITEMS(tuple), nargs, kwargs, &kwnames);
    if (args == NULL) {
        return NULL;
    }
    PyObject *result =
        func(callable, args, nargs | PY_VECTORCALL_ARGUMENTS_OFFSET, kwnames);
...
PyAPI_FUNC(PyObject *) PyEval_CallObjectWithKeywords(
    ...
    if (args != NULL && !PyTuple_Check(args)) {
        _PyErr_SetString(tstate, PyExc_TypeError, "argument list must be a tuple");
        return NULL;
    }

    if (kwargs != NULL && !PyDict_Check(kwargs)) {
        _PyErr_SetString(tstate, PyExc_TypeError, "keyword list must be a dictionary");
        return NULL;
    }

    if (args == NULL) {
        return _PyObject_VectorcallDictTstate(tstate, callable, NULL, 0, kwargs);
    } else {
        return _PyObject_Call(tstate, callable, args, kwargs);
    }
}
...
_PyObject_Call_Prepend(
    ...
    stack[0] = obj;
    memcpy(&stack[1], _PyTuple_ITEMS(args), argcount * sizeof(PyObject *));

    PyObject *result =
        _PyObject_VectorcallDictTstate(tstate, callable, stack, argcount + 1, kwargs);
...
_PyObject_CallFunctionVa(
    PyThreadState *tstate, PyObject *callable, const char *format, va_list va
) {
    PyObject *small_stack[_PY_FASTCALL_SMALL_STACK];
    ...
    if (nargs == 1 && PyTuple_Check(stack[0])) {
        ...
        result = _PyObject_VectorcallTstate(
            tstate, callable, _PyTuple_ITEMS(args), PyTuple_GET_SIZE(args), NULL
        );
    } else {
...
PyObject *
_PyObject_CallMethod(PyObject *obj, PyObject *name, const char *format, ...) {
    PyThreadState *tstate = _PyThreadState_GET();
    if (obj == NULL || name == NULL) {
        return null_error(tstate);
    }
...
PyObject *
_PyObject_CallMethodId(PyObject *obj, _Py_Identifier *name, const char *format, ...) {
    PyThreadState *tstate = _PyThreadState_GET();
    if (obj == NULL || name == NULL) {
        return null_error(tstate);
    }
...
static PyObject *
object_vacall(
    PyThreadState *tstate, PyObject *base, PyObject *callable, va_list vargs
) {
    PyObject *small_stack[_PY_FASTCALL_SMALL_STACK];
    PyObject **stack;
    Py_ssize_t nargs;
...
_PyStack_AsDict(PyObject *const *values, PyObject *kwnames) {
    ...
    return _PyDict_FromItems(&PyTuple_GET_ITEM(kwnames, 0), 1, values, 1, nkwargs);
}
...
_PyStack_UnpackDict(
    ...
    if (!keys_are_strings) {
        _PyErr_SetString(tstate, PyExc_TypeError, "keywords must be strings");
...
void
_PyStack_UnpackDict_Free(PyObject *const *stack, Py_ssize_t nargs, PyObject *kwnames) {
