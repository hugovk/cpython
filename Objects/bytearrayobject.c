...
bytearray_getbuffer(PyObject *self, Py_buffer *view, int flags) {
    ...
    if (view == NULL) {
        PyErr_SetString(
            PyExc_BufferError, "bytearray_getbuffer: view==NULL argument is obsolete"
        );
        return -1;
    }
...
_canresize(PyByteArrayObject *self) {
    if (self->ob_exports > 0) {
        PyErr_SetString(
            PyExc_BufferError, "Existing exports of data: object cannot be re-sized"
        );
        return 0;
    }
    return 1;
...
_PyByteArray_FromBufferObject(PyObject *obj) {
    ...
    if (result != NULL &&
        PyBuffer_ToContiguous(PyByteArray_AS_STRING(result), &view, view.len, 'C') <
            0) {
        Py_CLEAR(result);
    }
...
PyByteArray_FromStringAndSize(const char *bytes, Py_ssize_t size) {
    ...
    if (size < 0) {
        PyErr_SetString(
            PyExc_SystemError, "Negative size passed to PyByteArray_FromStringAndSize"
        );
        return NULL;
    }
...
PyByteArray_Concat(PyObject *a, PyObject *b) {
    ...
    result = (PyByteArrayObject *)PyByteArray_FromStringAndSize(NULL, va.len + vb.len);
...
bytearray_repeat(PyObject *op, Py_ssize_t count) {
    ...
    PyByteArrayObject *result =
        (PyByteArrayObject *)PyByteArray_FromStringAndSize(NULL, size);
...
bytearray_subscript(PyObject *op, PyObject *index) {
    ...
    if (_PyIndex_Check(index)) {
        ...
        slicelength =
            PySlice_AdjustIndices(PyByteArray_GET_SIZE(self), &start, &stop, step);
...
bytearray_setslice_linear(
    ...
    if (growth < 0) {
        ...
        if (lo == 0) {
            ...
            */
            memmove(buf + lo + bytes_len, buf + hi, Py_SIZE(self) - hi);
        }
        if (PyByteArray_Resize((PyObject *)self, Py_SIZE(self) + growth) < 0) {
        ...
        /*
          ...
          0   lo            new_hi              new_size
         */
        memmove(buf + lo + bytes_len, buf + hi, Py_SIZE(self) - lo - bytes_len);
    }
...
static int
bytearray_setslice(
    PyByteArrayObject *self, Py_ssize_t lo, Py_ssize_t hi, PyObject *values
) {
    Py_ssize_t needed;
    void *bytes;
    Py_buffer vbytes;
    ...
    if (values == (PyObject *)self) {
        ...
        int err;
        values = PyByteArray_FromStringAndSize(
            PyByteArray_AS_STRING(values), PyByteArray_GET_SIZE(values)
        );
        if (values == NULL)
            return -1;
    ...
    if (step == 1) {
        ...
        if (needed == 0) {
            ...
            for (cur = start, i = 0; i < slicelen; cur += step, i++) {
                ...
                memmove(buf + cur - i, buf + cur + 1, lim);
            }
            /* Move the tail of the bytes, in one chunk */
            cur = start + (size_t)slicelen * step;
            if (cur < (size_t)PyByteArray_GET_SIZE(self)) {
                memmove(
                    buf + cur - slicelen, buf + cur, PyByteArray_GET_SIZE(self) - cur
                );
            }
            if (PyByteArray_Resize(
                    (PyObject *)self, PyByteArray_GET_SIZE(self) - slicelen
                ) < 0)
                return -1;
...
bytearray___init___impl(
    PyByteArrayObject *self, PyObject *arg, const char *encoding, const char *errors
)
/*[clinic end generated code: output=4ce1304649c2f8b3 input=1141a7122eefd7b9]*/
{
    ...
    if (PyObject_CheckBuffer(arg)) {
        ...
        if (PyByteArray_Resize((PyObject *)self, size) < 0)
            goto fail;
        if (PyBuffer_ToContiguous(PyByteArray_AS_STRING(self), &view, size, 'C') < 0)
            goto fail;
        PyBuffer_Release(&view);
        return 0;
fail:
...
bytearray_repr(PyObject *op) {
    ...
    if (length > (PY_SSIZE_T_MAX - 6 - newsize) / 4) {
        PyErr_SetString(
            PyExc_OverflowError, "bytearray object is too large to make repr"
        );
        return NULL;
    }
    ...
    p = buffer;
    while (*className) *p++ = *className++;
    while (*quote_prefix) *p++ = *quote_prefix++;
    *p++ = quote;
...
bytearray_str(PyObject *op) {
    if (_Py_GetConfig()->bytes_warning) {
        if (PyErr_WarnEx(PyExc_BytesWarning, "str() on a bytearray instance", 1)) {
            return NULL;
        }
    }
...
bytearray_richcompare(PyObject *self, PyObject *other, int op) {
    ...
    if (!PyObject_CheckBuffer(self) || !PyObject_CheckBuffer(other)) {
        if (PyUnicode_Check(self) || PyUnicode_Check(other)) {
            if (_Py_GetConfig()->bytes_warning && (op == Py_EQ || op == Py_NE)) {
                if (PyErr_WarnEx(
                        PyExc_BytesWarning, "Comparison between bytearray and string", 1
                    ))
                    return NULL;
            }
        }
    ...
    if (self_size != other_size && (op == Py_EQ || op == Py_NE)) {
        ...
        cmp = memcmp(self_bytes.buf, other_bytes.buf, Py_MIN(self_size, other_size));
...
bytearray_dealloc(PyObject *op) {
    ...
    if (self->ob_exports > 0) {
        PyErr_SetString(
            PyExc_SystemError, "deallocated bytearray object has exported buffers"
        );
        PyErr_Print();
    }
    if (self->ob_bytes != 0) {
...
static PyObject *
bytearray_find_impl(
    PyByteArrayObject *self, PyObject *sub, Py_ssize_t start, Py_ssize_t end
)
/*[clinic end generated code: output=413e1cab2ae87da0 input=793dfad803e2952f]*/
{
    return _Py_bytes_find(
        PyByteArray_AS_STRING(self), PyByteArray_GET_SIZE(self), sub, start, end
    );
}
...
{
    return PyByteArray_FromStringAndSize(
        PyByteArray_AS_STRING((PyObject *)self), PyByteArray_GET_SIZE(self)
    );
}
...
bytearray_contains(PyObject *self, PyObject *arg) {
    return _Py_bytes_contains(
        PyByteArray_AS_STRING(self), PyByteArray_GET_SIZE(self), arg
    );
}
...
static PyObject *
bytearray_startswith_impl(
    PyByteArrayObject *self, PyObject *subobj, Py_ssize_t start, Py_ssize_t end
)
/*[clinic end generated code: output=a3d9b6d44d3662a6 input=76385e0b376b45c1]*/
{
    return _Py_bytes_startswith(
        PyByteArray_AS_STRING(self), PyByteArray_GET_SIZE(self), subobj, start, end
    );
}
...
/*[clinic end generated code: output=6cabc585e7f502e0 input=968aada38aedd262]*/
{
    ...
    if (self_len >= prefix_len && memcmp(self_start, prefix_start, prefix_len) == 0) {
        return PyByteArray_FromStringAndSize(
...
/*[clinic end generated code: output=2bc8cfb79de793d3 input=c1827e810b2f6b99]*/
{
    ...
    if (self_len >= suffix_len &&
        ...
        return PyByteArray_FromStringAndSize(self_start, self_len - suffix_len);
    }
...
bytearray_translate_impl(
    PyByteArrayObject *self, PyObject *table, PyObject *deletechars
)
...
bytearray_replace_impl(
    PyByteArrayObject *self, Py_buffer *old, Py_buffer *new, Py_ssize_t count
)
...
/*[clinic end generated code: output=76c775a70e7b07b7 input=b2b5d07e9de6c070]*/
{
    ...
    if (n == PY_SSIZE_T_MAX) {
        PyErr_SetString(PyExc_OverflowError, "cannot add more objects to bytearray");
        return NULL;
    }
...
/*[clinic end generated code: output=98155dbe249170b1 input=c617b3a93249ba28]*/
{
    ...
    if (PyObject_CheckBuffer(iterable_of_ints)) {
        if (bytearray_setslice(self, Py_SIZE(self), Py_SIZE(self), iterable_of_ints) ==
            -1)
            return NULL;
    ...
    while ((item = PyIter_Next(it)) != NULL) {
        if (!_getbytevalue(item, &value)) {
            if (PyErr_ExceptionMatches(PyExc_TypeError) &&
                ...
                PyErr_Format(
                    PyExc_TypeError, "expected iterable of integers; got: 'str'"
                );
            }
            Py_DECREF(item);
            Py_DECREF(it);
    ...
    if (striptype != LEFTSTRIP) {
        do {
            right--;
                 ...
                 memchr(bytesptr, (unsigned char)myptr[right], byteslen));
        right++;
    }
    if (bytes != Py_None)
        PyBuffer_Release(&vbytes);
...
static PyMethodDef bytearray_methods[] = {
    ...
    BYTEARRAY_REDUCE_METHODDEF BYTEARRAY_REDUCE_EX_METHODDEF BYTEARRAY_SIZEOF_METHODDEF
        BYTEARRAY_APPEND_METHODDEF{
            "capitalize", stringlib_capitalize, METH_NOARGS, _Py_capitalize__doc__
        },
    STRINGLIB_CENTER_METHODDEF BYTEARRAY_CLEAR_METHODDEF BYTEARRAY_COPY_METHODDEF
        BYTEARRAY_COUNT_METHODDEF BYTEARRAY_DECODE_METHODDEF
            BYTEARRAY_ENDSWITH_METHODDEF STRINGLIB_EXPANDTABS_METHODDEF
                BYTEARRAY_EXTEND_METHODDEF BYTEARRAY_FIND_METHODDEF
                    BYTEARRAY_FROMHEX_METHODDEF BYTEARRAY_HEX_METHODDEF
                        BYTEARRAY_INDEX_METHODDEF BYTEARRAY_INSERT_METHODDEF{
                            "isalnum",
                            stringlib_isalnum,
                            METH_NOARGS,
                            _Py_isalnum__doc__
    ...
    BYTEARRAY_JOIN_METHODDEF STRINGLIB_LJUST_METHODDEF{
        "lower", stringlib_lower, METH_NOARGS, _Py_lower__doc__
    },
    BYTEARRAY_LSTRIP_METHODDEF BYTEARRAY_MAKETRANS_METHODDEF
        BYTEARRAY_PARTITION_METHODDEF BYTEARRAY_POP_METHODDEF BYTEARRAY_REMOVE_METHODDEF
            BYTEARRAY_REPLACE_METHODDEF BYTEARRAY_REMOVEPREFIX_METHODDEF
                BYTEARRAY_REMOVESUFFIX_METHODDEF BYTEARRAY_REVERSE_METHODDEF
                    BYTEARRAY_RFIND_METHODDEF BYTEARRAY_RINDEX_METHODDEF
                        STRINGLIB_RJUST_METHODDEF BYTEARRAY_RPARTITION_METHODDEF
                            BYTEARRAY_RSPLIT_METHODDEF BYTEARRAY_RSTRIP_METHODDEF
                                BYTEARRAY_SPLIT_METHODDEF BYTEARRAY_SPLITLINES_METHODDEF
    ...
    STRINGLIB_ZFILL_METHODDEF{NULL}
};
...
bytearrayiter_next(bytesiterobject *it) {
    ...
    if (it->it_index < PyByteArray_GET_SIZE(seq)) {
        return _PyLong_FromUnsignedChar((unsigned char)PyByteArray_AS_STRING(seq
        )[it->it_index++]);
    }
...
static PyMethodDef bytearrayiter_methods[] = {
    {"__length_hint__",
     (PyCFunction)bytearrayiter_length_hint,
     METH_NOARGS,
     length_hint_doc},
    {"__reduce__",
     (PyCFunction)bytearrayiter_reduce,
     METH_NOARGS,
     bytearray_reduce__doc__},
    {"__setstate__", (PyCFunction)bytearrayiter_setstate, METH_O, setstate_doc},
    {NULL, NULL} /* sentinel */
};
