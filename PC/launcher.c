...
error(int rc, wchar_t *format, ...) {
    ...
    if (rc == 0) { /* a Windows error */
        ...
        if (len >= 0) {
            _snwprintf_s(
                &message[len], MSGSIZE - len, _TRUNCATE, L": %ls", win_message
            );
        }
    }
...
#define LOOKASIDE_PATH \
    L"SOFTWARE\\Microsoft\\AppModel\\Lookaside\\user\\Software\\Python\\PythonCore"
...
_locate_pythons_for_key(
    HKEY root, LPCWSTR subkey, REGSAM flags, int bits, int display_name_only
) {
    ...
    if (status != ERROR_SUCCESS)
        debug(
            L"locate_pythons_for_key: unable to open PythonCore key in %ls\n", key_name
        );
    else {
        ...
        for (i = 0; num_installed_pythons < MAX_INSTALLED_PYTHONS; i++) {
            ...
            if (status != ERROR_SUCCESS) {
                if (status != ERROR_NO_MORE_ITEMS) {
                    ...
                    debug(
                        L"Can't enumerate registry key for version %ls: %ls\n",
                        ip_version,
                        message
                    );
                }
                ...
                wcsncpy_s(
                    ip->version, MAX_VERSION_SIZE, ip_version, MAX_VERSION_SIZE - 1
                );
                ...
                status = RegQueryValueExW(
                    ip_key,
                    EXECUTABLEPATH_VALUE,
                    NULL,
                    &type,
                    (LPBYTE)ip->executable,
                    &data_size
                );
#else
                ...
                if (status != ERROR_SUCCESS || type != REG_SZ || !data_size) {
                    ...
                    data_size = sizeof(ip->executable) - 1;
                    status = RegQueryValueExW(
                        ip_key, NULL, NULL, &type, (LPBYTE)ip->executable, &data_size
                    );
                    if (status != ERROR_SUCCESS) {
                        winerror(status, message, MSGSIZE);
                ...
                for (checkp = location_checks; *checkp; ++checkp) {
                    ...
                    if (attrs == INVALID_FILE_ATTRIBUTES) {
                        ...
                        debug(
                            L"locate_pythons_for_key: %ls: %ls", ip->executable, message
                        );
...
compare_pythons(const void *p1, const void *p2) {
    ...
    int result = CompareStringW(
        LOCALE_INVARIANT, SORT_DIGITSASNUMBERS, ip2->version, -1, ip1->version, -1
    );
    switch (result) {
        case 0:
...
locate_store_pythons(void) {
    ...
    _locate_pythons_for_key(HKEY_LOCAL_MACHINE, LOOKASIDE_PATH, KEY_READ, 64, TRUE);
#else
    /* 32bit process, so check that we're on 64bit OS */
    BOOL f64 = FALSE;
    if (IsWow64Process(GetCurrentProcess(), &f64) && f64) {
        _locate_pythons_for_key(
            HKEY_LOCAL_MACHINE, LOOKASIDE_PATH, KEY_READ | KEY_WOW64_64KEY, 64, TRUE
        );
    }
#endif
}
...
find_python_by_version(wchar_t const *wanted_ver) {
    ...
    if (wcsstr(wanted_ver, L"-32")) {
        ...
        wlen -= wcslen(L"-32");
                   ...
                   wanted_ver, L"-64"
               )) { /* Added option to select 64 bit explicitly */
        bits = 64;
        wlen -= wcslen(L"-64");
    }
...
get_configured_value(wchar_t *key) {
    ...
    if (result == NULL && appdata_ini_path[0]) {
        ...
        size = GetPrivateProfileStringW(
            L"defaults", key, NULL, configured_value, MSGSIZE, appdata_ini_path
        );
        if (size > 0) {
    ...
    if (result == NULL && launcher_ini_path[0]) {
        ...
        size = GetPrivateProfileStringW(
            L"defaults", key, NULL, configured_value, MSGSIZE, launcher_ini_path
        );
        if (size > 0) {
            result = configured_value;
    ...
    if (result) {
        debug(
            L"found configured value '%ls=%ls' in %ls\n",
            key,
            result,
            found_in ? found_in : L"(unknown)"
        );
    } else {
...
locate_python(wchar_t *wanted_ver, BOOL from_shebang) {
    ...
    static wchar_t *last_char = &config_key[sizeof(config_key) / sizeof(wchar_t) - 2];
    INSTALLED_PYTHON *result = NULL;
...
locate_wrapped_script(void) {
    ...
    if (p == NULL) {
        debug(
            L"GetModuleFileNameW returned value has no extension: %ls\n",
            wrapped_script_path
        );
...
invoke_child(wchar_t *executable, wchar_t *suffix, wchar_t *cmdline) {
    ...
    if (no_suffix && no_cmdline)
        ...
        if (no_suffix) {
            ...
            child_command_size =
                wcslen(executable) + wcslen(suffix) + wcslen(cmdline) + 3;
        }
        child_command = calloc(child_command_size, sizeof(wchar_t));
        if (child_command == NULL)
            error(
                RC_CREATE_PROCESS,
                L"unable to allocate %zd bytes for child command.",
                child_command_size
            );
        if (no_suffix)
            _snwprintf_s(
...
find_on_path(wchar_t *name) {
    ...
    if (wcschr(name, L'.') != NULL) {
        ...
        if (rc == 0) {
            ...
            while (extension) {
                len = SearchPathW(
                    NULL, name, extension, MSGSIZE, path_command.value, NULL
                );
                if (len) {
                    result = &path_command;
                    break;
...
read_config_file(wchar_t *config_path) {
    ...
    read = GetPrivateProfileStringW(
        L"commands", NULL, NULL, keynames, MSGSIZE, config_path
    );
    if (read == MSGSIZE - 1) {
    ...
    while (*key) {
        read = GetPrivateProfileStringW(
            L"commands", key, NULL, value, MSGSIZE, config_path
        );
        if (read == MSGSIZE - 1) {
...
static BOOL
parse_shebang(
    wchar_t *shebang_line, int nchars, wchar_t **command, wchar_t **suffix, BOOL *search
) {
    BOOL rc = FALSE;
    SHEBANG *vpp;
    size_t plen;
    ...
    if ((*shebang_line++ == L'#') && (*shebang_line++ == L'!')) {
        ...
        if (*shebang_line) {
            *command = shebang_line;
            ...
            while ((endp > shebang_line) && isspace(*endp)) --endp;
...
validate_version(wchar_t *p) {
    ...
    */
    ...
    result = result &&
             iswdigit(*p); /* Result = False if first string element is not a digit. */
...
maybe_handle_shebang(wchar_t **argv, wchar_t *cmdline) {
    ...
    if (rc == 0) {
        ...
        if ((read >= 4) && (buffer[3] == '\n') && (buffer[2] == '\r')) {
            ip = find_by_magic(
                (((unsigned char)buffer[1]) << 8 | (unsigned char)buffer[0]) & 0xFFFF
            );
            if (ip != NULL) {
            ...
            switch (bom->code_page) {
                case CP_UTF8:
                    nchars = MultiByteToWideChar(
                        bom->code_page, 0, start, header_len, shebang_line, BUFSIZE
                    );
                    break;
                case CP_UTF16BE:
                    if (header_len % 2 != 0) {
                        debug(
                            L"maybe_handle_shebang: UTF-16BE, but an odd number \
of bytes: %d\n",
                            header_len
                        );
                ...
                case CP_UTF16LE:
                    if ((header_len % 2) != 0) {
                        debug(
                            L"UTF-16LE, but an odd number of bytes: %d\n", header_len
                        );
                        /* nchars = 0; Not needed - initialised to 0. */
                    } else {
                ...
                case CP_UTF32BE:
                    if (header_len % 4 != 0) {
                        ...
                        for (i = header_len, j = header_len / 2; i > 0;
                             i -= 4, j -= 2) {
            ...
            if (nchars > 0) {
                ...
                is_virt =
                    parse_shebang(shebang_line, nchars, &command, &suffix, &search);
                if (command != NULL) {
...
get_version_info(wchar_t *version_text, size_t size) {
    ...
    if (!version_high && !version_low)
        ...
        _snwprintf_s(version_text, size, _TRUNCATE, L"%d.%d.%d.%d", maj, min, rel, bld);
    }
}
...
-X-64  : Launch the latest 64bit Python X version",
    ...
    fputws(
        L"\nIf an exact version is not given, using the latest version can be "
        L"overridden by",
        stdout
    );
    ...
    fputws(
        L"\n With -2 or -3 flag a matching PY_PYTHON2 or PY_PYTHON3 Environment "
        L"variable",
        stdout
    );
...
show_python_list(wchar_t **argv) {
     ...
     */
    fwprintf(stderr, L"Installed Pythons found by %s Launcher for Windows", argv[0]);
    if (!_wcsicmp(p, L"-0p") || !_wcsicmp(p, L"--list-paths"))
        fmt = L"\n %-15ls%ls"; /* include path */

    if (num_installed_pythons ==
        0) /* We have somehow got here without searching for pythons */
...
get_process_name(void) {
    ...
    while (!r) {
        ...
        if (len == 0) {
            ...
            return NULL;
        } else if (len == bufferLen && GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
            free(r);
            r = NULL;
            bufferLen *= 2;
        }
...
process(int argc, wchar_t **argv) {
    ...
    hr = SHGetFolderPathW(NULL, CSIDL_LOCAL_APPDATA, NULL, 0, appdata_ini_path);
    if (hr != S_OK) {
    ...
    if (size == 0) {
        ...
        if (version_data) {
            valid = GetFileVersionInfoW(argv0, 0, size, version_data);
            if (!valid)
                debug(L"GetFileVersionInfo failed: %X\n", GetLastError());
            else {
                valid = VerQueryValueW(
                    version_data, L"\\", (LPVOID *)&file_info, &block_size
                );
                if (!valid)
    ...
    if (p == NULL) {
        debug(
            L"GetModuleFileNameW returned value has no backslash: %ls\n",
            launcher_ini_path
        );
...
#elif defined(VENV_REDIRECT)
    {
        ...
        if (!find_home_value(buffer, &start, &len)) {
            error(RC_BAD_VENV_CFG, L"Cannot find home in '%ls'", venv_cfg_path);
        }
        ...
        executable[0] = L'"';
        cch_actual =
            MultiByteToWideChar(CP_UTF8, 0, start, len, &executable[1], cch - 1);
        if (!cch_actual) {
            error(RC_BAD_VENV_CFG, L"Cannot decode home path in '%ls'", venv_cfg_path);
        }
        ...
        if (wcscat_s(&executable[1], cch - 1, PYTHON_EXECUTABLE)) {
            error(
                RC_BAD_VENV_CFG,
                L"Cannot create executable path from '%ls'",
                venv_cfg_path
            );
        }
    ...
    if (argc <= 1) {
        ...
        if (valid) {
            ...
            if (ip == NULL) {
                fwprintf(stdout, L"Python %ls not found!\n", &p[1]);
                valid = show_python_list(argv);
                error(
                    RC_NO_PYTHON,
                    L"Requested Python version (%ls) not \
installed, use -0 for available pythons",
                    &p[1]
                );
            }
