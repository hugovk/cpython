    ...
    for (Py_ssize_t i = 0, l = asdl_seq_LEN(seqs); i < l; i++) {
        ...
        for (Py_ssize_t j = 0, li = asdl_seq_LEN(inner_seq); j < li; j++) {
            asdl_seq_SET_UNTYPED(
                flattened_seq, flattened_seq_idx++, asdl_seq_GET_UNTYPED(inner_seq, j)
            );
        }
    }
...
_PyPegen_join_names_with_dot(Parser *p, expr_ty first_name, expr_ty second_name) {
    ...
    PyObject *uni =
        PyUnicode_FromFormat("%U.%U", first_name->v.Name.id, second_name->v.Name.id);
    if (!uni) {
        return NULL;
    }
...
_PyPegen_alias_for_star(
    ...
    return _PyAST_alias(
        str, NULL, lineno, col_offset, end_lineno, end_col_offset, arena
    );
}
...
_set_tuple_context(Parser *p, expr_ty e, expr_context_ty ctx) {
    return _PyAST_Tuple(
        _set_seq_context(p, e->v.Tuple.elts, ctx), ctx, EXTRA_EXPR(e, e)
    );
}

static expr_ty
_set_list_context(Parser *p, expr_ty e, expr_context_ty ctx) {
    return _PyAST_List(_set_seq_context(p, e->v.List.elts, ctx), ctx, EXTRA_EXPR(e, e));
}

static expr_ty
_set_subscript_context(Parser *p, expr_ty e, expr_context_ty ctx) {
    return _PyAST_Subscript(
        e->v.Subscript.value, e->v.Subscript.slice, ctx, EXTRA_EXPR(e, e)
    );
}

static expr_ty
_set_attribute_context(Parser *p, expr_ty e, expr_context_ty ctx) {
    return _PyAST_Attribute(
        e->v.Attribute.value, e->v.Attribute.attr, ctx, EXTRA_EXPR(e, e)
    );
}

static expr_ty
_set_starred_context(Parser *p, expr_ty e, expr_context_ty ctx) {
    return _PyAST_Starred(
        _PyPegen_set_expr_context(p, e->v.Starred.value, ctx), ctx, EXTRA_EXPR(e, e)
    );
}
...
_PyPegen_slash_with_default(
    Parser *p, asdl_arg_seq *plain_names, asdl_seq *names_with_defaults
) {
...
static int
_make_kwargs(
    Parser *p, StarEtc *star_etc, asdl_arg_seq **kwonlyargs, asdl_expr_seq **kwdefaults
) {
...
/* Constructs an arguments_ty object out of all the parsed constructs in the parameters
 * rule */
arguments_ty
_PyPegen_make_arguments(
    ...
    asdl_arg_seq *posonlyargs;
    if (_make_posonlyargs(p, slash_without_default, slash_with_default, &posonlyargs) ==
        -1) {
        return NULL;
    }
    ...
    asdl_expr_seq *posdefaults;
    if (_make_posdefaults(p, slash_with_default, names_with_default, &posdefaults) ==
        -1) {
        return NULL;
    }
...
_PyPegen_empty_arguments(Parser *p) {
    ...
    return _PyAST_arguments(
        posonlyargs, posargs, NULL, kwonlyargs, kwdefaults, NULL, posdefaults, p->arena
    );
}
...
_PyPegen_function_def_decorators(
    Parser *p, asdl_expr_seq *decorators, stmt_ty function_def
) {
    ...
    if (function_def->kind == AsyncFunctionDef_kind) {
        return _PyAST_AsyncFunctionDef(
            ...
            function_def->v.AsyncFunctionDef.body,
            decorators,
...
_PyPegen_make_module(Parser *p, asdl_stmt_seq *a) {
    ...
    if (num > 0) {
        ...
        for (Py_ssize_t i = 0; i < num; i++) {
            PyObject *tag =
                _PyPegen_new_type_comment(p, p->type_ignore_comments.items[i].comment);
            if (tag == NULL) {
                return NULL;
            }
...
_PyPegen_setup_full_format_spec(
    ...
    if (n == 0 || (n == 1 && asdl_seq_GET(spec, 0)->kind == Constant_kind)) {
        res = _PyAST_JoinedStr(
            spec, lineno, col_offset, end_lineno, end_col_offset, p->arena
        );
    } else {
        res = _PyPegen_concatenate_strings(
            p, spec, lineno, col_offset, end_lineno, end_col_offset, arena
        );
    }
    if (!res) {
        return NULL;
...
void *
_PyPegen_arguments_parsing_error(Parser *p, expr_ty e) {
...
_PyPegen_formatted_value(
    ...
    expr_ty formatted_value = _PyAST_FormattedValue(
        expression,
        conversion_val,
        format ? (expr_ty)format->result : NULL,
        lineno,
        col_offset,
        end_lineno,
        end_col_offset,
    ...
    if (debug) {
        ...
        expr_ty debug_text = _PyAST_Constant(
            debug_metadata,
            NULL,
            lineno,
            col_offset + 1,
        ...
        return _PyAST_JoinedStr(
            values, lineno, col_offset, debug_end_line, debug_end_offset, p->arena
        );
    } else {
...
_PyPegen_concatenate_strings(
    ...
    if (bytes_found) {
        ...
        return _PyAST_Constant(
            res, kind, lineno, col_offset, end_lineno, end_col_offset, p->arena
        );
    }
    ...
    for (i = 0; i < n_flattened_elements; i++) {
        ...
        if (elem->kind == Constant_kind) {
            if (i + 1 < n_flattened_elements &&
                ...
                elem = _PyAST_Constant(
                    concat_str,
                    kind,
                    first_elem->lineno,
                    first_elem->col_offset,
                    last_elem->end_lineno,
                    last_elem->end_col_offset,
                    p->arena
                );
                if (elem == NULL) {
                    return NULL;
    ...
    return _PyAST_JoinedStr(
        values, lineno, col_offset, end_lineno, end_col_offset, p->arena
    );
}
