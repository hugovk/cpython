...
resize_ringbuf(RingBuf *buf, Py_ssize_t capacity) {
    ...
    if (tail_size > 0) {
        memcpy(new_items, buf->items + buf->get_idx, tail_size * sizeof(PyObject *));
    }
    ...
    if (head_size > 0) {
        memcpy(new_items + tail_size, buf->items, head_size * sizeof(PyObject *));
    }
...
/*[clinic input]
module _queue
class _queue.SimpleQueue "simplequeueobject *"
"simplequeue_get_state_by_type(type)->SimpleQueueType" [clinic start generated code]*/
...
static PyObject *
_queue_SimpleQueue_put_impl(
    simplequeueobject *self, PyObject *item, int block, PyObject *timeout
)
/*[clinic end generated code: output=4333136e88f90d8b input=a16dbb33363c0fa8]*/
{
    ...
    if (self->has_threads_waiting) {
        ...
        _PyParkingLot_Unpark(
            &self->has_threads_waiting, (_Py_unpark_fn_t *)maybe_handoff_item, &data
        );
    }
    if (!data.handed_off) {
...
_queue_SimpleQueue_get_impl(
    simplequeueobject *self, PyTypeObject *cls, int block, PyObject *timeout_obj
)
/*[clinic end generated code: output=5c2cca914cd1e55b input=f7836c65e5839c51]*/
{
    ...
    if (block != 0 && !Py_IsNone(timeout_obj)) {
        /* With timeout */
        PyTime_t timeout;
        if (_PyTime_FromSecondsObject(&timeout, timeout_obj, _PyTime_ROUND_CEILING) <
            0) {
            return NULL;
        }
        if (timeout < 0) {
            PyErr_SetString(
                PyExc_ValueError, "'timeout' must be a non-negative number"
            );
            return NULL;
        }
    ...
    for (;;) {
        ...
        PyObject *item = NULL;
        int st = _PyParkingLot_Park(
            &self->has_threads_waiting,
            &waiting,
            sizeof(bool),
            timeout_ns,
            &item,
...
static PyObject *
_queue_SimpleQueue_get_nowait_impl(simplequeueobject *self, PyTypeObject *cls)
...
static PyMethodDef simplequeue_methods[] = {
    _QUEUE_SIMPLEQUEUE_EMPTY_METHODDEF _QUEUE_SIMPLEQUEUE_GET_METHODDEF
        _QUEUE_SIMPLEQUEUE_GET_NOWAIT_METHODDEF _QUEUE_SIMPLEQUEUE_PUT_METHODDEF
            _QUEUE_SIMPLEQUEUE_PUT_NOWAIT_METHODDEF _QUEUE_SIMPLEQUEUE_QSIZE_METHODDEF{
                "__class_getitem__",
                Py_GenericAlias,
                METH_O | METH_CLASS,
...
static struct PyMemberDef simplequeue_members[] = {
    {"__weaklistoffset__",
     Py_T_PYSSIZET,
     offsetof(simplequeueobject, weakreflist),
     Py_READONLY},
    {NULL},
};
...
queuemodule_exec(PyObject *module) {
    ...
    state->SimpleQueueType =
        (PyTypeObject *)PyType_FromModuleAndSpec(module, &simplequeue_spec, NULL);
