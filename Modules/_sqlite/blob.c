...
close_blob(pysqlite_Blob *self) {
    if (self->blob) {
        ...
        Py_BEGIN_ALLOW_THREADS sqlite3_blob_close(blob);
...
check_blob(pysqlite_Blob *self) {
    ...
    if (self->blob == NULL) {
        ...
        PyErr_SetString(state->ProgrammingError, "Cannot operate on a closed blob.");
        return 0;
    }
    return 1;
}
...
read_single(pysqlite_Blob *self, Py_ssize_t offset) {
    ...
    Py_BEGIN_ALLOW_THREADS rc =
        sqlite3_blob_read(self->blob, (void *)&buf, 1, (int)offset);
    Py_END_ALLOW_THREADS
...
read_multiple(pysqlite_Blob *self, Py_ssize_t length, Py_ssize_t offset) {
    ...
    int rc;
    Py_BEGIN_ALLOW_THREADS rc =
        sqlite3_blob_read(self->blob, raw_buffer, (int)length, (int)offset);
    Py_END_ALLOW_THREADS
...
static int
inner_write(pysqlite_Blob *self, const void *buf, Py_ssize_t len, Py_ssize_t offset) {
    ...
    int rc;
    Py_BEGIN_ALLOW_THREADS rc =
        sqlite3_blob_write(self->blob, buf, (int)len, (int)offset);
    Py_END_ALLOW_THREADS
...
static PyObject *
blob_exit_impl(pysqlite_Blob *self, PyObject *type, PyObject *val, PyObject *tb)
...
ass_subscript_index(pysqlite_Blob *self, PyObject *item, PyObject *value) {
    if (value == NULL) {
        PyErr_SetString(PyExc_TypeError, "Blob doesn't support item deletion");
        return -1;
    }
...
ass_subscript_slice(pysqlite_Blob *self, PyObject *item, PyObject *value) {
    if (value == NULL) {
        PyErr_SetString(PyExc_TypeError, "Blob doesn't support slice deletion");
        return -1;
    }
    ...
    int rc = -1;
    if (vbuf.len != len) {
        PyErr_SetString(PyExc_IndexError, "Blob slice assignment is wrong size");
    } else if (step == 1) {
...
static PyMethodDef blob_methods[] = {
    BLOB_CLOSE_METHODDEF BLOB_ENTER_METHODDEF BLOB_EXIT_METHODDEF BLOB_READ_METHODDEF
        BLOB_SEEK_METHODDEF BLOB_TELL_METHODDEF BLOB_WRITE_METHODDEF{NULL, NULL}
};

static struct PyMemberDef blob_members[] = {
    {"__weaklistoffset__",
     Py_T_PYSSIZET,
     offsetof(pysqlite_Blob, in_weakreflist),
     Py_READONLY},
    {NULL},
};
