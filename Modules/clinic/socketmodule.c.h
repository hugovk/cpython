...
#define _SOCKET_SOCKET_CLOSE_METHODDEF  \
    {"close",                           \
     (PyCFunction)_socket_socket_close, \
     METH_NOARGS,                       \
     _socket_socket_close__doc__},
...
#undef KWTUPLE
    ...
    fastargs = _PyArg_UnpackKeywords(
        _PyTuple_CAST(args)->ob_item, nargs, kwargs, NULL, &_parser, 0, 4, 0, argsbuf
    );
    if (!fastargs) {
        goto exit;
    }
...
skip_optional_pos:
    return_value =
        sock_initobj_impl((PySocketSockObject *)self, family, type, proto, fdobj);
...
PyDoc_STRVAR(
    ...
    "--\n"
    "\n"
    "Convert an IP address in string format (123.45.67.89) to the 32-bit packed binary "
    "format used in low-level network functions."
);

#define _SOCKET_SOCKET_INET_ATON_METHODDEF  \
    {"inet_aton",                           \
     (PyCFunction)_socket_socket_inet_aton, \
     METH_O,                                \
     _socket_socket_inet_aton__doc__},
...
#define _SOCKET_SOCKET_INET_NTOA_METHODDEF  \
    {"inet_ntoa",                           \
     (PyCFunction)_socket_socket_inet_ntoa, \
     METH_O,                                \
     _socket_socket_inet_ntoa__doc__},
...
#define _SOCKET_SOCKET_IF_NAMETOINDEX_METHODDEF  \
    {"if_nametoindex",                           \
     (PyCFunction)_socket_socket_if_nametoindex, \
     METH_O,                                     \
     _socket_socket_if_nametoindex__doc__},
