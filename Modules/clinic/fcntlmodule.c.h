...
#define FCNTL_FCNTL_METHODDEF                  \
    {"fcntl",                                  \
     (PyCFunction)(void (*)(void))fcntl_fcntl, \
     METH_FASTCALL,                            \
     fcntl_fcntl__doc__},
...
fcntl_fcntl(PyObject *module, PyObject *const *args, Py_ssize_t nargs) {
    ...
    if (nargs < 2) {
        PyErr_Format(
            PyExc_TypeError, "fcntl expected at least 2 arguments, got %zd", nargs
        );
        goto exit;
    }
    if (nargs > 3) {
        PyErr_Format(
            PyExc_TypeError, "fcntl expected at most 3 arguments, got %zd", nargs
        );
        goto exit;
    }
...
#define FCNTL_IOCTL_METHODDEF                  \
    {"ioctl",                                  \
     (PyCFunction)(void (*)(void))fcntl_ioctl, \
     METH_FASTCALL,                            \
     fcntl_ioctl__doc__},
...
fcntl_ioctl(PyObject *module, PyObject *const *args, Py_ssize_t nargs) {
    ...
    if (nargs < 2) {
        PyErr_Format(
            PyExc_TypeError, "ioctl expected at least 2 arguments, got %zd", nargs
        );
        goto exit;
    }
    if (nargs > 4) {
        PyErr_Format(
            PyExc_TypeError, "ioctl expected at most 4 arguments, got %zd", nargs
        );
        goto exit;
    }
    fd = PyObject_AsFileDescriptor(args[0]);
    ...
    if (!PyLong_Check(args[1])) {
        PyErr_Format(
            PyExc_TypeError, "ioctl() argument 2 must be int, not %T", args[1]
        );
        goto exit;
    }
...
#define FCNTL_FLOCK_METHODDEF                  \
    {"flock",                                  \
     (PyCFunction)(void (*)(void))fcntl_flock, \
     METH_FASTCALL,                            \
     fcntl_flock__doc__},
...
#define FCNTL_LOCKF_METHODDEF                  \
    {"lockf",                                  \
     (PyCFunction)(void (*)(void))fcntl_lockf, \
     METH_FASTCALL,                            \
     fcntl_lockf__doc__},
...
fcntl_lockf(PyObject *module, PyObject *const *args, Py_ssize_t nargs) {
    ...
    if (nargs < 2) {
        PyErr_Format(
            PyExc_TypeError, "lockf expected at least 2 arguments, got %zd", nargs
        );
        goto exit;
    }
    if (nargs > 5) {
        PyErr_Format(
            PyExc_TypeError, "lockf expected at most 5 arguments, got %zd", nargs
        );
        goto exit;
    }
    fd = PyObject_AsFileDescriptor(args[0]);
