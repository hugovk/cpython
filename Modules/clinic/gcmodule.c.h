...
gc_set_threshold(PyObject *module, PyObject *args) {
    ...
    switch (PyTuple_GET_SIZE(args)) {
        ...
        case 3:
            if (!PyArg_ParseTuple(
                    args, "iii:set_threshold", &threshold0, &threshold1, &threshold2
                )) {
                goto exit;
            }
            group_right_1 = 1;
        ...
        default:
            PyErr_SetString(
                PyExc_TypeError, "gc.set_threshold requires 1 to 3 arguments"
            );
            goto exit;
    }
    return_value = gc_set_threshold_impl(
        module, threshold0, group_right_1, threshold1, group_right_2, threshold2
    );
...
#define GC_GET_REFERRERS_METHODDEF        \
    {"get_referrers",                     \
     _PyCFunction_CAST(gc_get_referrers), \
     METH_FASTCALL,                       \
     gc_get_referrers__doc__},
...
#define GC_GET_REFERENTS_METHODDEF        \
    {"get_referents",                     \
     _PyCFunction_CAST(gc_get_referents), \
     METH_FASTCALL,                       \
     gc_get_referents__doc__},
...
#define GC_GET_OBJECTS_METHODDEF        \
    {"get_objects",                     \
     _PyCFunction_CAST(gc_get_objects), \
     METH_FASTCALL | METH_KEYWORDS,     \
     gc_get_objects__doc__},
