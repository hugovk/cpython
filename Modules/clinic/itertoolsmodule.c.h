...
itertools__grouper(PyTypeObject *type, PyObject *args, PyObject *kwargs) {
    ...
    if (!PyObject_TypeCheck(
            PyTuple_GET_ITEM(args, 0), clinic_state_by_cls()->groupby_type
        )) {
        _PyArg_BadArgument(
            "_grouper",
            "argument 1",
            (clinic_state_by_cls()->groupby_type)->tp_name,
...
itertools_teedataobject(PyTypeObject *type, PyObject *args, PyObject *kwargs) {
    ...
    if (!PyList_Check(PyTuple_GET_ITEM(args, 1))) {
        _PyArg_BadArgument(
            "teedataobject", "argument 2", "list", PyTuple_GET_ITEM(args, 1)
        );
        goto exit;
    }
...
PyDoc_STRVAR(
    ...
    "--\n"
    "\n"
    "Return successive entries from an iterable as long as the predicate evaluates to "
    "true for each entry."
);
...
PyDoc_STRVAR(
    ...
    "argument tuple taken from the given sequence."
);
...
PyDoc_STRVAR(
    ...
    "--\n"
    "\n"
    "Alternative chain() constructor taking a single iterable argument that evaluates "
    "lazily."
);

#define ITERTOOLS_CHAIN_FROM_ITERABLE_METHODDEF  \
    {"from_iterable",                            \
     (PyCFunction)itertools_chain_from_iterable, \
     METH_O | METH_CLASS,                        \
     itertools_chain_from_iterable__doc__},
