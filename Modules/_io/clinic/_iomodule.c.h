...
#undef KWTUPLE
    ...
    args =
        _PyArg_UnpackKeywords(args, nargs, NULL, kwnames, &_parser, 1, 8, 0, argsbuf);
    if (!args) {
        goto exit;
    }
    file = args[0];
    ...
    opener = args[7];
skip_optional_pos:
    return_value = _io_open_impl(
        module, file, mode, buffering, encoding, errors, newline, closefd, opener
    );
...
#define _IO_TEXT_ENCODING_METHODDEF        \
    {"text_encoding",                      \
     _PyCFunction_CAST(_io_text_encoding), \
     METH_FASTCALL,                        \
     _io_text_encoding__doc__},
...
#define _IO_OPEN_CODE_METHODDEF        \
    {"open_code",                      \
     _PyCFunction_CAST(_io_open_code), \
     METH_FASTCALL | METH_KEYWORDS,    \
     _io_open_code__doc__},
