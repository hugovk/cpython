...
/*[clinic end generated code: output=59eb0d5fc7285ac4 input=9dd8a50c0c36a4d7]*/
{
    ...
    const char *str =
        _Py_SourceAsString(source, "symtable", "string or bytes", &cf, &source_copy);
    if (str == NULL) {
        return NULL;
    }
    ...
    if (strcmp(startstr, "exec") == 0)
        ...
        PyErr_SetString(
            PyExc_ValueError, "symtable() arg 3 must be 'exec' or 'eval' or 'single'"
        );
        Py_DECREF(filename);
...
symtable_init_constants(PyObject *m) {
    if (PyModule_AddIntMacro(m, USE) < 0)
        return -1;
    if (PyModule_AddIntMacro(m, DEF_GLOBAL) < 0)
        return -1;
    ...
    if (PyModule_AddIntMacro(m, DEF_PARAM) < 0)
        return -1;
    if (PyModule_AddIntMacro(m, DEF_TYPE_PARAM) < 0)
        return -1;
    ...
    if (PyModule_AddIntMacro(m, DEF_ANNOT) < 0)
        return -1;
    if (PyModule_AddIntMacro(m, DEF_COMP_ITER) < 0)
        return -1;
    if (PyModule_AddIntMacro(m, DEF_COMP_CELL) < 0)
        return -1;
