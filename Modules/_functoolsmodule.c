...
static PyMethodDef placeholder_methods[] = {
    {"__reduce__", placeholder_reduce, METH_NOARGS, NULL}, {NULL, NULL}
};
...
partial_new(PyTypeObject *type, PyObject *args, PyObject *kw) {
    ...
    if (new_nargs < 0) {
        PyErr_SetString(PyExc_TypeError, "type 'partial' takes at least one argument");
        return NULL;
    }
    ...
    if (new_nargs && PyTuple_GET_ITEM(args, new_nargs) == phold) {
        PyErr_SetString(PyExc_TypeError, "trailing Placeholders are not allowed");
        return NULL;
    }
...
partial_vectorcall(
    ...
    if (nargskw == 0) {
        return _PyObject_VectorcallTstate(tstate, pto->fn, pto_args, pto_nargs, NULL);
    }
    ...
    if (pto_nargs == 1 && (nargsf & PY_VECTORCALL_ARGUMENTS_OFFSET)) {
        ...
        PyObject *ret =
            _PyObject_VectorcallTstate(tstate, pto->fn, newargs, nargs + 1, kwnames);
        newargs[0] = tmp;
        return ret;
    }
    ...
    PyObject *ret =
        _PyObject_VectorcallTstate(tstate, pto->fn, stack, tot_nargs, kwnames);
...
static PyMemberDef partial_memberlist[] = {
    ...
    {"__weaklistoffset__",
     ...
     Py_READONLY},
    {"__dictoffset__", Py_T_PYSSIZET, offsetof(partialobject, dict), Py_READONLY},
    {"__vectorcalloffset__",
     Py_T_PYSSIZET,
     offsetof(partialobject, vectorcall),
     Py_READONLY},
    {NULL} /* Sentinel */
};
...
partial_repr(PyObject *self) {
    ...
    for (i = 0; i < n; i++) {
        Py_SETREF(
            arglist,
            PyUnicode_FromFormat("%U, %R", arglist, PyTuple_GET_ITEM(pto->args, i))
        );
        if (arglist == NULL)
            goto done;
    }
    ...
    for (i = 0; PyDict_Next(pto->kw, &i, &key, &value);) {
        ...
        Py_INCREF(value);
        Py_SETREF(arglist, PyUnicode_FromFormat("%U, %S=%R", arglist, key, value));
        Py_DECREF(value);
        if (arglist == NULL)
            goto done;
    }
...
partial_setstate(PyObject *self, PyObject *state) {
    ...
    if (!PyArg_ParseTuple(state, "OOOO", &fn, &fnargs, &kw, &dict) ||
        !PyCallable_Check(fn) || !PyTuple_Check(fnargs) ||
...
static PyGetSetDef keyobject_getset[] = {
    {"__text_signature__", keyobject_text_signature, (setter)NULL}, {NULL}
};
...
functools_reduce(PyObject *self, PyObject *args) {
    ...
    if (it == NULL) {
        if (PyErr_ExceptionMatches(PyExc_TypeError))
            PyErr_SetString(PyExc_TypeError, "reduce() arg 2 must support iteration");
        Py_XDECREF(result);
        return NULL;
    }
    ...
    if (result == NULL)
        PyErr_SetString(
            PyExc_TypeError, "reduce() of empty iterable with no initial value"
        );
...
typedef struct lru_list_elem {
    PyObject_HEAD struct lru_list_elem *prev, *next; /* borrowed links */
    Py_hash_t hash;
    PyObject *key, *result;
} lru_list_elem;
...
static PyType_Slot lru_list_elem_type_slots[] = {
    {Py_tp_dealloc, lru_list_elem_dealloc}, {0, 0}
};
...
typedef PyObject
    *(*lru_cache_ternaryfunc)(struct lru_cache_object *, PyObject *, PyObject *);
...
static PyObject *
lru_cache_make_key(PyObject *kwd_mark, PyObject *args, PyObject *kwds, int typed) {
...
bounded_lru_cache_wrapper(lru_cache_object *self, PyObject *args, PyObject *kwds) {
    ...
    if (PyDict_GET_SIZE(self->cache) < self->maxsize ||
        ...
        if (_PyDict_SetItem_KnownHash(self->cache, key, (PyObject *)link, hash) < 0) {
            Py_DECREF(link);
            return NULL;
        }
        lru_cache_append_link(self, link);
    ...
    */
    ...
    int res = _PyDict_Pop_KnownHash(
        (PyDictObject *)self->cache, link->key, link->hash, &popresult
    );
    if (res < 0) {
...
static PyMemberDef lru_cache_memberlist[] = {
    {"__dictoffset__", Py_T_PYSSIZET, offsetof(lru_cache_object, dict), Py_READONLY},
    {"__weaklistoffset__",
     Py_T_PYSSIZET,
...
static PyType_Spec lru_cache_type_spec = {
    ...
    .flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC | Py_TPFLAGS_METHOD_DESCRIPTOR |
             Py_TPFLAGS_IMMUTABLETYPE,
...
PyDoc_STRVAR(_functools_doc, "Tools that operate on functions.");

static PyMethodDef _functools_methods[] = {
    {"reduce", functools_reduce, METH_VARARGS, functools_reduce_doc},
    _FUNCTOOLS_CMP_TO_KEY_METHODDEF{NULL, NULL} /* sentinel */
};
...
_functools_exec(PyObject *module) {
    ...
    state->placeholder_type =
        (PyTypeObject *)PyType_FromModuleAndSpec(module, &placeholder_type_spec, NULL);
    ...
    PyObject *lru_cache_type =
        PyType_FromModuleAndSpec(module, &lru_cache_type_spec, NULL);
