...
ascii_buffer_converter(PyObject *arg, Py_buffer *buf) {
    ...
    if (PyUnicode_Check(arg)) {
        if (!PyUnicode_IS_ASCII(arg)) {
            PyErr_SetString(
                PyExc_ValueError, "string argument should contain only ASCII characters"
            );
            return 0;
        }
        ...
        */
        if (this_ch == BASE64_PAD) {
            ...
            if (quad_pos >= 2 && quad_pos + ++pads >= 4) {
                ...
                ** in strict mode, an error should raise if there's excess data after
                *the padding.
                */
...
static unsigned int
internal_crc32(
    const unsigned char *bin_data, Py_ssize_t len, unsigned int crc
...
#else  /* USE_ZLIB_CRC32 */
{
    ...
    if (len > 1024 * 5) {
        unsigned int result;
        Py_BEGIN_ALLOW_THREADS result = internal_crc32(bin_data, len, crc);
...
binascii_b2a_hex_impl(
    PyObject *module, Py_buffer *data, PyObject *sep, int bytes_per_sep
)
/*[clinic end generated code: output=a26937946a81d2c7 input=ec0ade6ba2e43543]*/
{
    return _Py_strhex_bytes_with_sep(
        (const char *)data->buf, data->len, sep, bytes_per_sep
    );
}
...
binascii_b2a_qp_impl(
    PyObject *module, Py_buffer *data, int quotetabs, int istext, int header
)
/*[clinic end generated code: output=e9884472ebb1a94c input=e9102879afb0defd]*/
{
    ...
    while (in < datalen) {
        ...
        if ((databuf[in] > 126) || (databuf[in] == '=') ||
            ...
            ((databuf[in] < 33) && (databuf[in] != '\r') && (databuf[in] != '\n') &&
...
static struct PyMethodDef binascii_module_methods[] = {
    BINASCII_A2B_UU_METHODDEF BINASCII_B2A_UU_METHODDEF BINASCII_A2B_BASE64_METHODDEF
        BINASCII_B2A_BASE64_METHODDEF BINASCII_A2B_HEX_METHODDEF
            BINASCII_B2A_HEX_METHODDEF BINASCII_HEXLIFY_METHODDEF
                BINASCII_UNHEXLIFY_METHODDEF BINASCII_CRC_HQX_METHODDEF
                    BINASCII_CRC32_METHODDEF BINASCII_A2B_QP_METHODDEF
