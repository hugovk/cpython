...
typedef Py_UCS4 (*iso2022_decode_func)(
    const MultibyteCodec *codec, const unsigned char *data
);
typedef DBCHAR (*iso2022_encode_func)(
    const MultibyteCodec *codec, const Py_UCS4 *data, Py_ssize_t *length
);
...
iso2022processesc(
    ...
    switch (esclen) {
        ...
        case 6: /* designation with prefix */
            if (CONFIG_ISSET(USE_JISX0208_EXT) && (*inbuf)[3] == ESC &&
                (*inbuf)[4] == '$' && (*inbuf)[5] == 'B') {
...
#define ISO8859_7_DECODE(c, writer)                                     \
    if ((c) < 0xa0) {                                                   \
        ...
        OUTCHAR(c);                                                     \
    } else if ((c) >= 0xb4 && (c) <= 0xfe &&                            \
               ((c) >= 0xd4 || (0xbffffd77L & (1L << ((c) - 0xb4))))) { \
        OUTCHAR(0x02d0 + (c));                                          \
    } else if ((c) == 0xa1) {                                           \
        OUTCHAR(0x2018);                                                \
    } else if ((c) == 0xa2) {                                           \
        ...
        switch (c) {
            case ESC:
                ...
                if (IS_ISO2022ESC(INBYTE2)) {
                    err = iso2022processesc(codec, state, inbuf, &inleft);
                    if (err != 0)
                        return err;
                } else if (CONFIG_ISSET(USE_G2) && INBYTE2 == 'N') { /* SS2 */
                    REQUIRE_INBUF(3);
                    err = iso2022processg2(codec, state, inbuf, &inleft, writer);
                    if (err != 0)
                        return err;
                } else {
                    OUTCHAR(ESC);
...
bypass:
                    ...
                    if (dsgcache != NULL && dsgcache->mark == charset)
                        dsg = dsgcache;
                    else {
                        for (dsg = CONFIG_DESIGNATIONS; dsg->mark != charset
#ifdef Py_DEBUG
                                                        && dsg->mark != '\0'
#endif
                             ;
...
static DBCHAR
ksx1001_encoder(const MultibyteCodec *codec, const Py_UCS4 *data, Py_ssize_t *length) {
    DBCHAR coded;
    assert(*length == 1);
    if (*data < 0x10000) {
...
static DBCHAR
jisx0208_encoder(const MultibyteCodec *codec, const Py_UCS4 *data, Py_ssize_t *length) {
    DBCHAR coded;
    assert(*length == 1);
    if (*data < 0x10000) {
...
static DBCHAR
jisx0212_encoder(const MultibyteCodec *codec, const Py_UCS4 *data, Py_ssize_t *length) {
    DBCHAR coded;
    assert(*length == 1);
    if (*data < 0x10000) {
...
jisx0213_2000_1_decoder(const MultibyteCodec *codec, const unsigned char *data) {
    ...
    else if (TRYMAP_DEC_ST(jisx0213_1_emp, u, data[0], data[1])) u |= 0x20000;
...
jisx0213_encoder(
    ...
    switch (*length) {
        case 1: /* first character */
            if (*data >= 0x10000) {
                if ((*data) >> 16 == 0x20000 >> 16) {
                    ...
                    else if (TRYMAP_ENC_ST(
                                 jisx0213_emp, coded, (*data) & 0xffff
                             )) return coded;
                }
                return MAP_UNMAPPABLE;
            }
        ...
        case -1: /* flush unterminated */
            *length = 1;
            coded = find_pairencmap(
                (ucs2_t)data[0], 0, jisx0213_pair_encmap, JISX0213_ENCPAIRS
            );
            if (coded == DBCINV)
                return MAP_UNMAPPABLE;
            else
...
static DBCHAR
gb2312_encoder(const MultibyteCodec *codec, const Py_UCS4 *data, Py_ssize_t *length) {
    DBCHAR coded;
    assert(*length == 1);
    if (*data < 0x10000) {
...
static DBCHAR
dummy_encoder(const MultibyteCodec *codec, const Py_UCS4 *data, Py_ssize_t *length) {
    return MAP_UNMAPPABLE;
}
...
#define REGISTRY_KSX1001_G0 \
    {CHARSET_KSX1001, 0, 2, ksx1001_init, ksx1001_decoder, ksx1001_encoder}
#define REGISTRY_KSX1001_G1 \
    {CHARSET_KSX1001, 1, 2, ksx1001_init, ksx1001_decoder, ksx1001_encoder}
#define REGISTRY_JISX0201_R \
    {CHARSET_JISX0201_R, 0, 1, NULL, jisx0201_r_decoder, jisx0201_r_encoder}
#define REGISTRY_JISX0201_K \
    {CHARSET_JISX0201_K, 0, 1, NULL, jisx0201_k_decoder, jisx0201_k_encoder}
#define REGISTRY_JISX0208 \
    {CHARSET_JISX0208, 0, 2, jisx0208_init, jisx0208_decoder, jisx0208_encoder}
#define REGISTRY_JISX0208_O \
    {CHARSET_JISX0208_O, 0, 2, jisx0208_init, jisx0208_decoder, jisx0208_encoder}
#define REGISTRY_JISX0212 \
    {CHARSET_JISX0212, 0, 2, jisx0212_init, jisx0212_decoder, jisx0212_encoder}
...
#define REGISTRY_GB2312 \
    {CHARSET_GB2312, 0, 2, gb2312_init, gb2312_decoder, gb2312_encoder}
#define REGISTRY_CNS11643_1 \
    {CHARSET_CNS11643_1, 1, 2, cns11643_init, cns11643_1_decoder, cns11643_1_encoder}
#define REGISTRY_CNS11643_2 \
    {CHARSET_CNS11643_2, 2, 2, cns11643_init, cns11643_2_decoder, cns11643_2_encoder}
#define REGISTRY_ISO8859_1 {CHARSET_ISO8859_1, 2, 1, NULL, dummy_decoder, dummy_encoder}
#define REGISTRY_ISO8859_7 {CHARSET_ISO8859_7, 2, 1, NULL, dummy_decoder, dummy_encoder}
...
#define ISO2022_CODEC(variation)                                 \
    NEXT_CODEC = (MultibyteCodec){"iso2022_" #variation,         \
