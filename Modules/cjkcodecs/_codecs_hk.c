        ...
        if (c < 0x10000) {
            if (TRYMAP_ENC(big5hkscs_bmp, code, c)) {
                if (code == MULTIC) {
                    ...
                    if (inlen - *inpos >= 2 && ((c & 0xffdf) == 0x00ca) &&
                        ((c2 & 0xfff7) == 0x0304)) {
                        code = big5hkscs_pairenc_table[((c >> 4) | (c2 >> 3)) & 3];
                        insize = 2;
                    } else if (inlen - *inpos < 2 && !(flags & MBENC_FLUSH))
                        return MBERR_TOOFEW;
                    else {
                        if (c == 0xca)
                            code = 0x8866;
...
DECODER(big5hkscs) {
    while (inleft > 0) {
        ...
        switch ((c << 8) | INBYTE2) {
            case 0x8862:
                OUTCHAR2(0x00ca, 0x0304);
                break;
            case 0x8864:
                OUTCHAR2(0x00ca, 0x030c);
                break;
            case 0x88a3:
                OUTCHAR2(0x00ea, 0x0304);
                break;
            case 0x88a5:
                OUTCHAR2(0x00ea, 0x030c);
                break;
            default:
                return 1;
        }
