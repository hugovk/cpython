...
static const size_t faulthandler_nsignals = Py_ARRAY_LENGTH(faulthandler_handlers);
...
faulthandler_get_fileno(PyObject **file_ptr) {
    ...
    if (file == NULL || file == Py_None) {
        ...
        if (fd == -1 && PyErr_Occurred())
            return -1;
        if (fd < 0) {
            PyErr_SetString(PyExc_ValueError, "file is not a valid file descriptor");
            return -1;
        }
        *file_ptr = NULL;
        return fd;
    ...
    if (fd == -1) {
        PyErr_SetString(
            PyExc_RuntimeError, "file.fileno() is not a valid file descriptor"
        );
        return -1;
    }
...
get_thread_state(void) {
    ...
    if (tstate == NULL) {
        ...
        PyErr_SetString(PyExc_RuntimeError, "unable to get the current thread state");
        return NULL;
    }
    return tstate;
}
...
static void
faulthandler_dump_traceback(int fd, int all_threads, PyInterpreterState *interp) {
...
static PyObject *
faulthandler_dump_traceback_py(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "|Op:dump_traceback", kwlist, &file, &all_threads
        ))
        return NULL;
...
faulthandler_exc_handler(struct _EXCEPTION_POINTERS *exc_info) {
    ...
    switch (code) {
        ...
        case EXCEPTION_ACCESS_VIOLATION:
            PUTS(fd, "access violation");
            break;
        case EXCEPTION_FLT_DIVIDE_BY_ZERO:
            PUTS(fd, "float divide by zero");
            break;
        case EXCEPTION_FLT_OVERFLOW:
            PUTS(fd, "float overflow");
            break;
        case EXCEPTION_INT_DIVIDE_BY_ZERO:
            PUTS(fd, "int divide by zero");
            break;
        case EXCEPTION_INT_OVERFLOW:
            PUTS(fd, "integer overflow");
            break;
...
faulthandler_py_enable(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "|Op:enable", kwlist, &file, &all_threads
        ))
        return NULL;
...
faulthandler_thread(void *unused) {
    ...
    do {
        st = PyThread_acquire_lock_timed(thread.cancel_event, thread.timeout_us, 0);
...
format_timeout(PyTime_t us) {
    ...
    if (us != 0) {
        ...
        PyOS_snprintf(
            buffer, sizeof(buffer), "Timeout (%lu:%02lu:%02lu)!\n", hour, min, sec
        );
    }
...
faulthandler_dump_traceback_later(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if (PyThread_start_new_thread(faulthandler_thread, NULL) ==
        PYTHREAD_INVALID_THREAD_ID) {
        PyThread_release_lock(thread.running);
        Py_CLEAR(thread.file);
        PyMem_Free(header);
        thread.header = NULL;
        PyErr_SetString(PyExc_RuntimeError, "unable to start watchdog thread");
        return NULL;
    }
...
faulthandler_register_py(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "i|Opp:register", kwlist, &signum, &file, &all_threads, &chain
        ))
        return NULL;
...
faulthandler_sigsegv(PyObject *self, PyObject *args) {
    ...
    if (release_gil) {
        Py_BEGIN_ALLOW_THREADS faulthandler_raise_sigsegv();
...
static PyMethodDef module_methods[] = {
    {"enable",
     _PyCFunction_CAST(faulthandler_py_enable),
    ...
    {"dump_traceback",
     _PyCFunction_CAST(faulthandler_dump_traceback_py),
     METH_VARARGS | METH_KEYWORDS,
...
#ifdef FAULTHANDLER_USER
    {"register",
     _PyCFunction_CAST(faulthandler_register_py),
     METH_VARARGS | METH_KEYWORDS,
     PyDoc_STR("register($module, /, signum, file=sys.stderr, all_threads=True, "
               "chain=False)\n--\n\n"
               "Register a handler for the signal 'signum': dump the "
               "traceback of the current thread, or of all threads if "
               "all_threads is True, into file.")},
    {"unregister",
     _PyCFunction_CAST(faulthandler_unregister_py),
     METH_VARARGS,
    ...
    {"_fatal_error_c_thread",
     faulthandler_fatal_error_c_thread,
     METH_NOARGS,
    ...
    {"_sigfpe",
     (PyCFunction)faulthandler_sigfpe,
     METH_NOARGS,
...
PyExec_faulthandler(PyObject *module) {
    ...
    if (PyModule_AddIntConstant(
            module, "_EXCEPTION_ACCESS_VIOLATION", EXCEPTION_ACCESS_VIOLATION
        )) {
        return -1;
    }
    if (PyModule_AddIntConstant(
            module, "_EXCEPTION_INT_DIVIDE_BY_ZERO", EXCEPTION_INT_DIVIDE_BY_ZERO
        )) {
        return -1;
    }
    if (PyModule_AddIntConstant(
            module, "_EXCEPTION_STACK_OVERFLOW", EXCEPTION_STACK_OVERFLOW
        )) {
        return -1;
    }
    ...
    if (PyModule_AddIntConstant(
            module, "_EXCEPTION_NONCONTINUABLE", EXCEPTION_NONCONTINUABLE
        )) {
        return -1;
    }
    if (PyModule_AddIntConstant(
