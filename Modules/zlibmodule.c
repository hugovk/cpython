...
OutputBuffer_InitAndGrow(
    ...
    allocated = _BlocksOutputBuffer_InitAndGrow(buffer, max_length, (void **)next_out);
...
/* On success, return value >= 0
   On failure, return -1 */
static inline Py_ssize_t
OutputBuffer_Grow(_BlocksOutputBuffer *buffer, Bytef **next_out, uint32_t *avail_out) {
    Py_ssize_t allocated;

    allocated =
        _BlocksOutputBuffer_Grow(buffer, (void **)next_out, (Py_ssize_t)*avail_out);
    *avail_out = (uint32_t)allocated;
    return allocated;
}
...
static inline Py_ssize_t
OutputBuffer_WindowInitWithSize(
    _BlocksOutputBuffer *buffer,
    ...
    Py_ssize_t allocated =
        _BlocksOutputBuffer_InitWithSize(buffer, init_size, (void **)next_out);
...
OutputBuffer_WindowGrow(
    ...
    if (*avail_out != 0) {
        PyErr_SetString(
            PyExc_SystemError, "*avail_out != 0 in OutputBuffer_WindowGrow()."
        );
        return -1;
    }
...
static inline PyObject *
OutputBuffer_WindowFinish(
    _BlocksOutputBuffer *buffer, _Uint32Window *window, uint32_t avail_out
) {
...
#define ENTER_ZLIB(obj)                                                   \
    do {                                                                  \
        if (!PyThread_acquire_lock((obj)->lock, 0)) {                     \
            Py_BEGIN_ALLOW_THREADS PyThread_acquire_lock((obj)->lock, 1); \
...
/*[clinic end generated code: output=46bd152fadd66df2 input=c4d06ee5782a7e3f]*/
{
    ...
    zst.next_in = ibuf;
    int err =
        deflateInit2(&zst, level, DEFLATED, wbits, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY);
    ...
    do {
        ...
        do {
            ...
            Py_BEGIN_ALLOW_THREADS err = deflate(&zst, flush);
            Py_END_ALLOW_THREADS
...
static PyObject *
zlib_decompress_impl(PyObject *module, Py_buffer *data, int wbits, Py_ssize_t bufsize)
/*[clinic end generated code: output=77c7e35111dc8c42 input=a9ac17beff1f893f]*/
{
    ...
    if (OutputBuffer_WindowInitWithSize(
            &buffer, &window, bufsize, &zst.next_out, &zst.avail_out
        ) < 0) {
        goto error;
    }
    ...
    switch (err) {
        ...
        case Z_MEM_ERROR:
            PyErr_SetString(
                PyExc_MemoryError, "Out of memory while decompressing data"
            );
            goto error;
        default:
            inflateEnd(&zst);
    ...
    do {
        ...
        do {
            if (zst.avail_out == 0) {
                if (OutputBuffer_WindowGrow(
                        &buffer, &window, &zst.next_out, &zst.avail_out
                    ) < 0) {
                    inflateEnd(&zst);
                    goto error;
                }
            ...
            Py_BEGIN_ALLOW_THREADS err = inflate(&zst, flush);
            Py_END_ALLOW_THREADS
...
/*[clinic end generated code: output=8b5bed9c8fc3814d input=2fa3d026f90ab8d5]*/
{
    ...
    if (zdict->buf != NULL && (size_t)zdict->len > UINT_MAX) {
        PyErr_SetString(
            PyExc_OverflowError, "zdict length does not fit in an unsigned int"
        );
        return NULL;
    }
    ...
    switch (err) {
        case Z_OK:
            ...
            if (zdict->buf == NULL) {
                ...
                err = deflateSetDictionary(
                    &self->zst, zdict->buf, (unsigned int)zdict->len
                );
                switch (err) {
                    case Z_OK:
                        goto success;
        ...
        case Z_MEM_ERROR:
            PyErr_SetString(
                PyExc_MemoryError, "Can't allocate memory for compression object"
            );
            goto error;
        case Z_STREAM_ERROR:
...
set_inflate_zdict(zlibstate *state, compobject *self) {
    ...
    int err;
    err = inflateSetDictionary(&self->zst, zdict_buf.buf, (unsigned int)zdict_buf.len);
    PyBuffer_Release(&zdict_buf);
    if (err != Z_OK) {
...
/*[clinic end generated code: output=3069b99994f36906 input=d3832b8511fc977b]*/
{
    ...
    if (zdict != NULL && !PyObject_CheckBuffer(zdict)) {
        PyErr_SetString(
            PyExc_TypeError, "zdict argument must support the buffer protocol"
        );
        return NULL;
    }
    ...
    switch (err) {
        ...
        case Z_MEM_ERROR:
            Py_DECREF(self);
            PyErr_SetString(
                PyExc_MemoryError, "Can't allocate memory for decompression object"
            );
            return NULL;
        default:
...
static PyObject *
zlib_Compress_compress_impl(compobject *self, PyTypeObject *cls, Py_buffer *data)
/*[clinic end generated code: output=6731b3f0ff357ca6 input=04d00f65ab01d260]*/
{
    ...
    if (OutputBuffer_InitAndGrow(
            &buffer, -1, &self->zst.next_out, &self->zst.avail_out
        ) < 0) {
        goto error;
    }
    ...
    do {
        ...
        do {
            if (self->zst.avail_out == 0) {
                if (OutputBuffer_Grow(
                        &buffer, &self->zst.next_out, &self->zst.avail_out
                    ) < 0) {
                    goto error;
                }
            }

            Py_BEGIN_ALLOW_THREADS err = deflate(&self->zst, Z_NO_FLUSH);
            Py_END_ALLOW_THREADS
...
save_unconsumed_input(compobject *self, Py_buffer *data, int err) {
    if (err == Z_STREAM_END) {
        ...
        if (self->zst.avail_in > 0) {
            ...
            if (new_data == NULL)
                return -1;
            memcpy(
                PyBytes_AS_STRING(new_data),
                PyBytes_AS_STRING(self->unused_data),
                old_size
            );
            memcpy(
                PyBytes_AS_STRING(new_data) + old_size, self->zst.next_in, left_size
            );
    ...
    if (self->zst.avail_in > 0 || PyBytes_GET_SIZE(self->unconsumed_tail)) {
        ...
        PyObject *new_data =
            PyBytes_FromStringAndSize((char *)self->zst.next_in, left_size);
        if (new_data == NULL)
            return -1;
...
zlib_Decompress_decompress_impl(
    compobject *self, PyTypeObject *cls, Py_buffer *data, Py_ssize_t max_length
)
/*[clinic end generated code: output=b024a93c2c922d57 input=bfb37b3864cfb606]*/
{
    ...
    if (OutputBuffer_InitAndGrow(
            &buffer, max_length, &self->zst.next_out, &self->zst.avail_out
        ) < 0) {
        goto abort;
    }
    ...
    do {
        ...
        do {
            if (self->zst.avail_out == 0) {
                if (OutputBuffer_GetDataSize(&buffer, self->zst.avail_out) ==
                    max_length) {
                    goto save;
                }
                if (OutputBuffer_Grow(
            ...
            Py_BEGIN_ALLOW_THREADS err = inflate(&self->zst, Z_SYNC_FLUSH);
            Py_END_ALLOW_THREADS
...
/*[clinic end generated code: output=c7efd13efd62add2 input=286146e29442eb6c]*/
{
    ...
    do {
        ...
        Py_BEGIN_ALLOW_THREADS err = deflate(&self->zst, mode);
        Py_END_ALLOW_THREADS
...
static PyObject *
zlib_Compress___deepcopy___impl(compobject *self, PyTypeObject *cls, PyObject *memo)
...
static PyObject *
zlib_Decompress___deepcopy___impl(compobject *self, PyTypeObject *cls, PyObject *memo)
...
static PyObject *
zlib_Decompress_flush_impl(compobject *self, PyTypeObject *cls, Py_ssize_t length)
/*[clinic end generated code: output=4532fc280bd0f8f2 input=42f1f4b75230e2cd]*/
{
    ...
    if (OutputBuffer_WindowInitWithSize(
            &buffer, &window, length, &self->zst.next_out, &self->zst.avail_out
        ) < 0) {
        goto abort;
    }
    ...
    do {
        ...
        do {
            if (self->zst.avail_out == 0) {
                if (OutputBuffer_WindowGrow(
                        &buffer, &window, &self->zst.next_out, &self->zst.avail_out
                    ) < 0) {
                    goto abort;
                }
            }
...
decompress_buf(ZlibDecompressor *self, Py_ssize_t max_length) {
    ...
    do {
        ...
        do {
            ...
            if (obuflen == -1) {
                PyErr_SetString(
                    PyExc_MemoryError, "Insufficient memory for buffer allocation"
                );
                goto error;
            } else if (obuflen == -2) {
                break;
            ...
            Py_BEGIN_ALLOW_THREADS err = inflate(&self->zst, Z_SYNC_FLUSH);
            Py_END_ALLOW_THREADS switch (err) {
                case Z_OK:
                    _Py_FALLTHROUGH;
                case Z_BUF_ERROR:
                    _Py_FALLTHROUGH;
    ...
    if (_PyBytes_Resize(
            &return_value,
            self->zst.next_out - (uint8_t *)PyBytes_AS_STRING(return_value)
        ) != 0) {
        goto error;
    }
...
static PyObject *
decompress(ZlibDecompressor *self, uint8_t *data, size_t len, Py_ssize_t max_length) {
    ...
    if (self->zst.next_in != NULL) {
        ...
        if (avail_total < len) {
            ...
            memmove(self->input_buffer, self->zst.next_in, self->avail_in_real);
...
zlib_ZlibDecompressor_decompress_impl(
    ZlibDecompressor *self, Py_buffer *data, Py_ssize_t max_length
)
...
ZlibDecompressor__new__(PyTypeObject *cls, PyObject *args, PyObject *kwargs) {
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, format, keywords, &wbits, &zdict)) {
        return NULL;
    }
...
static PyMethodDef comp_methods[] = {
    ZLIB_COMPRESS_COMPRESS_METHODDEF ZLIB_COMPRESS_FLUSH_METHODDEF
        ZLIB_COMPRESS_COPY_METHODDEF ZLIB_COMPRESS___COPY___METHODDEF
            ZLIB_COMPRESS___DEEPCOPY___METHODDEF{NULL, NULL}
};

static PyMethodDef Decomp_methods[] = {
    ZLIB_DECOMPRESS_DECOMPRESS_METHODDEF ZLIB_DECOMPRESS_FLUSH_METHODDEF
        ZLIB_DECOMPRESS_COPY_METHODDEF ZLIB_DECOMPRESS___COPY___METHODDEF
            ZLIB_DECOMPRESS___DEEPCOPY___METHODDEF{NULL, NULL}
};

static PyMethodDef ZlibDecompressor_methods[] = {
    ZLIB_ZLIBDECOMPRESSOR_DECOMPRESS_METHODDEF{NULL}
};
...
PyDoc_STRVAR(
    ZlibDecompressor_eof__doc__, "True if the end-of-stream marker has been reached."
);
...
static PyMemberDef ZlibDecompressor_members[] = {
    {"eof",
     Py_T_BOOL,
     offsetof(ZlibDecompressor, eof),
     Py_READONLY,
     ZlibDecompressor_eof__doc__},
    {"unused_data",
     Py_T_OBJECT_EX,
     offsetof(ZlibDecompressor, unused_data),
     Py_READONLY,
     ZlibDecompressor_unused_data__doc__},
    {"needs_input",
     Py_T_BOOL,
     offsetof(ZlibDecompressor, needs_input),
     Py_READONLY,
     ZlibDecompressor_needs_input_doc},
    {NULL},
};
...
static PyMethodDef zlib_methods[] = {
    ZLIB_ADLER32_METHODDEF ZLIB_COMPRESS_METHODDEF ZLIB_COMPRESSOBJ_METHODDEF
...
zlib_exec(PyObject *mod) {
    ...
    state->Comptype =
        (PyTypeObject *)PyType_FromModuleAndSpec(mod, &Comptype_spec, NULL);
    if (state->Comptype == NULL) {
        return -1;
    }

    state->Decomptype =
        (PyTypeObject *)PyType_FromModuleAndSpec(mod, &Decomptype_spec, NULL);
    ...
    if (PyModule_AddObjectRef(
            mod, "_ZlibDecompressor", (PyObject *)state->ZlibDecompressorType
        ) < 0) {
        return -1;
    }
...
#define ZLIB_ADD_INT_MACRO(c)                            \
    ...
    if (PyModule_Add(mod, "ZLIB_RUNTIME_VERSION", PyUnicode_FromString(zlibVersion())) <
        0) {
        return -1;
    }
