        ...
        if (cmd->cmd == LC_SEGMENT_64 && strcmp(cmd->segname, "__DATA") == 0) {
            while (cmd->filesize != size) {
                ...
                if (mach_vm_region(
                        ...
                        (vm_region_info_t
                        )&region_info,  // cppcheck-suppress [uninitvar]
                        &count,
                        &object_name
                    ) != KERN_SUCCESS) {
                    PyErr_SetString(
...
analyze_macho(char *path, void *base, mach_vm_size_t size, mach_port_t proc_ref) {
    ...
    switch (hdr->magic) {
        ...
        case FAT_CIGAM:
            PyErr_SetString(
                PyExc_RuntimeError, "32-bit Mach-O binaries are not supported"
            );
            break;
        case MH_MAGIC_64:
        case MH_CIGAM_64:
...
get_py_runtime_macos(pid_t pid) {
    ...
    while (mach_vm_region(
        ...
        if (!match_found && strncmp(filename, "python", 6) == 0) {
            match_found = 1;
            result_address =
                analyze_macho(map_filename, (void *)address, size, proc_ref);
        }
...
get_py_runtime_linux(pid_t pid) {
    ...
    if (start_address == 0) {
        PyErr_SetString(
            PyExc_RuntimeError,
            "No memory map associated with python or libpython found"
        );
        return NULL;
    }
    ...
    if (kr != KERN_SUCCESS) {
        switch (kr) {
            case KERN_PROTECTION_FAILURE:
                PyErr_SetString(
                    PyExc_PermissionError, "Not enough permissions to read memory"
                );
                break;
            case KERN_INVALID_ARGUMENT:
                PyErr_SetString(
                    PyExc_PermissionError, "Invalid argument to mach_vm_read_overwrite"
                );
                break;
            default:
...
read_string(
    ...
    ssize_t bytes_read = read_memory(
        pid, address + debug_offsets->unicode_object.length, sizeof(Py_ssize_t), &len
    );
    if (bytes_read == -1) {
        return -1;
    }
...
parse_code_object(
    ...
    if (read_string(
            pid, offsets, address_of_function_name, function_name, sizeof(function_name)
        ) != 0) {
        return -1;
    }
...
parse_frame_object(
    ...
    bytes_read = read_memory(
        pid, (void *)(address + offsets->interpreter_frame.owner), sizeof(char), &owner
    );
    if (bytes_read < 0) {
        return -1;
    }
    ...
    return parse_code_object(
        pid, result, offsets, (void *)address_of_code_object, previous_frame
    );
}
...
get_stack_trace(PyObject *self, PyObject *args) {
    ...
    PyErr_SetString(
        PyExc_RuntimeError, "get_stack_trace is not supported on this platform"
    );
    return NULL;
#endif
    int pid;
