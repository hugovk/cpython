...
SRE(at)(SRE_STATE *state, const SRE_CHAR *ptr, SRE_CODE at) {
    ...
    switch (at) {
        ...
        case SRE_AT_BEGINNING_LINE:
            return ((void *)ptr == state->beginning || SRE_IS_LINEBREAK((int)ptr[-1]));

        case SRE_AT_END:
            return (
                ((SRE_CHAR *)state->end - ptr == 1 && SRE_IS_LINEBREAK((int)ptr[0])) ||
                ((void *)ptr == state->end)
            );

        case SRE_AT_END_LINE:
            return ((void *)ptr == state->end || SRE_IS_LINEBREAK((int)ptr[0]));
        ...
        case SRE_AT_BOUNDARY:
            if (state->beginning == state->end)
                return 0;
            thatp = ((void *)ptr > state->beginning) ? SRE_IS_WORD((int)ptr[-1]) : 0;
            thisp = ((void *)ptr < state->end) ? SRE_IS_WORD((int)ptr[0]) : 0;
            return thisp != thatp;
        ...
        case SRE_AT_LOC_BOUNDARY:
            if (state->beginning == state->end)
                return 0;
            thatp =
                ((void *)ptr > state->beginning) ? SRE_LOC_IS_WORD((int)ptr[-1]) : 0;
            thisp = ((void *)ptr < state->end) ? SRE_LOC_IS_WORD((int)ptr[0]) : 0;
            return thisp != thatp;
        ...
        case SRE_AT_UNI_BOUNDARY:
            if (state->beginning == state->end)
                return 0;
            thatp =
                ((void *)ptr > state->beginning) ? SRE_UNI_IS_WORD((int)ptr[-1]) : 0;
            thisp = ((void *)ptr < state->end) ? SRE_UNI_IS_WORD((int)ptr[0]) : 0;
            return thisp != thatp;
        ...
        switch (*set++) {
            ...
            case SRE_OP_BIGCHARSET:
                /* <BIGCHARSET> <blockcount> <256 blockindices> <blocks> */
                {
                    ...
                    if (block >= 0 && (set[(block * 256 + (ch & 255)) / SRE_CODE_BITS] &
...
SRE(count)(SRE_STATE *state, const SRE_CODE *pattern, Py_ssize_t maxcount) {
    ...
    switch (pattern[0]) {
        ...
        case SRE_OP_ANY:
            ...
            TRACE(("|%p|%p|COUNT ANY\n", pattern, ptr));
            while (ptr < end && !SRE_IS_LINEBREAK(*ptr)) ptr++;
            break;
        ...
        case SRE_OP_LITERAL_IGNORE:
            ...
            while (ptr < end && (SRE_CODE)sre_lower_ascii(*ptr) == chr) ptr++;
            break;
        ...
        case SRE_OP_LITERAL_UNI_IGNORE:
            ...
            while (ptr < end && (SRE_CODE)sre_lower_unicode(*ptr) == chr) ptr++;
            break;
        ...
        case SRE_OP_LITERAL_LOC_IGNORE:
            ...
            while (ptr < end && char_loc_ignore(chr, *ptr)) ptr++;
            break;
        ...
        default:
            ...
            TRACE(("|%p|%p|COUNT %zd\n", pattern, ptr, (SRE_CHAR *)state->ptr - ptr));
...
#define LAST_PTR_PUSH()                                                    \
    do {                                                                   \
        TRACE(("push last_ptr: %zd", PTR_TO_INDEX(ctx->u.rep->last_ptr))); \
        DATA_PUSH(&ctx->u.rep->last_ptr);                                  \
    } while (0)
#define LAST_PTR_POP()                                                    \
    do {                                                                  \
        DATA_POP(&ctx->u.rep->last_ptr);                                  \
        TRACE(("pop last_ptr: %zd", PTR_TO_INDEX(ctx->u.rep->last_ptr))); \
    } while (0)
...
#endif
    {
        ...
        TARGET(SRE_OP_IN)
            : /* match set member (or non_member) */
              /* <IN> <skip> <set> */
              TRACE(("|%p|%p|IN\n", pattern, ptr));
        if (ptr >= end || !SRE(charset)(state, pattern + 1, *ptr))
            RETURN_FAILURE;
        pattern += pattern[0];
        ptr++;
        DISPATCH;
        ...
        TARGET(SRE_OP_IN_IGNORE) : TRACE(("|%p|%p|IN_IGNORE\n", pattern, ptr));
        if (ptr >= end ||
            !SRE(charset)(state, pattern + 1, (SRE_CODE)sre_lower_ascii(*ptr)))
            RETURN_FAILURE;
        pattern += pattern[0];
        ptr++;
        DISPATCH;

        TARGET(SRE_OP_IN_UNI_IGNORE) : TRACE(("|%p|%p|IN_UNI_IGNORE\n", pattern, ptr));
        if (ptr >= end ||
            !SRE(charset)(state, pattern + 1, (SRE_CODE)sre_lower_unicode(*ptr)))
            RETURN_FAILURE;
        pattern += pattern[0];
        ptr++;
        DISPATCH;

        TARGET(SRE_OP_IN_LOC_IGNORE) : TRACE(("|%p|%p|IN_LOC_IGNORE\n", pattern, ptr));
        if (ptr >= end || !SRE(charset_loc_ignore)(state, pattern + 1, *ptr))
            RETURN_FAILURE;
        pattern += pattern[0];
        ptr++;
        DISPATCH;
        ...
        TARGET(SRE_OP_REPEAT_ONE)
            : /* match repeated sequence (maximizing regexp) */
              ...
              TRACE(("|%p|%p|REPEAT_ONE %d %d\n", pattern, ptr, pattern[1], pattern[2])
              );
        ...
        TRACE(
            ("|%p|%p|MIN_UNTIL %zd %p\n", pattern, ptr, ctx->count, ctx->u.rep->pattern)
        );
              ...
              TRACE(("|%p|%p|GROUPREF_IGNORE %d\n", pattern, ptr, pattern[0]));
        {
            ...
            if (groupref >= state->lastmark) {
                ...
                if (!p || !e || e < p)
                    RETURN_FAILURE;
                while (p < e) {
                    if (ptr >= end || sre_lower_ascii(*ptr) != sre_lower_ascii(*p))
                        RETURN_FAILURE;
                    p++;
                    ptr++;
                }
              ...
              TRACE(("|%p|%p|GROUPREF_UNI_IGNORE %d\n", pattern, ptr, pattern[0]));
        {
            ...
            if (groupref >= state->lastmark) {
                ...
                if (!p || !e || e < p)
                    RETURN_FAILURE;
                while (p < e) {
                    if (ptr >= end || sre_lower_unicode(*ptr) != sre_lower_unicode(*p))
                        RETURN_FAILURE;
                    p++;
                    ptr++;
                }
              ...
              TRACE(("|%p|%p|GROUPREF_LOC_IGNORE %d\n", pattern, ptr, pattern[0]));
        {
            ...
            if (groupref >= state->lastmark) {
                ...
                if (!p || !e || e < p)
                    RETURN_FAILURE;
                while (p < e) {
                    if (ptr >= end || sre_lower_locale(*ptr) != sre_lower_locale(*p))
                        RETURN_FAILURE;
                    p++;
                    ptr++;
                }
