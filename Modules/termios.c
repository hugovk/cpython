...
/*[clinic end generated code: output=2b3da39db870e629 input=54dad9779ebe74b1]*/
{
    ...
    Py_BEGIN_ALLOW_THREADS r = tcgetattr(fd, &mode);
    Py_END_ALLOW_THREADS if (r == -1) {
...
/*[clinic end generated code: output=bcd2b0a7b98a4bf5 input=5dafabdd5a08f018]*/
{
    if (!PyList_Check(term) || PyList_Size(term) != 7) {
        PyErr_SetString(PyExc_TypeError, "tcsetattr, arg 3: must be 7 element list");
        return NULL;
    }
...
#define SET_FROM_LIST(TYPE, VAR, LIST, N)         \
    do {                                          \
...
#undef SET_FROM_LIST
    ...
    if (!PyList_Check(cc) || PyList_Size(cc) != NCCS) {
        PyErr_Format(
            PyExc_TypeError, "tcsetattr: attributes[6] must be %d element list", NCCS
        );
        return NULL;
    }
    ...
    for (i = 0; i < NCCS; i++) {
        ...
        if (PyBytes_Check(v) && PyBytes_Size(v) == 1)
            ...
            PyErr_SetString(
                PyExc_TypeError,
                "tcsetattr: elements of attributes must be bytes objects of length 1 "
                "or integers"
            );
            return NULL;
        }
    ...
    Py_BEGIN_ALLOW_THREADS r = tcsetattr(fd, when, &mode);
...
/*[clinic end generated code: output=5945f589b5d3ac66 input=dc2f32417691f8ed]*/
{
    ...
    Py_BEGIN_ALLOW_THREADS r = tcsendbreak(fd, duration);
    Py_END_ALLOW_THREADS
...
/*[clinic end generated code: output=5fd86944c6255955 input=c99241b140b32447]*/
{
    ...
    Py_BEGIN_ALLOW_THREADS r = tcdrain(fd);
    Py_END_ALLOW_THREADS
...
/*[clinic end generated code: output=2424f80312ec2f21 input=0f7d08122ddc07b5]*/
{
    ...
    Py_BEGIN_ALLOW_THREADS r = tcflush(fd, queue);
    Py_END_ALLOW_THREADS
...
/*[clinic end generated code: output=afd10928e6ea66eb input=c6aff0640b6efd9c]*/
{
    ...
    Py_BEGIN_ALLOW_THREADS r = tcflow(fd, action);
    Py_END_ALLOW_THREADS
...
/*[clinic end generated code: output=31825977d5325fb6 input=5706c379d7fd984d]*/
{
    ...
    Py_BEGIN_ALLOW_THREADS r = ioctl(fd, TIOCGWINSZ, &w);
    Py_END_ALLOW_THREADS
    ...
    Py_BEGIN_ALLOW_THREADS r = ioctl(fd, TIOCGSIZE, &s);
    Py_END_ALLOW_THREADS if (r == -1) {
        return PyErr_SetFromErrno(state->TermiosError);
    }
...
/*[clinic end generated code: output=2ac3c9bb6eda83e1 input=4a06424465b24aee]*/
{
    if (!PySequence_Check(winsz) || PySequence_Size(winsz) != 2) {
        PyErr_SetString(
            PyExc_TypeError, "tcsetwinsize, arg 2: must be a two-item sequence"
        );
        return NULL;
    }
    ...
    if ((((long)w.ws_row) != winsz_0) || (((long)w.ws_col) != winsz_1)) {
        PyErr_SetString(PyExc_OverflowError, "winsize value(s) out of range.");
        return NULL;
    }

    int r;
    Py_BEGIN_ALLOW_THREADS r = ioctl(fd, TIOCSWINSZ, &w);
    Py_END_ALLOW_THREADS
    ...
    Py_BEGIN_ALLOW_THREADS r = ioctl(fd, TIOCSSIZE, &s);
    Py_END_ALLOW_THREADS
    ...
    PyErr_SetString(
        PyExc_NotImplementedError,
        "requires termios.TIOCGWINSZ, termios.TIOCSWINSZ and/or termios.TIOCGSIZE, "
        "termios.TIOCSSIZE"
    );
    return NULL;
...
static PyMethodDef termios_methods[] = {
    TERMIOS_TCGETATTR_METHODDEF TERMIOS_TCSETATTR_METHODDEF
        TERMIOS_TCSENDBREAK_METHODDEF TERMIOS_TCDRAIN_METHODDEF
            TERMIOS_TCFLUSH_METHODDEF TERMIOS_TCFLOW_METHODDEF
                TERMIOS_TCGETWINSIZE_METHODDEF TERMIOS_TCSETWINSIZE_METHODDEF{
                    NULL, NULL
                }
};
...
static int
termiosmodule_traverse(PyObject *m, visitproc visit, void *arg) {
...
termios_exec(PyObject *mod) {
    ...
    while (constant->name != NULL) {
        if (strncmp(constant->name, "TIO", 3) == 0) {
            ...
            if (PyModule_Add(mod, constant->name, PyLong_FromUnsignedLong(value)) < 0) {
                return -1;
            }
        } else {
            if (PyModule_AddIntConstant(mod, constant->name, constant->value) < 0) {
                return -1;
            }
        }
        ++constant;
