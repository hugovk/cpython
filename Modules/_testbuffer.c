...
ndbuf_new(Py_ssize_t nitems, Py_ssize_t itemsize, Py_ssize_t offset, int flags) {
    ...
    len = nitems * itemsize;
    if (offset % itemsize) {
        PyErr_SetString(PyExc_ValueError, "offset must be a multiple of itemsize");
        return NULL;
    }
...
ndarray_dealloc(NDArrayObject *self) {
    if (self->head) {
        if (ND_IS_CONSUMER(self)) {
            ...
            while (self->head) ndbuf_pop(self);
        }
    }
    PyObject_Free(self);
}
...
static Py_ssize_t *
strides_from_shape(const ndbuf_t *, int flags);
...
static int
pack_from_list(PyObject *obj, PyObject *items, PyObject *format, Py_ssize_t itemsize) {
    ...
    for (i = 0; i < nitems; i++) {
        ...
        for (j = 1; j < 2 + nmemb; j++) PyTuple_SET_ITEM(args, j, NULL);
        ...
        if ((PyBytes_Check(item) || PyLong_Check(item) || PyFloat_Check(item)) &&
            nmemb == 1) {
...
pack_single(char *ptr, PyObject *item, const char *fmt, Py_ssize_t itemsize) {
    ...
    if ((PyBytes_Check(item) || PyLong_Check(item) || PyFloat_Check(item)) &&
        ...
        PyErr_SetString(
            PyExc_ValueError, "mismatch between initializer element and format string"
        );
        goto args_out;
    }
...
args_out:
    for (i = 0; i < 2 + nmemb; i++) Py_XINCREF(PyTuple_GET_ITEM(args, i));
    Py_XDECREF(args);
out:
...
copy_rec(
    ...
    for (i = 0; i < shape[0]; dptr += dstrides[0], sptr += sstrides[0], i++) {
        ...
        copy_rec(
            ...
            xdptr,
            dstrides + 1,
            dsuboffsets ? dsuboffsets + 1 : NULL,
            xsptr,
            sstrides + 1,
            ssuboffsets ? ssuboffsets + 1 : NULL,
            mem
        );
    }
}
...
cmp_structure(Py_buffer *dest, Py_buffer *src) {
    ...
    if (strcmp(FIX_FORMAT(dest->format), FIX_FORMAT(src->format)) != 0 ||
        dest->itemsize != src->itemsize || dest->ndim != src->ndim)
        return -1;
...
static int
init_simple(ndbuf_t *ndbuf, PyObject *items, PyObject *format, Py_ssize_t itemsize) {
    PyObject *mview;
    Py_buffer *base = &ndbuf->base;
    int ret;
...
seq_as_ssize_array(PyObject *seq, Py_ssize_t len, int is_shape) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (is_shape && x < 0) {
            PyErr_Format(PyExc_ValueError, "elements of shape must be integers >= 0");
            PyMem_Free(dest);
            return NULL;
        }
        dest[i] = x;
...
strides_from_shape(const ndbuf_t *ndbuf, int flags) {
    ...
    if (flags & ND_FORTRAN) {
        s[0] = base->itemsize;
        for (i = 1; i < base->ndim; i++) s[i] = s[i - 1] * base->shape[i - 1];
    } else {
        s[base->ndim - 1] = base->itemsize;
        for (i = base->ndim - 2; i >= 0; i--) s[i] = s[i + 1] * base->shape[i + 1];
    }
...
verify_structure(
    ...
    for (n = 0; n < ndim; n++)
        if (strides[n] % itemsize) {
            PyErr_SetString(PyExc_ValueError, "strides must be a multiple of itemsize");
            return -1;
        }
...
invalid_combination:
    PyErr_SetString(
        PyExc_ValueError, "invalid combination of buffer, shape and strides"
    );
    return -1;
}
...
init_suboffsets(ndbuf_t *ndbuf) {
    ...
    for (n = 1; n < base->ndim; n++) base->suboffsets[n] = -1;
...
init_len(Py_buffer *base) {
    ...
    base->len = 1;
    for (i = 0; i < base->ndim; i++) base->len *= base->shape[i];
    base->len *= base->itemsize;
}

static int
init_structure(ndbuf_t *ndbuf, PyObject *shape, PyObject *strides, Py_ssize_t ndim) {
    Py_buffer *base = &ndbuf->base;
    ...
    if (ndim == 0) {
        if (ndbuf->flags & ND_PIL) {
            PyErr_SetString(
                PyExc_TypeError, "ndim = 0 cannot be used in conjunction with ND_PIL"
            );
            return -1;
        }
    ...
    if (base->strides == NULL)
        return -1;
    if (verify_structure(
            base->len, base->itemsize, ndbuf->offset, base->shape, base->strides, ndim
        ) < 0)
        return -1;
...
init_ndbuf(
    ...
    if (ndim > ND_MAX_NDIM) {
        PyErr_Format(PyExc_ValueError, "ndim must not exceed %d", ND_MAX_NDIM);
        return NULL;
    }
    ...
    if (strides) {
        ...
        if (PySequence_Fast_GET_SIZE(strides) == 0)
            ...
            PyErr_SetString(
                PyExc_TypeError, "ND_FORTRAN cannot be used together with strides"
            );
            return NULL;
        } else if (PySequence_Fast_GET_SIZE(strides) != ndim) {
            PyErr_SetString(PyExc_ValueError, "len(shape) != len(strides)");
            return NULL;
        }
    }
    ...
    if (itemsize <= 0) {
        if (itemsize == 0) {
            PyErr_SetString(PyExc_ValueError, "itemsize must not be zero");
        }
        return NULL;
    }
    ...
    if (nitems == 0) {
        PyErr_SetString(
            PyExc_ValueError, "initializer list or tuple must not be empty"
        );
        Py_DECREF(items);
        return NULL;
    }
...
ndarray_push(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (ND_IS_CONSUMER(nd)) {
        PyErr_SetString(
            PyExc_BufferError, "structure of re-exporting object is immutable"
        );
        return NULL;
    }
    ...
    if (ndarray_push_base(nd, items, shape, strides, offset, format, flags) < 0)
        return NULL;
    Py_RETURN_NONE;
}
...
ndarray_pop(PyObject *self, PyObject *dummy) {
    ...
    if (nd->head->next == NULL) {
        PyErr_SetString(PyExc_BufferError, "list only has a single base");
        return NULL;
    }
...
ndarray_getbuf(NDArrayObject *self, Py_buffer *view, int flags) {
    ...
    *view = *base;
    ...
    if (base->ndim != 0 && ((REQ_SHAPE(flags) && base->shape == NULL) ||
        ...
        PyErr_SetString(
            PyExc_BufferError, "re-exporter does not provide format, shape or strides"
        );
        return -1;
    }

    if (baseflags & ND_GETBUF_FAIL) {
        PyErr_SetString(PyExc_BufferError, "ND_GETBUF_FAIL: forced test exception");
    ...
    if (REQ_WRITABLE(flags) && base->readonly) {
        PyErr_SetString(PyExc_BufferError, "ndarray is not writable");
        return -1;
    }
    if (!REQ_FORMAT(flags)) {
    ...
    if (REQ_C_CONTIGUOUS(flags) && !ND_C_CONTIGUOUS(baseflags)) {
        PyErr_SetString(PyExc_BufferError, "ndarray is not C-contiguous");
        return -1;
    }
    if (REQ_F_CONTIGUOUS(flags) && !ND_FORTRAN_CONTIGUOUS(baseflags)) {
        PyErr_SetString(PyExc_BufferError, "ndarray is not Fortran contiguous");
        return -1;
    }
    if (REQ_ANY_CONTIGUOUS(flags) && !ND_ANY_CONTIGUOUS(baseflags)) {
        PyErr_SetString(PyExc_BufferError, "ndarray is not contiguous");
        return -1;
    }
    if (!REQ_INDIRECT(flags) && (baseflags & ND_PIL)) {
        PyErr_SetString(
            PyExc_BufferError, "ndarray cannot be represented without suboffsets"
        );
        return -1;
    }
    ...
    if (ND_C_CONTIGUOUS(baseflags) != PyBuffer_IsContiguous(view, 'C') ||
        ...
        PyErr_SetString(PyExc_BufferError, "ndarray: contiguity mismatch in getbuf()");
        return -1;
    }
...
adjust_buf:
        ...
        Py_ssize_t n = dim - 1;
        while (n >= 0 && base->suboffsets[n] < 0) n--;
        if (n < 0)
...
type_error:
    PyErr_Format(
        PyExc_TypeError, "cannot index memory using \"%.200s\"", Py_TYPE(key)->tp_name
    );
err_occurred:
    Py_DECREF(nd);
    return NULL;
...
ndarray_ass_subscript(NDArrayObject *self, PyObject *key, PyObject *value) {
    ...
    if (dest->ndim == 0) {
        if (key == Py_Ellipsis || (PyTuple_Check(key) && PyTuple_GET_SIZE(key) == 0)) {
            ptr = (char *)dest->buf;
...
slice_indices(PyObject *self, PyObject *args) {
    ...
    if (!PySlice_Check(key)) {
        PyErr_SetString(PyExc_TypeError, "first argument must be a slice object");
        return NULL;
    }
...
ndarray_c_contig(PyObject *self, PyObject *dummy) {
    ...
    if (ret != ND_C_CONTIGUOUS(nd->head->flags)) {
        PyErr_SetString(
            PyExc_RuntimeError, "results from PyBuffer_IsContiguous() and flags differ"
        );
        return NULL;
    }
...
ndarray_add_suboffsets(PyObject *self, PyObject *dummy) {
    ...
    if (base->suboffsets != NULL) {
        PyErr_SetString(PyExc_TypeError, "cannot add suboffsets to PIL-style array");
        return NULL;
    }
    if (base->strides == NULL) {
        PyErr_SetString(
            PyExc_TypeError, "cannot add suboffsets to array without strides"
        );
        return NULL;
    }
    ...
    for (i = 0; i < base->ndim; i++) base->suboffsets[i] = -1;
...
get_pointer(PyObject *self, PyObject *args) {
    ...
    if (view.ndim > ND_MAX_NDIM) {
        PyErr_Format(PyExc_ValueError, "get_pointer(): ndim > %d", ND_MAX_NDIM);
        goto out;
    }
    if (PySequence_Fast_GET_SIZE(seq) != view.ndim) {
        PyErr_SetString(PyExc_ValueError, "get_pointer(): len(indices) != ndim");
        goto out;
    }
...
get_ascii_order(PyObject *order) {
    ...
    if (!PyUnicode_Check(order)) {
        PyErr_SetString(PyExc_TypeError, "order must be a string");
        return CHAR_MAX;
    }
    ...
    if (ord != 'C' && ord != 'F' && ord != 'A') {
        PyErr_SetString(PyExc_ValueError, "invalid order, must be C, F or A");
        return CHAR_MAX;
    }
...
get_contiguous(PyObject *self, PyObject *args) {
    ...
    if (!PyLong_Check(buffertype)) {
        PyErr_SetString(
            PyExc_TypeError, "buffertype must be PyBUF_READ or PyBUF_WRITE"
        );
        return NULL;
    }
...
static int
arraycmp(
    const Py_ssize_t *a1, const Py_ssize_t *a2, const Py_ssize_t *shape, Py_ssize_t ndim
) {
    Py_ssize_t i;
...
cmp_contig(PyObject *self, PyObject *args) {
    ...
    if (v1.len != v2.len || v1.itemsize != v2.itemsize || v1.ndim != v2.ndim ||
        !fmtcmp(v1.format, v2.format) || !!v1.shape != !!v2.shape ||
        !!v1.strides != !!v2.strides || !!v1.suboffsets != !!v2.suboffsets) {
        goto result;
    }

    if ((v1.shape && !arraycmp(v1.shape, v2.shape, NULL, v1.ndim)) ||
        (v1.strides && !arraycmp(v1.strides, v2.strides, v1.shape, v1.ndim)) ||
        (v1.suboffsets && !arraycmp(v1.suboffsets, v2.suboffsets, NULL, v1.ndim))) {
        goto result;
    }
...
ndarray_hash(PyObject *self) {
    ...
    if (!view->readonly) {
        PyErr_SetString(PyExc_ValueError, "cannot hash writable ndarray object");
        return -1;
    }
...
static PyTypeObject NDArray_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "ndarray", /* Name of this type */
...
typedef struct {
    PyObject_HEAD int legacy_mode; /* if true, use the view.obj==NULL hack */
} StaticArrayObject;
...
staticarray_init(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    static char *kwlist[] = {"legacy_mode", NULL};
...
static PyTypeObject StaticArray_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "staticarray", /* Name of this type */
