...
/*[clinic end generated code: output=be23376e1a185231 input=933f8107993f23d0]*/
{
    ...
    if (rc < 0) {
        if (default_value == NULL) {
            PyErr_SetString(PyExc_ValueError, "not a decimal");
            return NULL;
        } else {
...
/*[clinic end generated code: output=7d699f3ec7565d27 input=e4c12459ad68507b]*/
{
    ...
    if (code < 0 || code >= 0x110000)
        ...
        index =
            decomp_index2[(index << DECOMP_SHIFT) + (code & ((1 << DECOMP_SHIFT) - 1))];
    }
    ...
    while (count-- > 0) {
        ...
        PyOS_snprintf(decomp + i, sizeof(decomp) - i, "%04X", decomp_data[++index]);
        i += strlen(decomp + i);
    }
...
get_decomp_record(PyObject *self, Py_UCS4 code, int *index, int *prefix, int *count) {
    if (code >= 0x110000) {
        *index = 0;
    } else if (UCD_Check(self) && get_old_record(self, code)->category_changed == 0) {
        /* unassigned in old version */
        *index = 0;
    } else {
        *index = decomp_index1[(code >> DECOMP_SHIFT)];
        *index = decomp_index2
            [(*index << DECOMP_SHIFT) + (code & ((1 << DECOMP_SHIFT) - 1))];
    }
...
nfd_nfkd(PyObject *self, PyObject *input, int k) {
    ...
    for (i++; i < PyUnicode_GET_LENGTH(result); i++) {
        ...
        while (1) {
            ...
            PyUnicode_WRITE(kind, data, o + 1, PyUnicode_READ(kind, data, o));
...
again:
    while (i < len) {
        ...
        if (LBase <= code && code < (LBase + LCount) && i + 1 < len &&
            ...
            i += 2;
            if (i < len && TBase < PyUnicode_READ(kind, data, i) &&
...
not_combinable:
            ...
            code =
                comp_data[(index1 << COMP_SHIFT) + (index & ((1 << COMP_SHIFT) - 1))];
            if (code == 0)
                goto not_combinable;
...
is_normalized_quickcheck(
    PyObject *self, PyObject *input, bool nfc, bool k, bool yes_only
) {
...
is_unified_ideograph(Py_UCS4 code) {
    return (0x3400 <= code && code <= 0x4DBF) ||   /* CJK Ideograph Extension A */
...
#define IS_NAMED_SEQ(cp) ((cp >= named_sequences_start) && (cp < named_sequences_end))
...
_dawg_decode_edge(
    bool is_first_edge,
    unsigned int prev_target_node_offset,
    unsigned int edge_offset,
    unsigned int *size,
    unsigned int *label_offset,
    unsigned int *target_node_offset
) {
    unsigned int num;
...
_lookup_dawg_packed(const char *name, unsigned int namelen) {
    ...
    unsigned int result =
        0;  // this is the number of final nodes that we skipped to match name
    while (stringpos < namelen) {
        ...
        for (;;) {
            ...
            int matched =
                _dawg_match_edge(name, namelen, size, label_offset, stringpos);
            if (matched == -1) {
                return -1;
            }
            if (matched) {
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            unsigned int descendant_count =
                _dawg_node_descendant_count(target_node_offset);
...
static int
_getucname(
    PyObject *self, Py_UCS4 code, char *buffer, int buflen, int with_alias_and_seq
) {
    ...
    offset = dawg_codepoint_to_pos_index2
        [(offset << DAWG_CODEPOINT_TO_POS_SHIFT) +
         (code & ((1 << DAWG_CODEPOINT_TO_POS_SHIFT) - 1))];
    if (offset == DAWG_CODEPOINT_TO_POS_NOTFOUND)
        return 0;

    assert(buflen >= 0);
    return _inverse_dawg_lookup(
        buffer, Py_SAFE_DOWNCAST(buflen, int, unsigned int), offset
    );
}

static int
capi_getucname(Py_UCS4 code, char *buffer, int buflen, int with_alias_and_seq) {
...
_getcode(const char *name, int namelen, Py_UCS4 *code) {
    ...
    assert(namelen >= 0);
    int position =
        _lookup_dawg_packed(name, Py_SAFE_DOWNCAST(namelen, int, unsigned int));
    if (position < 0) {
        return 0;
    }
...
static int
capi_getcode(const char *name, int namelen, Py_UCS4 *code, int with_named_seq) {
...
unicodedata_create_capi(void) {
    ...
    PyObject *capsule =
        PyCapsule_New(capi, PyUnicodeData_CAPSULE_NAME, unicodedata_destroy_capi);
    if (capsule == NULL) {
        PyMem_Free(capi);
    }
    return capsule;
...
static PyMethodDef unicodedata_functions[] = {
    UNICODEDATA_UCD_DECIMAL_METHODDEF UNICODEDATA_UCD_DIGIT_METHODDEF
        UNICODEDATA_UCD_NUMERIC_METHODDEF UNICODEDATA_UCD_CATEGORY_METHODDEF
            UNICODEDATA_UCD_BIDIRECTIONAL_METHODDEF UNICODEDATA_UCD_COMBINING_METHODDEF
...
unicodedata_exec(PyObject *module) {
    ...
    PyObject *v;
    v = new_previous_version(ucd_type, "3.2.0", get_change_3_2_0, normalization_3_2_0);
    Py_DECREF(ucd_type);
