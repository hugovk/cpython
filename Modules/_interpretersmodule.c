...
_memoryview_shared(
    ...
    _PyCrossInterpreterData_Init(
        data, tstate->interp, view, NULL, _memoryview_from_xid
    );
    return 0;
}
...
register_memoryview_xid(PyObject *mod, PyTypeObject **p_state) {
    ...
    assert(*p_state == NULL);
    PyTypeObject *cls =
        (PyTypeObject *)PyType_FromModuleAndSpec(mod, &XIBufferViewType_spec, NULL);
    if (cls == NULL) {
        return -1;
    }
...
check_code_object(PyCodeObject *code) {
    assert(code != NULL);
    if (code->co_argcount > 0 || code->co_posonlyargcount > 0 ||
        code->co_kwonlyargcount > 0 || code->co_flags & (CO_VARARGS | CO_VARKEYWORDS)) {
...
get_code_str(PyObject *arg, Py_ssize_t *len_p, PyObject **bytes_p, int *flags_p) {
    ...
    if (PyUnicode_Check(arg)) {
        ...
        if (strlen(codestr) != (size_t)len) {
            PyErr_SetString(
                PyExc_ValueError, "source code string cannot contain null bytes"
            );
            return NULL;
        }
        flags = RUN_TEXT;
    } else {
        assert(PyCode_Check(arg) && (check_code_object((PyCodeObject *)arg) == NULL));
        flags = RUN_CODE;
...
init_named_config(PyInterpreterConfig *config, const char *name) {
    if (name == NULL || strcmp(name, "") == 0 || strcmp(name, "default") == 0) {
        name = "isolated";
    }
    ...
    if (strcmp(name, "isolated") == 0) {
        ...
        *config = (PyInterpreterConfig){0};
        ...
        PyErr_Format(PyExc_ValueError, "unsupported config name '%s'", name);
        return -1;
    }
    return 0;
}
...
resolve_interp(PyObject *idobj, int restricted, int reqready, const char *op) {
    ...
    if (reqready && !_PyInterpreterState_IsReady(interp)) {
        if (idobj == NULL) {
            PyErr_Format(
                PyExc_InterpreterError, "cannot %s current interpreter (not ready)", op
            );
        } else {
            PyErr_Format(
                PyExc_InterpreterError,
                "cannot %s interpreter %R (not ready)",
                op,
                idobj
            );
        }
    ...
    if (restricted && get_whence(interp) != _PyInterpreterState_WHENCE_STDLIB) {
        if (idobj == NULL) {
            PyErr_Format(
                PyExc_InterpreterError, "cannot %s unrecognized current interpreter", op
            );
        } else {
            PyErr_Format(
                PyExc_InterpreterError,
                "cannot %s unrecognized interpreter %R",
                op,
                idobj
            );
        }
...
get_summary(PyInterpreterState *interp) {
    ...
    PyObject *whenceobj = PyLong_FromLong(get_whence(interp));
    if (whenceobj == NULL) {
        Py_DECREF(idobj);
        return NULL;
    }
...
interp_create(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwds, "|O$p:create", kwlist, &configobj, &reqrefs
        )) {
        return NULL;
    }
    ...
    PyInterpreterState *interp = _PyXI_NewInterpreter(&config, &whence, NULL, NULL);
    if (interp == NULL) {
...
interp_destroy(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    // XXX Use "L" for id?
    if (!PyArg_ParseTupleAndKeywords(
            args, kwds, "O|$p:destroy", kwlist, &id, &restricted
        )) {
        return NULL;
    }
    ...
    int reqready = 0;
    PyInterpreterState *interp = resolve_interp(id, restricted, reqready, "destroy");
    if (interp == NULL) {
        return NULL;
    }
    ...
    if (interp == current) {
        PyErr_SetString(
            PyExc_InterpreterError, "cannot destroy the current interpreter"
        );
        return NULL;
    }
...
interp_list_all(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    int reqready = 0;
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "|$p:" MODULE_NAME_STR ".list_all", kwlist, &reqready
        )) {
        return NULL;
    }
...
interp_set___main___attrs(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if (updates != Py_None) {
        ...
        if (size == 0) {
            PyErr_SetString(PyExc_ValueError, "arg 2 must be a non-empty mapping");
            return NULL;
        }
    }
...
convert_script_arg(
    PyObject *arg, const char *fname, const char *displayname, const char *expected
) {
    ...
    if (err != NULL) {
        Py_DECREF(str);
        PyErr_Format(PyExc_ValueError, "%.200s(): bad script text (%s)", fname, err);
        return NULL;
    }
...
convert_code_arg(
    ...
    if (PyFunction_Check(arg)) {
        if (PyFunction_GetClosure(arg) != NULL) {
            PyErr_Format(PyExc_ValueError, "%.200s(): closures not supported", fname);
            return NULL;
        }
        ...
        if (code == NULL) {
            if (PyErr_Occurred()) {
                // This chains.
                PyErr_Format(PyExc_ValueError, "%.200s(): bad func", fname);
            } else {
                PyErr_Format(
                    PyExc_ValueError, "%.200s(): func.__code__ missing", fname
                );
            }
            return NULL;
        }
    ...
    if (err != NULL) {
        Py_DECREF(code);
        PyErr_Format(PyExc_ValueError, "%.200s(): bad %s (%s)", fname, kind, err);
        return NULL;
    }
...
_interp_exec(
    ...
    PyObject *bytes_obj = NULL;
    int flags = 0;
    const char *codestr = get_code_str(code_arg, &codestrlen, &bytes_obj, &flags);
    if (codestr == NULL) {
        return -1;
    }
    ...
    int res =
        _run_in_interpreter(interp, codestr, codestrlen, shared_arg, flags, p_excinfo);
...
interp_exec(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (PyUnicode_Check(code)) {
        code = (PyObject *)convert_script_arg(
            code, MODULE_NAME_STR ".exec", "argument 2", expected
        );
    } else {
        code = (PyObject *)convert_code_arg(
            code, MODULE_NAME_STR ".exec", "argument 2", expected
        );
    }
    if (code == NULL) {
...
interp_call(PyObject *self, PyObject *args, PyObject *kwds) {
    static char *kwlist[] = {"id", "callable", "args", "kwargs", "restrict", NULL};
    ...
    PyObject *code = (PyObject *)convert_code_arg(
        callable, MODULE_NAME_STR ".call", "argument 2", "a function"
    );
    if (code == NULL) {
        return NULL;
    }
...
interp_run_string(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    script = (PyObject *)convert_script_arg(
        script, MODULE_NAME_STR ".exec", "argument 2", "a string"
    );
    if (script == NULL) {
        return NULL;
    }
...
interp_run_func(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    PyCodeObject *code = convert_code_arg(
        func, MODULE_NAME_STR ".exec", "argument 2", "a function or a code object"
    );
    if (code == NULL) {
        return NULL;
    }
...
object_is_shareable(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    PyObject *obj;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O:is_shareable", kwlist, &obj)) {
        return NULL;
    }
...
interp_is_running(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    int restricted = 0;
    if (!PyArg_ParseTupleAndKeywords(
            args, kwds, "O|$p:is_running", kwlist, &id, &restricted
        )) {
        return NULL;
    }
...
interp_get_config(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    int restricted = 0;
    if (!PyArg_ParseTupleAndKeywords(
            args, kwds, "O|$p:get_config", kwlist, &idobj, &restricted
        )) {
        return NULL;
    }
    if (idobj == Py_None) {
...
interp_whence(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    PyObject *id;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O:whence", kwlist, &id)) {
        return NULL;
    }
...
interp_incref(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    int restricted = 0;
    if (!PyArg_ParseTupleAndKeywords(
            args, kwds, "O|$pp:incref", kwlist, &id, &implieslink, &restricted
        )) {
        return NULL;
    }

    int reqready = 1;
    PyInterpreterState *interp = resolve_interp(id, restricted, reqready, "incref");
    if (interp == NULL) {
        return NULL;
    }
...
interp_decref(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    int restricted = 0;
    if (!PyArg_ParseTupleAndKeywords(
            args, kwds, "O|$p:decref", kwlist, &id, &restricted
        )) {
        return NULL;
    }

    int reqready = 1;
    PyInterpreterState *interp = resolve_interp(id, restricted, reqready, "decref");
    if (interp == NULL) {
        return NULL;
    }
...
capture_exception(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwds, "|O:capture_exception", kwlist, &exc_arg
        )) {
        return NULL;
    }
...
static PyMethodDef module_functions[] = {
    {"new_config",
     _PyCFunction_CAST(interp_new_config),
     METH_VARARGS | METH_KEYWORDS,
     new_config_doc},

    {"create",
     _PyCFunction_CAST(interp_create),
     METH_VARARGS | METH_KEYWORDS,
     create_doc},
    {"destroy",
     _PyCFunction_CAST(interp_destroy),
     METH_VARARGS | METH_KEYWORDS,
     destroy_doc},
    {"list_all",
     _PyCFunction_CAST(interp_list_all),
     METH_VARARGS | METH_KEYWORDS,
     list_all_doc},
    {"get_current", interp_get_current, METH_NOARGS, get_current_doc},
    {"get_main", interp_get_main, METH_NOARGS, get_main_doc},

    {"is_running",
     _PyCFunction_CAST(interp_is_running),
     METH_VARARGS | METH_KEYWORDS,
     is_running_doc},
    {"get_config",
     _PyCFunction_CAST(interp_get_config),
     METH_VARARGS | METH_KEYWORDS,
     get_config_doc},
    {"whence",
     _PyCFunction_CAST(interp_whence),
     METH_VARARGS | METH_KEYWORDS,
     whence_doc},
    {"exec", _PyCFunction_CAST(interp_exec), METH_VARARGS | METH_KEYWORDS, exec_doc},
    {"call", _PyCFunction_CAST(interp_call), METH_VARARGS | METH_KEYWORDS, call_doc},
    {"run_string",
     _PyCFunction_CAST(interp_run_string),
     METH_VARARGS | METH_KEYWORDS,
     run_string_doc},
    {"run_func",
     _PyCFunction_CAST(interp_run_func),
     METH_VARARGS | METH_KEYWORDS,
     run_func_doc},

    {"set___main___attrs",
     _PyCFunction_CAST(interp_set___main___attrs),
     METH_VARARGS | METH_KEYWORDS,
     set___main___attrs_doc},

    {"incref", _PyCFunction_CAST(interp_incref), METH_VARARGS | METH_KEYWORDS, NULL},
    {"decref", _PyCFunction_CAST(interp_decref), METH_VARARGS | METH_KEYWORDS, NULL},

    {"is_shareable",
     _PyCFunction_CAST(object_is_shareable),
     METH_VARARGS | METH_KEYWORDS,
     is_shareable_doc},

    {"capture_exception",
     _PyCFunction_CAST(capture_exception),
     METH_VARARGS | METH_KEYWORDS,
     capture_exception_doc},
...
#define ADD_WHENCE(NAME)                                            \
    if (PyModule_AddIntConstant(                                    \
            mod, "WHENCE_" #NAME, _PyInterpreterState_WHENCE_##NAME \
        ) < 0) {                                                    \
        goto error;                                                 \
    }
    ADD_WHENCE(UNKNOWN)
