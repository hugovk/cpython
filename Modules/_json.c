...
typedef struct _PyScannerObject {
    PyObject_HEAD signed char strict;
...
static PyMemberDef scanner_members[] = {
    ...
    {"object_hook",
     _Py_T_OBJECT,
     offsetof(PyScannerObject, object_hook),
     Py_READONLY,
     "object_hook"},
    {"object_pairs_hook",
     _Py_T_OBJECT,
     offsetof(PyScannerObject, object_pairs_hook),
     Py_READONLY},
    {"parse_float",
     _Py_T_OBJECT,
     offsetof(PyScannerObject, parse_float),
     Py_READONLY,
     "parse_float"},
    {"parse_int",
     _Py_T_OBJECT,
     offsetof(PyScannerObject, parse_int),
     Py_READONLY,
     "parse_int"},
    {"parse_constant",
     _Py_T_OBJECT,
     offsetof(PyScannerObject, parse_constant),
     Py_READONLY,
     "parse_constant"},
    {NULL}
};
...
static PyMemberDef encoder_members[] = {
    ...
    {"default",
     _Py_T_OBJECT,
     offsetof(PyEncoderObject, defaultfn),
     Py_READONLY,
     "default"},
    ...
    {"key_separator",
     _Py_T_OBJECT,
     offsetof(PyEncoderObject, key_separator),
     Py_READONLY,
     "key_separator"},
    {"item_separator",
     _Py_T_OBJECT,
     offsetof(PyEncoderObject, item_separator),
     Py_READONLY,
     "item_separator"},
    {"sort_keys",
     Py_T_BOOL,
     offsetof(PyEncoderObject, sort_keys),
     Py_READONLY,
     "sort_keys"},
...
encoder_listencode_list(
    PyEncoderObject *s, PyUnicodeWriter *writer, PyObject *seq, PyObject *newline_indent
);
static int
encoder_listencode_obj(
    PyEncoderObject *s, PyUnicodeWriter *writer, PyObject *obj, PyObject *newline_indent
);
static int
encoder_listencode_dict(
    PyEncoderObject *s, PyUnicodeWriter *writer, PyObject *dct, PyObject *newline_indent
);
static PyObject *
...
static PyObject *
scanstring_unicode(
    PyObject *pystr, Py_ssize_t end, int strict, Py_ssize_t *next_end_ptr
) {
    ...
    */
    ...
    if (idx > end_idx || PyUnicode_READ(kind, str, idx) != '}') {
        while (1) {
            ...
            if (idx > end_idx || PyUnicode_READ(kind, str, idx) != '"') {
                raise_errmsg(
                    "Expecting property name enclosed in double quotes", pystr, idx
                );
                goto bail;
            }
            ...
            while (idx <= end_idx && IS_WHITESPACE(PyUnicode_READ(kind, str, idx)))
                idx++;
            if (idx > end_idx || PyUnicode_READ(kind, str, idx) != ':') {
            ...
            if (idx <= end_idx && PyUnicode_READ(kind, str, idx) == '}') {
                raise_errmsg(
                    "Illegal trailing comma before end of object", pystr, comma_idx
                );
                goto bail;
            }
        }
    ...
    */
    ...
    if (idx > end_idx || PyUnicode_READ(kind, str, idx) != ']') {
        while (1) {
            ...
            if (idx <= end_idx && PyUnicode_READ(kind, str, idx) == ']') {
                raise_errmsg(
                    "Illegal trailing comma before end of array", pystr, comma_idx
                );
                goto bail;
            }
        }
...
static PyObject *
_parse_constant(
    PyScannerObject *s, const char *constant, Py_ssize_t idx, Py_ssize_t *next_idx_ptr
) {
...
_match_number_unicode(
    ...
    */
    ...
    if (PyUnicode_READ(kind, str, idx) >= '1' &&
        PyUnicode_READ(kind, str, idx) <= '9') {
        idx++;
        while (idx <= end_idx && PyUnicode_READ(kind, str, idx) >= '0' &&
               PyUnicode_READ(kind, str, idx) <= '9')
            idx++;
    }
    ...
    if (idx < end_idx && PyUnicode_READ(kind, str, idx) == '.' &&
        PyUnicode_READ(kind, str, idx + 1) >= '0' &&
        PyUnicode_READ(kind, str, idx + 1) <= '9') {
        is_float = 1;
        idx += 2;
        while (idx <= end_idx && PyUnicode_READ(kind, str, idx) >= '0' &&
               PyUnicode_READ(kind, str, idx) <= '9')
    ...
    if (idx < end_idx && (PyUnicode_READ(kind, str, idx) == 'e' ||
        ...
        if (PyUnicode_READ(kind, str, idx - 1) >= '0' &&
            PyUnicode_READ(kind, str, idx - 1) <= '9') {
            is_float = 1;
        } else {
            idx = e_start;
    ...
    if (custom_func) {
        ...
        numstr =
            PyUnicode_FromKindAndData(kind, (char *)str + kind * start, idx - start);
        if (numstr == NULL)
            return NULL;
...
scan_once_unicode(
    ...
    */
    ...
    switch (PyUnicode_READ(kind, str, idx)) {
        ...
        case 'n':
            /* null */
            if ((idx + 3 < length) && PyUnicode_READ(kind, str, idx + 1) == 'u' &&
        ...
        case 't':
            /* true */
            if ((idx + 3 < length) && PyUnicode_READ(kind, str, idx + 1) == 'r' &&
...
_create_newline_indent(PyObject *indent, Py_ssize_t indent_level) {
    ...
    if (newline_indent != NULL && indent_level) {
        PyUnicode_AppendAndDel(
            &newline_indent, PySequence_Repeat(indent, indent_level)
        );
    }
...
encoder_call(PyEncoderObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwds, "On:_iterencode", kwlist, &obj, &indent_level
        ))
        return NULL;
...
PyDoc_STRVAR(
    encoder_doc,
    "Encoder(markers, default, encoder, indent, key_separator, item_separator, "
...
static PyMethodDef speedups_methods[] = {
    ...
    {"encode_basestring",
     ...
     METH_O,
     pydoc_encode_basestring},
    {"scanstring", (PyCFunction)py_scanstring, METH_VARARGS, pydoc_scanstring},
    {NULL, NULL, 0, NULL}
};

PyDoc_STRVAR(module_doc, "json speedups\n");
