...
codec_incrementalencoder(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "s|s:test_incrementalencoder", &encoding, &errors))
        return NULL;
...
codec_incrementaldecoder(PyObject *self, PyObject *args) {
    const char *encoding, *errors = NULL;
    if (!PyArg_ParseTuple(args, "s|s:test_incrementaldecoder", &encoding, &errors))
        return NULL;
...
unicode_copy(PyObject *unicode) {
    ...
    PyObject *copy =
        PyUnicode_DecodeUTF8(PyBytes_AsString(utf8), PyBytes_Size(utf8), NULL);
    Py_DECREF(utf8);
...
unicode_fromwidechar(PyObject *self, PyObject *args) {
    ...
    if (size == -100) {
        if (bsize % SIZEOF_WCHAR_T) {
            PyErr_SetString(
                PyExc_AssertionError, "invalid size in unicode_fromwidechar()"
            );
            return NULL;
        }
...
unicode_findchar(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(
            args, "OInni:unicode_findchar", &str, &ch, &start, &end, &direction
        )) {
        return NULL;
    }
    NULLABLE(str);
...
check_raised_systemerror(PyObject *result, char *msg) {
    if (result) {
        // no exception
        PyErr_Format(PyExc_AssertionError, "SystemError not raised: %s", msg);
        return 0;
    }
...
#define CHECK_FORMAT_1(FORMAT, EXPECTED, ARG) CHECK_FORMAT_2(FORMAT, EXPECTED, ARG, 0)

#define CHECK_FORMAT_0(FORMAT, EXPECTED) CHECK_FORMAT_2(FORMAT, EXPECTED, 0, 0)
...
static PyMethodDef TestMethods[] = {
    ...
    {"test_unicode_compare_with_ascii", test_unicode_compare_with_ascii, METH_NOARGS},
