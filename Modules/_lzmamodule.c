...
/* On success, return value >= 0
   On failure, return -1 */
static inline Py_ssize_t
OutputBuffer_Grow(_BlocksOutputBuffer *buffer, uint8_t **next_out, size_t *avail_out) {
    Py_ssize_t allocated;
...
typedef struct {
    PyObject_HEAD lzma_allocator alloc;
    lzma_stream lzs;
    int flushed;
...
#define INT_TYPE_CONVERTER_FUNC(TYPE, FUNCNAME)                           \
    static int FUNCNAME(PyObject *obj, void *ptr) {                       \
        ...
        if ((unsigned long long)(TYPE)val != val) {                       \
            PyErr_SetString(                                              \
                PyExc_OverflowError, "Value too large for " #TYPE " type" \
            );                                                            \
            return 0;                                                     \
        }                                                                 \
        *(TYPE *)ptr = (TYPE)val;                                         \
...
parse_filter_spec_lzma(_lzma_state *state, PyObject *spec) {
    ...
    if (!PyArg_ParseTupleAndKeywords(
        ...
        PyErr_SetString(PyExc_ValueError, "Invalid filter specifier for LZMA filter");
        PyMem_Free(options);
        return NULL;
    }
...
parse_filter_spec_delta(_lzma_state *state, PyObject *spec) {
    ...
    if (!PyArg_ParseTupleAndKeywords(
            state->empty_tuple, spec, "|OO&", optnames, &id, uint32_converter, &dist
        )) {
        PyErr_SetString(PyExc_ValueError, "Invalid filter specifier for delta filter");
        return NULL;
    }
...
parse_filter_spec_bcj(_lzma_state *state, PyObject *spec) {
    ...
    if (!PyArg_ParseTupleAndKeywords(
        ...
        PyErr_SetString(PyExc_ValueError, "Invalid filter specifier for BCJ filter");
        return NULL;
    }
...
lzma_filter_converter(_lzma_state *state, PyObject *spec, void *ptr) {
    ...
    if (!PyMapping_Check(spec)) {
        PyErr_SetString(
            PyExc_TypeError, "Filter specifier must be a dict or dict-like object"
        );
        return 0;
    }
...
parse_filter_chain_spec(
    _lzma_state *state, lzma_filter filters[], PyObject *filterspecs
) {
...
compress(Compressor *c, uint8_t *data, size_t len, lzma_action action) {
    ...
    if (OutputBuffer_InitAndGrow(&buffer, -1, &c->lzs.next_out, &c->lzs.avail_out) <
        0) {
        goto error;
    }
    c->lzs.next_in = data;
    ...
    for (;;) {
        ...
        Py_BEGIN_ALLOW_THREADS lzret = lzma_code(&c->lzs, action);
        Py_END_ALLOW_THREADS
...
Compressor_init_alone(
    _lzma_state *state, lzma_stream *lzs, uint32_t preset, PyObject *filterspecs
) {
    lzma_ret lzret;
    ...
    if (filterspecs == Py_None) {
        ...
        if (parse_filter_chain_spec(state, filters, filterspecs) == -1)
            return -1;
        if (filters[0].id == LZMA_FILTER_LZMA1 && filters[1].id == LZMA_VLI_UNKNOWN) {
...
Compressor_init_raw(_lzma_state *state, lzma_stream *lzs, PyObject *filterspecs) {
    ...
    if (filterspecs == Py_None) {
        PyErr_SetString(PyExc_ValueError, "Must specify filters for FORMAT_RAW");
        return -1;
    }
    if (parse_filter_chain_spec(state, filters, filterspecs) == -1) {
        return -1;
...
Compressor_new(PyTypeObject *type, PyObject *args, PyObject *kwargs) {
    ...
    if (format != FORMAT_XZ && check != -1 && check != LZMA_CHECK_NONE) {
        PyErr_SetString(
            PyExc_ValueError, "Integrity checks are only supported by FORMAT_XZ"
        );
        return NULL;
    }
    ...
    if (preset_obj != Py_None && filterspecs != Py_None) {
        PyErr_SetString(
            PyExc_ValueError, "Cannot specify both preset and filter chain"
        );
        return NULL;
    }
    ...
    switch (format) {
        case FORMAT_XZ:
            ...
            if (Compressor_init_xz(state, &self->lzs, check, preset, filterspecs) !=
                0) {
                goto error;
            }
            break;
        ...
        default:
            PyErr_Format(PyExc_ValueError, "Invalid container format: %d", format);
            goto error;
    }
...
static PyMethodDef Compressor_methods[] = {
    _LZMA_LZMACOMPRESSOR_COMPRESS_METHODDEF _LZMA_LZMACOMPRESSOR_FLUSH_METHODDEF{NULL}
};
...
decompress_buf(Decompressor *d, Py_ssize_t max_length) {
    ...
    if (OutputBuffer_InitAndGrow(&buffer, max_length, &lzs->next_out, &lzs->avail_out) <
        0) {
        goto error;
    }
    ...
    for (;;) {
        ...
        Py_BEGIN_ALLOW_THREADS lzret = lzma_code(lzs, LZMA_RUN);
        Py_END_ALLOW_THREADS
...
decompress(Decompressor *d, uint8_t *data, size_t len, Py_ssize_t max_length) {
    ...
    if (lzs->next_in != NULL) {
        ...
        avail_now =
            (d->input_buffer + d->input_buffer_size) - (lzs->next_in + lzs->avail_in);
        ...
        if (avail_total < len) {
            ...
            memmove(d->input_buffer, lzs->next_in, lzs->avail_in);
    ...
    if (d->eof) {
        ...
        if (!input_buffer_in_use) {
            ...
            if (d->input_buffer != NULL && d->input_buffer_size < lzs->avail_in) {
...
_lzma_LZMADecompressor_decompress_impl(
    Decompressor *self, Py_buffer *data, Py_ssize_t max_length
)
...
static PyObject *
_lzma_LZMADecompressor_impl(
    PyTypeObject *type, int format, PyObject *memlimit, PyObject *filters
)
/*[clinic end generated code: output=2d46d5e70f10bc7f input=ca40cd1cb1202b0d]*/
{
    ...
    if (memlimit != Py_None) {
        if (format == FORMAT_RAW) {
            PyErr_SetString(
                PyExc_ValueError, "Cannot specify memory limit with FORMAT_RAW"
            );
            return NULL;
        }
    ...
    if (format == FORMAT_RAW && filters == Py_None) {
        ...
        PyErr_SetString(
            PyExc_ValueError, "Cannot specify filters except with FORMAT_RAW"
        );
        return NULL;
    }
...
static PyMethodDef Decompressor_methods[] = {
    _LZMA_LZMADECOMPRESSOR_DECOMPRESS_METHODDEF{NULL}
};

PyDoc_STRVAR(
    Decompressor_check_doc, "ID of the integrity check used by the input stream."
);

PyDoc_STRVAR(
    Decompressor_eof_doc, "True if the end-of-stream marker has been reached."
);
...
PyDoc_STRVAR(
    Decompressor_unused_data_doc, "Data found after the end of the compressed stream."
);

static PyMemberDef Decompressor_members[] = {
    {"check",
     Py_T_INT,
     offsetof(Decompressor, check),
     Py_READONLY,
     Decompressor_check_doc},
    {"eof", Py_T_BOOL, offsetof(Decompressor, eof), Py_READONLY, Decompressor_eof_doc},
    {"needs_input",
     Py_T_BOOL,
     offsetof(Decompressor, needs_input),
     Py_READONLY,
     Decompressor_needs_input_doc},
    {"unused_data",
     Py_T_OBJECT_EX,
     offsetof(Decompressor, unused_data),
     Py_READONLY,
...
PyDoc_STRVAR(
    ...
    "--\n"
    "\n"
    "Return a bytes object encoding the options (properties) of the filter specified "
    "by *filter* (a dict).\n"
    "\n"
...
#define _LZMA__ENCODE_FILTER_PROPERTIES_METHODDEF  \
    {"_encode_filter_properties",                  \
     (PyCFunction)_lzma__encode_filter_properties, \
     METH_O,                                       \
     _lzma__encode_filter_properties__doc__},
...
_lzma__encode_filter_properties_impl(PyObject *module, lzma_filter filter) {
    ...
    lzret = lzma_properties_encode(&filter, (uint8_t *)PyBytes_AS_STRING(result));
    if (catch_lzma_error(state, lzret)) {
        goto error;
    }
...
Return a bytes object encoding the options (properties) of the filter specified by
*filter* (a dict).
...
_lzma__decode_filter_properties_impl(
    PyObject *module, lzma_vli filter_id, Py_buffer *encoded_props
)
/*[clinic end generated code: output=714fd2ef565d5c60 input=246410800782160c]*/
{
    ...
    lzret =
        lzma_properties_decode(&filter, NULL, encoded_props->buf, encoded_props->len);
    if (catch_lzma_error(state, lzret)) {
        return NULL;
    }
...
#define ADD_INT_MACRO(module, macro)                   \
    ...
    state->lzma_compressor_type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module, &lzma_compressor_type_spec, NULL
    );
    ...
    state->lzma_decompressor_type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module, &lzma_decompressor_type_spec, NULL
    );
...
static PyMethodDef lzma_methods[] = {
    _LZMA_IS_CHECK_SUPPORTED_METHODDEF _LZMA__ENCODE_FILTER_PROPERTIES_METHODDEF
