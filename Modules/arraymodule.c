...
typedef struct arrayobject {
    PyObject_VAR_HEAD char *ob_item;
...
#define get_array_state_by_class(cls) (get_array_state(PyType_GetModule(cls)))
...
array_resize(arrayobject *self, Py_ssize_t newsize) {
    ...
    if (self->ob_exports > 0 && newsize != Py_SIZE(self)) {
        PyErr_SetString(
            PyExc_BufferError, "cannot resize an array that is exporting buffers"
        );
        return -1;
    }
...
b_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
    ...
    if (!PyArg_Parse(v, "h;array item must be integer", &x))
        ...
        PyErr_SetString(PyExc_OverflowError, "signed char is less than minimum");
        return -1;
    } else if (x > 127) {
        PyErr_SetString(PyExc_OverflowError, "signed char is greater than maximum");
        return -1;
    }
    if (i >= 0)
...
u_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
    if (!PyUnicode_Check(v)) {
        PyErr_Format(
            PyExc_TypeError, "array item must be a unicode character, not %T", v
        );
        return -1;
    }
...
HH_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
    ...
    if (!PyArg_Parse(v, "i;array item must be integer", &x))
        ...
        PyErr_SetString(PyExc_OverflowError, "unsigned short is less than minimum");
        return -1;
    } else if (x > USHRT_MAX) {
        PyErr_SetString(PyExc_OverflowError, "unsigned short is greater than maximum");
        return -1;
    }
    if (i >= 0)
...
II_getitem(arrayobject *ap, Py_ssize_t i) {
    return PyLong_FromUnsignedLong((unsigned long)((unsigned int *)ap->ob_item)[i]);
}
...
II_setitem(arrayobject *ap, Py_ssize_t i, PyObject *v) {
    ...
    if (x > UINT_MAX) {
        PyErr_SetString(PyExc_OverflowError, "unsigned int is greater than maximum");
        if (do_decref) {
            Py_DECREF(v);
        }
        return -1;
...
QQ_getitem(arrayobject *ap, Py_ssize_t i) {
    return PyLong_FromUnsignedLongLong(((unsigned long long *)ap->ob_item)[i]);
}
...
#define DEFINE_COMPAREITEMS(code, type)                     \
    static int code##_compareitems(                         \
        const void *lhs, const void *rhs, Py_ssize_t length \
    ) {                                                     \
...
ins1(arrayobject *self, Py_ssize_t where, PyObject *v) {
    ...
    if (where != n)
        memmove(
            items + (where + 1) * self->ob_descr->itemsize,
...
array_richcompare(PyObject *v, PyObject *w, int op) {
    ...
    if (va->ob_descr == wa->ob_descr && va->ob_descr->compareitems != NULL) {
        ...
        int result =
            va->ob_descr->compareitems(va->ob_item, wa->ob_item, common_length);
        if (result == 0)
            goto compare_sizes;
...
array_slice(arrayobject *a, Py_ssize_t ilow, Py_ssize_t ihigh) {
    ...
    if (ihigh > ilow) {
        memcpy(
            np->ob_item,
            a->ob_item + ilow * a->ob_descr->itemsize,
...
#define b ((arrayobject *)bb)
    ...
    if (Py_SIZE(b) > 0) {
        memcpy(
            np->ob_item + Py_SIZE(a) * a->ob_descr->itemsize,
            b->ob_item,
            Py_SIZE(b) * b->ob_descr->itemsize
        );
    }
    return (PyObject *)np;
#undef b
...
array_repeat(arrayobject *a, Py_ssize_t n) {
    ...
    arrayobject *np =
        (arrayobject *)newarrayobject(state->ArrayType, size, a->ob_descr);
    if (np == NULL)
        return NULL;
    if (size == 0)
        return (PyObject *)np;
...
array_del_slice(arrayobject *a, Py_ssize_t ilow, Py_ssize_t ihigh) {
    ...
    if (d > 0) { /* Delete d items */
        memmove(
            item + (ihigh - d) * a->ob_descr->itemsize,
...
#define b ((arrayobject *)bb)
    if (self->ob_descr != b->ob_descr) {
        PyErr_SetString(PyExc_TypeError, "can only extend with array of same kind");
        return -1;
    }
    ...
    if (bbsize > 0) {
        memcpy(
            self->ob_item + oldsize * self->ob_descr->itemsize,
            b->ob_item,
            bbsize * b->ob_descr->itemsize
        );
    }
...
static PyObject *
array_array_index_impl(
    arrayobject *self, PyObject *v, Py_ssize_t start, Py_ssize_t stop
)
...
items of the buffer used to hold array's contents.
...
/*[clinic end generated code: output=5f8236cbdf0d90b5 input=6a85591b950a0186]*/
{
    ...
    switch (self->ob_descr->itemsize) {
        ...
        default:
            PyErr_SetString(
                PyExc_RuntimeError, "don't know how to byteswap this array type"
            );
            return NULL;
    }
    Py_RETURN_NONE;
...
/*[clinic end generated code: output=c04868b36f6f4089 input=cd904f01b27d966a]*/
{
    ...
    if (Py_SIZE(self) > 1) {
        for (p = self->ob_item, q = self->ob_item + (Py_SIZE(self) - 1) * itemsize;
             p < q;
...
array_array_fromfile_impl(
    arrayobject *self, PyTypeObject *cls, PyObject *f, Py_ssize_t n
)
...
/*[clinic end generated code: output=26411c2d228a3e3f input=be2605a96c49680f]*/
{
    ...
    if (n > 0) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if ((*self->ob_descr->setitem)(self, Py_SIZE(self) - n + i, v) != 0) {
                array_resize(self, old_size);
                return NULL;
            }
            if (n != PyList_GET_SIZE(list)) {
                PyErr_SetString(
                    PyExc_RuntimeError, "list changed size during iteration"
                );
                array_resize(self, old_size);
                return NULL;
            }
...
frombytes(arrayobject *self, Py_buffer *buffer) {
    ...
    if (n > 0) {
        ...
        memcpy(self->ob_item + old_size * itemsize, buffer->buf, n * itemsize);
    }
    PyBuffer_Release(buffer);
    Py_RETURN_NONE;
}
...
Appends items from the string, interpreting it as an array of machine values, as if it
had been read from a file using the fromfile() method. [clinic start generated code]*/
...
/*[clinic end generated code: output=24359f5e001a7f2b input=025db1fdade7a4ce]*/
{
    ...
    if (typecode == 'u') {
        ...
        // must not fail
        Py_UCS4 *u = PyUnicode_AsUCS4(
            ustr, ((Py_UCS4 *)self->ob_item) + old_size, ustr_length, 0
        );
        assert(u != NULL);
...
/*[clinic end generated code: output=08e442378336e1ef input=127242eebe70b66d]*/
{
    ...
    if (typecode == 'u') {
        ...
        return PyUnicode_DecodeUTF32(
            (const char *)self->ob_item, Py_SIZE(self) * 4, NULL, &byteorder
        );
    }
}
...
/*[clinic end generated code: output=e05263141ba28365 input=2464dc8f4c7736b5]*/
{
    ...
    if (descr->typecode == '\0') {
        PyErr_SetString(PyExc_ValueError, "second argument must be a valid type code");
        return NULL;
    }
    if (mformat_code < MACHINE_FORMAT_CODE_MIN ||
        mformat_code > MACHINE_FORMAT_CODE_MAX) {
        PyErr_SetString(
            PyExc_ValueError, "third argument must be a valid machine format code."
        );
        return NULL;
    }
    ...
    if (Py_SIZE(items) % mformat_descriptors[mformat_code].size != 0) {
        PyErr_SetString(PyExc_ValueError, "string length not a multiple of item size");
        return NULL;
    }
    switch (mformat_code) {
        ...
        case IEEE_754_FLOAT_BE: {
            ...
            for (i = 0; i < itemcount; i++) {
                PyObject *pyfloat =
                    PyFloat_FromDouble(PyFloat_Unpack4(&memstr[i * 4], le));
                if (pyfloat == NULL) {
        ...
        case IEEE_754_DOUBLE_BE: {
            ...
            for (i = 0; i < itemcount; i++) {
                PyObject *pyfloat =
                    PyFloat_FromDouble(PyFloat_Unpack8(&memstr[i * 8], le));
                if (pyfloat == NULL) {
                    Py_DECREF(converted_items);
                    return NULL;
                }
        ...
        case UTF16_BE: {
            ...
            converted_items = PyUnicode_DecodeUTF16(
                PyBytes_AS_STRING(items), Py_SIZE(items), "strict", &byteorder
            );
            if (converted_items == NULL)
                return NULL;
            break;
        ...
        case SIGNED_INT64_BE: {
            Py_ssize_t i;
            const struct mformatdescr mf_descr = mformat_descriptors[mformat_code];
            Py_ssize_t itemcount = Py_SIZE(items) / mf_descr.size;
            const unsigned char *memstr = (unsigned char *)PyBytes_AS_STRING(items);
            const struct arraydescr *descr;
...
static PyObject *
array_array___reduce_ex___impl(arrayobject *self, PyTypeObject *cls, PyObject *value)
/*[clinic end generated code: output=4958ee5d79452ad5 input=19968cf0f91d3eea]*/
{
    ...
    if (state->array_reconstructor == NULL) {
        state->array_reconstructor =
            _PyImport_GetModuleAttrString("array", "_array_reconstructor");
        if (state->array_reconstructor == NULL) {
            return NULL;
        }
    }
    ...
    if (mformat_code == UNKNOWN_FORMAT || protocol < 3) {
        ...
        result = Py_BuildValue("O(CO)O", Py_TYPE(self), typecode, list, dict);
        Py_DECREF(list);
        Py_DECREF(dict);
        return result;
    }
...
static PyGetSetDef array_getsets[] = {
    {"typecode",
     (getter)array_get_typecode,
     NULL,
     "the typecode character used to create the array"},
    {"itemsize",
     (getter)array_get_itemsize,
     NULL,
...
static PyMethodDef array_methods[] = {
    ARRAY_ARRAY_APPEND_METHODDEF ARRAY_ARRAY_BUFFER_INFO_METHODDEF
        ARRAY_ARRAY_BYTESWAP_METHODDEF ARRAY_ARRAY_CLEAR_METHODDEF
            ARRAY_ARRAY___COPY___METHODDEF ARRAY_ARRAY_COUNT_METHODDEF
                ARRAY_ARRAY___DEEPCOPY___METHODDEF ARRAY_ARRAY_EXTEND_METHODDEF
                    ARRAY_ARRAY_FROMFILE_METHODDEF ARRAY_ARRAY_FROMLIST_METHODDEF
                        ARRAY_ARRAY_FROMBYTES_METHODDEF ARRAY_ARRAY_FROMUNICODE_METHODDEF
                            ARRAY_ARRAY_INDEX_METHODDEF ARRAY_ARRAY_INSERT_METHODDEF
                                ARRAY_ARRAY_POP_METHODDEF ARRAY_ARRAY___REDUCE_EX___METHODDEF
                                    ARRAY_ARRAY_REMOVE_METHODDEF ARRAY_ARRAY_REVERSE_METHODDEF
...
array_repr(arrayobject *a) {
    ...
    if (len == 0) {
        return PyUnicode_FromFormat(
            "%s('%c')", _PyType_Name(Py_TYPE(a)), (int)typecode
        );
    }
    if (typecode == 'u' || typecode == 'w') {
    ...
    s = PyUnicode_FromFormat(
        "%s('%c', %R)", _PyType_Name(Py_TYPE(a)), (int)typecode, v
    );
    Py_DECREF(v);
    return s;
}
...
array_subscr(arrayobject *self, PyObject *item) {
    ...
    if (PyIndex_Check(item)) {
        ...
        if (slicelength <= 0) {
            ...
            PyObject *result =
                newarrayobject(state->ArrayType, slicelength, self->ob_descr);
            if (result == NULL)
                return NULL;
            memcpy(
                ((arrayobject *)result)->ob_item,
            ...
            for (cur = start, i = 0; i < slicelength; cur += step, i++) {
                memcpy(
                    ar->ob_item + i * itemsize, self->ob_item + cur * itemsize, itemsize
                );
            }
        ...
        PyErr_SetString(PyExc_TypeError, "array indices must be integers");
        return NULL;
    }
}
...
array_ass_subscr(arrayobject *self, PyObject *item, PyObject *value) {
    ...
    if (step == 1) {
        if (slicelength > needed) {
            memmove(
                self->ob_item + (start + needed) * itemsize,
                self->ob_item + stop * itemsize,
                (Py_SIZE(self) - stop) * itemsize
            );
            if (array_resize(self, Py_SIZE(self) + needed - slicelength) < 0)
                return -1;
        ...
        if (needed > 0)
            memcpy(self->ob_item + start * itemsize, other->ob_item, needed * itemsize);
        return 0;
    } else if (needed == 0) {
        /* Delete slice */
        size_t cur;
        ...
        if (cur < (size_t)Py_SIZE(self)) {
            memmove(
                self->ob_item + (cur - slicelength) * itemsize,
        ...
        for (cur = start, i = 0; i < slicelength; cur += step, i++) {
            memcpy(
                self->ob_item + cur * itemsize, other->ob_item + i * itemsize, itemsize
            );
        }
        return 0;
    }
...
array_buffer_getbuf(arrayobject *self, Py_buffer *view, int flags) {
    if (view == NULL) {
        PyErr_SetString(
            PyExc_BufferError, "array_buffer_getbuf: view==NULL argument is obsolete"
        );
        return -1;
    }
...
array_new(PyTypeObject *type, PyObject *args, PyObject *kwds) {
    ...
    if ((type == state->ArrayType || type->tp_init == state->ArrayType->tp_init) &&
    ...
    if (!(initial == NULL || PyList_Check(initial) || PyByteArray_Check(initial) ||
            ...
            if (len > 0 && !array_Check(initial, state)) {
                ...
                for (i = 0; i < len; i++) {
                    ...
                    Py_DECREF(v);
                       ...
                       (PyByteArray_Check(initial) || PyBytes_Check(initial))) {
                PyObject *v;
                v = array_array_frombytes((arrayobject *)a, initial);
                if (v == NULL) {
                    Py_DECREF(a);
                    return NULL;
                }
...
static struct PyMemberDef array_members[] = {
    {"__weaklistoffset__",
     Py_T_PYSSIZET,
     offsetof(arrayobject, weakreflist),
     Py_READONLY},
    {NULL},
};
...
/*[clinic input]
class array.arrayiterator "arrayiterobject *"
"find_array_state_by_type(type)->ArrayIterType" [clinic start generated code]*/
...
static PyMethodDef arrayiter_methods[] = {
    ARRAY_ARRAYITERATOR___REDUCE___METHODDEF ARRAY_ARRAYITERATOR___SETSTATE___METHODDEF{
        NULL, NULL
    } /* sentinel */
};
...
static PyMethodDef a_methods[] = {
    ARRAY__ARRAY_RECONSTRUCTOR_METHODDEF{NULL, NULL, 0, NULL} /* Sentinel */
};
...
array_modexec(PyObject *m) {
    ...
    if (PyModule_AddObjectRef(m, "ArrayType", (PyObject *)state->ArrayType) < 0) {
        return -1;
    }

    PyObject *mutablesequence =
        _PyImport_GetModuleAttrString("collections.abc", "MutableSequence");
    if (!mutablesequence) {
        Py_DECREF((PyObject *)state->ArrayType);
        return -1;
    }
    PyObject *res = PyObject_CallMethod(
        mutablesequence, "register", "O", (PyObject *)state->ArrayType
    );
