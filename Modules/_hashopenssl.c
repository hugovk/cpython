...
#define PY_EVP_MD_up_ref(md) \
    do {                     \
    } while (0)
#define PY_EVP_MD_free(md) \
    do {                   \
    } while (0)
#endif
...
py_hashentry_table_new(void) {
    ...
    for (const py_hashentry_t *h = py_hashes; h->py_name != NULL; h++) {
        ...
        if (h->py_alias != NULL) {
            if (_Py_hashtable_set(ht, (const void *)entry->py_alias, (void *)entry) <
                0) {
                PyMem_Free(entry);
                goto error;
            }
...
typedef struct {
    PyObject_HEAD EVP_MD_CTX *ctx; /* OpenSSL message digest context */
...
typedef struct {
    PyObject_HEAD HMAC_CTX *ctx; /* OpenSSL hmac context */
    // Prevents undefined behavior via multiple threads entering the C API.
    bool use_mutex;
...
*)PyModule_GetState(module))->EVPtype" class _hashlib.HASHXOF "EVPobject *"
"((_hashlibstate *)PyModule_GetState(module))->EVPXOFtype" class _hashlib.HMAC
"HMACobject *" "((_hashlibstate *)PyModule_GetState(module))->HMACtype" [clinic start
generated code]*/
...
py_digest_by_name(PyObject *module, const char *name, enum Py_hash_type py_ht) {
    ...
    py_hashentry_t *entry =
        (py_hashentry_t *)_Py_hashtable_get(state->hashtable, (const void *)name);
    ...
    if (digest == NULL) {
        _setException(
            state->unsupported_digestmod_error, "unsupported hash type %s", name
        );
        return NULL;
    }
    return digest;
...
/*[clinic end generated code: output=ec1d55ed2432e966 input=9b30ec848f015501]*/
{
    ...
    if (self->use_mutex) {
        Py_BEGIN_ALLOW_THREADS PyMutex_Lock(&self->mutex);
...
static PyMethodDef EVP_methods[] = {
    EVP_UPDATE_METHODDEF EVP_DIGEST_METHODDEF EVP_HEXDIGEST_METHODDEF
        EVP_COPY_METHODDEF{NULL, NULL} /* sentinel */
};
...
static PyGetSetDef EVP_getseters[] = {
    ...
    {"name", (getter)EVP_get_name, NULL, NULL, PyDoc_STR("algorithm name.")},
    {NULL} /* Sentinel */
};
...
EVP_repr(EVPobject *self) {
    ...
    repr = PyUnicode_FromFormat(
        "<%U %s object @ %p>", name_obj, Py_TYPE(self)->tp_name, self
    );
    Py_DECREF(name_obj);
    return repr;
}
...
/*[clinic end generated code: output=ef9320c23280efad input=816a6537cea3d1db]*/
{
    ...
    if (!EVP_DigestFinalXOF(
            temp_ctx, (unsigned char *)PyBytes_AS_STRING(retval), length
        )) {
        Py_DECREF(retval);
        EVP_MD_CTX_free(temp_ctx);
        _setException(PyExc_ValueError, NULL);
...
static PyMethodDef EVPXOF_methods[] = {
    EVPXOF_DIGEST_METHODDEF EVPXOF_HEXDIGEST_METHODDEF{NULL, NULL} /* sentinel */
};
...
static PyGetSetDef EVPXOF_getseters[] = {
    {"digest_size", (getter)EVPXOF_get_digest_size, NULL, NULL, NULL},
    {NULL} /* Sentinel */
};
...
static PyObject *
py_evp_fromname(
    PyObject *module, const char *digestname, PyObject *data_obj, int usedforsecurity
) {
    Py_buffer view = {0};
    PY_EVP_MD *digest = NULL;
    PyTypeObject *type;
    ...
    if (view.buf && view.len) {
        if (view.len >= HASHLIB_GIL_MINSIZE) {
            ...
            Py_BEGIN_ALLOW_THREADS result = EVP_hash(self, view.buf, view.len);
            Py_END_ALLOW_THREADS
        } else {
            result = EVP_hash(self, view.buf, view.len);
        }
...
static PyObject *
EVP_new_impl(
    PyObject *module, PyObject *name_obj, PyObject *data_obj, int usedforsecurity
)
...
static PyObject *
_hashlib_openssl_md5_impl(PyObject *module, PyObject *data_obj, int usedforsecurity)
...
static PyObject *
_hashlib_openssl_sha1_impl(PyObject *module, PyObject *data_obj, int usedforsecurity)
...
static PyObject *
_hashlib_openssl_sha224_impl(PyObject *module, PyObject *data_obj, int usedforsecurity)
...
static PyObject *
_hashlib_openssl_sha256_impl(PyObject *module, PyObject *data_obj, int usedforsecurity)
...
static PyObject *
_hashlib_openssl_sha384_impl(PyObject *module, PyObject *data_obj, int usedforsecurity)
...
static PyObject *
_hashlib_openssl_sha512_impl(PyObject *module, PyObject *data_obj, int usedforsecurity)
...
_hashlib_openssl_sha3_224_impl(
    PyObject *module, PyObject *data_obj, int usedforsecurity
)
...
Password based key derivation function 2 (PKCS #5 v2.0) with HMAC as pseudorandom
function. [clinic start generated code]*/
...
/*[clinic end generated code: output=144b76005416599b input=ed3ab0d2d28b5d5c]*/
{
    ...
    if (iterations < 1) {
        PyErr_SetString(PyExc_ValueError, "iteration value must be greater than 0.");
        goto end;
    }
    if (iterations > INT_MAX) {
        PyErr_SetString(PyExc_OverflowError, "iteration value is too great.");
        goto end;
    }
    ...
    if (dklen < 1) {
        PyErr_SetString(PyExc_ValueError, "key length must be greater than 0.");
        goto end;
    }
    if (dklen > INT_MAX) {
        /* INT_MAX is always smaller than dkLen max (2^32 - 1) * hLen */
        PyErr_SetString(PyExc_OverflowError, "key length is too great.");
        goto end;
    }
    ...
    Py_BEGIN_ALLOW_THREADS retval = PKCS5_PBKDF2_HMAC(
        (char *)password->buf,
        (int)password->len,
...
/*[clinic end generated code: output=14849e2aa2b7b46c input=48a7d63bf3f75c42]*/
{
    ...
    if (salt->buf == NULL) {
        PyErr_SetString(PyExc_TypeError, "salt is required");
        return NULL;
    }
    if (salt->len > INT_MAX) {
        PyErr_SetString(PyExc_OverflowError, "salt is too long.");
    ...
    if (n == (unsigned long)-1 && PyErr_Occurred()) {
        PyErr_SetString(PyExc_TypeError, "n is required and must be an unsigned int");
        return NULL;
    }
    if (n < 2 || n & (n - 1)) {
    ...
    if (r == (unsigned long)-1 && PyErr_Occurred()) {
        PyErr_SetString(PyExc_TypeError, "r is required and must be an unsigned int");
        return NULL;
    }

    p = PyLong_AsUnsignedLong(p_obj);
    if (p == (unsigned long)-1 && PyErr_Occurred()) {
        PyErr_SetString(PyExc_TypeError, "p is required and must be an unsigned int");
        return NULL;
    }
    ...
    if (maxmem < 0 || maxmem > INT_MAX) {
        ...
        PyErr_Format(
            PyExc_ValueError, "maxmem must be positive and smaller than %d", INT_MAX
        );
        return NULL;
    }
    ...
    if (!retval) {
        _setException(
            PyExc_ValueError, "Invalid parameter combination for n, r, p, maxmem."
        );
        return NULL;
    }
    ...
    Py_BEGIN_ALLOW_THREADS retval = EVP_PBE_scrypt(
        (const char *)password->buf,
        (size_t)password->len,
...
_hashlib_hmac_singleshot_impl(
    PyObject *module, Py_buffer *key, Py_buffer *msg, PyObject *digest
)
/*[clinic end generated code: output=82f19965d12706ac input=0a0790cc3db45c2e]*/
{
    ...
    Py_BEGIN_ALLOW_THREADS result = HMAC(
        evp,
        (const void *)key->buf,
        (int)key->len,
    ...
    Py_END_ALLOW_THREADS PY_EVP_MD_free(evp);
...
static int
_hmac_update(HMACobject *, PyObject *);
...
_hashlib_hmac_new_impl(
    PyObject *module, Py_buffer *key, PyObject *msg_obj, PyObject *digestmod
)
/*[clinic end generated code: output=c20d9e4d9ed6d219 input=5f4071dcc7f34362]*/
{
    ...
    if (digestmod == NULL) {
        PyErr_SetString(PyExc_TypeError, "Missing required parameter 'digestmod'.");
        return NULL;
    }
    ...
    r = HMAC_Init_ex(ctx, (const char *)key->buf, (int)key->len, digest, NULL /*impl*/);
    PY_EVP_MD_free(digest);
    if (r == 0) {
        _setException(PyExc_ValueError, NULL);
        goto error;
...
_hmac_repr(HMACobject *self) {
    ...
    PyObject *repr = PyUnicode_FromFormat("<%U HMAC object @ %p>", digest_name, self);
...
static PyMethodDef HMAC_methods[] = {
    _HASHLIB_HMAC_UPDATE_METHODDEF _HASHLIB_HMAC_DIGEST_METHODDEF
        _HASHLIB_HMAC_HEXDIGEST_METHODDEF _HASHLIB_HMAC_COPY_METHODDEF{
            NULL, NULL
        } /* sentinel */
};
...
#else
_openssl_hash_name_mapper(const EVP_MD *md, const char *from, const char *to, void *arg)
#endif
{
...
hashlib_md_meth_names(PyObject *module) {
    _InternalNameMapperState state = {.set = PyFrozenSet_New(NULL), .error = 0};
    if (state.set == NULL) {
        return -1;
    }
...
static int
_tscmp(
    const unsigned char *a, const unsigned char *b, Py_ssize_t len_a, Py_ssize_t len_b
) {
...
/*[clinic end generated code: output=6f1c13927480aed9 input=9c40c6e566ca12f5]*/
{
    ...
    /* fallback to buffer interface for bytes, bytearray and other */
        ...
        rc = _tscmp(
            (const unsigned char *)view_a.buf,
...
static struct PyMethodDef EVP_functions[] = {
    EVP_NEW_METHODDEF PBKDF2_HMAC_METHODDEF _HASHLIB_SCRYPT_METHODDEF
        _HASHLIB_GET_FIPS_MODE_METHODDEF _HASHLIB_COMPARE_DIGEST_METHODDEF
            _HASHLIB_HMAC_SINGLESHOT_METHODDEF _HASHLIB_HMAC_NEW_METHODDEF
                _HASHLIB_OPENSSL_MD5_METHODDEF _HASHLIB_OPENSSL_SHA1_METHODDEF
                    _HASHLIB_OPENSSL_SHA224_METHODDEF _HASHLIB_OPENSSL_SHA256_METHODDEF
...
hashlib_init_constructors(PyObject *module) {
    ...
    return PyModule_Add(module, "_constructors", PyDictProxy_New(state->constructs));
}
...
hashlib_exception(PyObject *module) {
    ...
    if (PyModule_AddObjectRef(
            module, "UnsupportedDigestmodError", state->unsupported_digestmod_error
        ) < 0) {
        return -1;
    }
    return 0;
