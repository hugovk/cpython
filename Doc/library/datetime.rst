...
and then +4:30 UTC thereafter::
   ...
   class KabulTz(dt.tzinfo):
       ...
       def utcoffset(self, my_dt):
           ...
           elif (1945, 1, 1, 0, 0) <= my_dt.timetuple()[:5] < (1945, 1, 1, 0, 30):
               # An ambiguous ("imaginary") half-hour range representing
               # a 'fold' in time due to the shift from +4 to +4:30.
               # If my_dt falls in the imaginary range, use fold to decide how
               # to resolve. See PEP 495.
               return dt.timedelta(hours=4, minutes=(30 if my_dt.fold else 0))
           else:
               return dt.timedelta(hours=4, minutes=30)

       def fromutc(self, my_dt):
           # Follow same validations as in datetime.tzinfo
           if not isinstance(my_dt, dt.datetime):
               raise TypeError("fromutc() requires a datetime argument")
           if my_dt.tzinfo is not self:
               raise ValueError("my_dt.tzinfo is not self")

           # A custom implementation is required for fromutc as
           # the input to this function is a datetime with utc values
           # but with a tzinfo set to self.
           # See datetime.astimezone or fromtimestamp.
           if my_dt.replace(tzinfo=dt.timezone.utc) >= self.UTC_MOVE_DATE:
               return my_dt + dt.timedelta(hours=4, minutes=30)
           else:
               return my_dt + dt.timedelta(hours=4)

       def dst(self, my_dt):
           # Kabul does not observe daylight saving time.
           return dt.timedelta(0)

       def tzname(self, my_dt):
           if my_dt >= self.UTC_MOVE_DATE:
               return "+04:30"
           return "+04"
...
.. method:: time.isoformat(timespec='auto')
   ...
   Example::
      ...
      '12:34'
      >>> my_datetime = dt.time(hour=12, minute=34, second=56, microsecond=0)
      >>> my_datetime.isoformat(timespec='microseconds')
      '12:34:56.000000'
      >>> my_datetime.isoformat(timespec='auto')
      '12:34:56'
...
.. method:: tzinfo.dst(dt)
   ...
   or::
      ...
      def dst(self, my_dt):
          ...
          # transition times based on the input my_dt.year, and expressed
          # in standard local time.

          if dston <= my_dt.replace(tzinfo=None) < dstoff:
              return dt.timedelta(hours=1)
          else:
              return dt.timedelta(0)
...
.. method:: tzinfo.fromutc(dt)
   ...
   like::
      ...
      def fromutc(self, my_dt):
          # raise ValueError error if my_dt.tzinfo is not self
          dtoff = my_dt.utcoffset()
          dtdst = my_dt.dst()
          # raise ValueError if dtoff is None or dtdst is None
          delta = dtoff - dtdst  # this is self's standard offset
          if delta:
              my_dt += delta   # convert to standard local time
              dtdst = my_dt.dst()
