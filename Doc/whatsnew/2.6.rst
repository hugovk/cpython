...
and :meth:`~multiprocessing.pool.Pool.map` or :meth:`~multiprocessing.pool.Pool.map_async` to add a number of
...
:class:`~multiprocessing.Manager`'s methods also include :meth:`Lock`, :meth:`RLock`,
...
* :class:`~io.RawIOBase` defines raw I/O operations: :meth:`~io.RawIOBase.read`,
  ...
  :meth:`~io.RawIOBase.write`, :meth:`seek`, :meth:`tell`, :meth:`truncate`,
  and :meth:`close`.
...
* :class:`~io.TextIOBase`: Provides functions for reading and writing
  ...
  the :meth:`~io.TextIOBase.readline` method and supports iteration upon
  objects.
...
different ABCs that represent these distinctions.  :class:`~collections.abc.Iterable`
indicates that a class defines :meth:`~object.__iter__`, and
:class:`~collections.abc.Container` means the class defines a :meth:`~object.__contains__`
method and therefore supports ``x in y`` expressions.  The basic
dictionary interface of getting items, setting items, and
:meth:`~dict.keys`, :meth:`~dict.values`, and :meth:`~dict.items`, is defined by the
...
:attr:`~numbers.Rational.numerator` and :attr:`~numbers.Rational.denominator` properties, and can be
...
:class:`~fractions.Fraction` instead of :class:`~numbers.Rational` to avoid
...
* Many floating-point features were added.  The :func:`float` function
  ...
  binary.  Floats have a :meth:`~float.hex` method that returns a string
  representation, and the :meth:`float.fromhex` method converts a string
  back into a number::
...
* Classes that inherit a :meth:`~object.__hash__` method from a parent class
  ...
  :class:`~collections.abc.Hashable` ABC.
...
* A new data type in the :mod:`collections` module: ``namedtuple(typename, fieldnames)``
  ...
  the :meth:`Decimal.as_tuple() <decimal.Decimal.as_tuple>` method now returns a named tuple with
...
* The :mod:`datetime` module's :meth:`~datetime.strftime` methods now support a
...
* The :mod:`decimal` module was updated to version 1.66 of
  ...
  The :meth:`~decimal.Decimal.as_tuple` method of :class:`Decimal` objects now returns a
...
* An optional ``timeout`` parameter, specifying a timeout measured in
  ...
  :class:`HTTPSConnection <http.client.HTTPConnection>` class constructors.  (Added by Facundo
  Batista.)
...
* Several new functions were added to the :mod:`math` module:
  ...
  * :func:`~math.trunc` rounds a number toward zero, returning the closest
    :class:`~math.Integral` that's between the function's argument and zero.
...
* :class:`~mmap.mmap` objects now have a :meth:`~mmap.mmap.rfind` method that searches for a
...
* The :mod:`rlcompleter` module's :meth:`Completer.complete() <rlcompleter.Completer.complete>` method
...
* The :mod:`sched` module's :class:`~sched.scheduler` instances now
  have a read-only :attr:`~sched.scheduler.queue` attribute that returns the
...
* The :mod:`smtplib` module now supports SMTP over SSL thanks to the
  ...
  :meth:`SMTP.starttls() <smtplib.SMTP.starttls>` now complies with :rfc:`3207` and forgets any
...
* The :class:`~subprocess.Popen` objects provided by the :mod:`subprocess` module
  ...
  On Windows, :meth:`~subprocess.Popen.send_signal` only supports the :const:`~signal.SIGTERM`
  signal, and all these methods are aliases for the Win32 API function
  :c:func:`TerminateProcess`.
  (Contributed by Christian Heimes.)

* A new variable in the :mod:`sys` module, :attr:`~sys.float_info`, is an
...
* The :mod:`tarfile` module now supports POSIX.1-2001 (pax) tarfiles in
  ...
  The :meth:`TarFile.add <tarfile.TarFile.add>` method now accepts an ``exclude`` argument that's
...
* The :class:`tempfile.NamedTemporaryFile` class usually deletes
  ...
  The :class:`~tempfile.NamedTemporaryFile` and :class:`~tempfile.SpooledTemporaryFile` classes
...
* The :mod:`threading` module API is being changed to use properties
  ...
  :meth:`!activeCount` method is renamed to :meth:`~threading.active_count`.  Both
...
* The :mod:`turtle` module for turtle graphics was greatly enhanced by
  ...
  * Control over turtle movement using the new :meth:`~turtle.delay`,
    :meth:`~turtle.tracer`, and :meth:`~turtle.speed` methods.
  * The ability to set new shapes for the turtle, and to
    define a new coordinate system.
  * Turtles now have an :meth:`~turtle.undo` method that can roll back actions.
...
* The :mod:`warnings` module's :func:`~warnings.formatwarning` and :func:`~warnings.showwarning`
...
* The :mod:`zipfile` module's :class:`~zipfile.ZipFile` class now has
  :meth:`~zipfile.ZipFile.extract` and :meth:`~zipfile.ZipFile.extractall` methods that will unpack
  ...
  The :meth:`~zipfile.ZipFile.open`, :meth:`~zipfile.ZipFile.read` and :meth:`~zipfile.ZipFile.extract` methods can now
  take either a filename or a :class:`~zipfile.ZipFile` object.  This is useful when an
...
and then call the module-level methods :meth:`~ctypes.set_errno` and
:meth:`~ctypes.get_errno` to set and retrieve the error value.
...
and then call the module-level methods :meth:`~ctypes.set_last_error`
and :meth:`~ctypes.get_last_error`.
