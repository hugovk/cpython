...
static void
PyThread__init_thread(void); /* Forward */
...
PyThread_ParseTimeoutArg(PyObject *arg, int blocking, PY_TIMEOUT_T *timeout_p) {
    ...
    if (_PyTime_AsMicroseconds(timeout, _PyTime_ROUND_TIMEOUT) > PY_TIMEOUT_MAX) {
...
PyThread_acquire_lock_timed_with_retries(
    PyThread_type_lock lock, PY_TIMEOUT_T timeout
) {
    ...
    do {
        ...
        if (r == PY_LOCK_FAILURE && microseconds != 0) {
            Py_BEGIN_ALLOW_THREADS r =
                PyThread_acquire_lock_timed(lock, microseconds, 1);
