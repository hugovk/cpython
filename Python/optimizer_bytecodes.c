...
#define sym_set_type_version(SYM, VERSION) \
...
dummy_func(void) {
    ...
    op(_LOAD_FAST, (--value)) { value = GETLOCAL(oparg); }
    ...
    op(_STORE_FAST, (value--)) { GETLOCAL(oparg) = value; }

    op(_PUSH_NULL, (--res)) { res = sym_new_null(ctx); }
    ...
    op(_GUARD_TYPE_VERSION, (type_version / 2, owner-- owner)) {
        ...
        if (sym_matches_type_version(owner, type_version)) {
            ...
            if (type) {
                ...
                // if it wasn't this means that the type version was previously set to
                // something else and we set the owner to bottom, so we don't need to
                // add a watcher because we must have already added one earlier.
    ...
    op(_BINARY_OP, (left, right-- res)) {
        ...
        if (ltype != NULL && (ltype == &PyLong_Type || ltype == &PyFloat_Type) &&
            ...
            if (oparg != NB_TRUE_DIVIDE && oparg != NB_INPLACE_TRUE_DIVIDE &&
                ...
                /* If both inputs are ints and the op is not division the result is an
                 * int */
    ...
    op(_BINARY_OP_ADD_INT, (left, right-- res)) {
        if (sym_is_const(left) && sym_is_const(right) &&
            ...
            PyObject *temp = _PyLong_Add(
                (PyLongObject *)sym_get_const(left),
    ...
    op(_BINARY_OP_SUBTRACT_INT, (left, right-- res)) {
        if (sym_is_const(left) && sym_is_const(right) &&
            ...
            assert(PyLong_CheckExact(sym_get_const(left)));
            assert(PyLong_CheckExact(sym_get_const(right)));
            PyObject *temp = _PyLong_Subtract(
                (PyLongObject *)sym_get_const(left),
                (PyLongObject *)sym_get_const(right)
            );
            if (temp == NULL) {
    ...
    op(_BINARY_OP_ADD_UNICODE, (left, right-- res)) {
        if (sym_is_const(left) && sym_is_const(right) &&
            ...
            sym_matches_type(right, &PyUnicode_Type)) {
            PyObject *temp =
                PyUnicode_Concat(sym_get_const(left), sym_get_const(right));
            if (temp == NULL) {
                goto error;
            }
            res = sym_new_const(ctx, temp);
    ...
    op(_BINARY_SUBSCR_INIT_CALL,
       (container, sub-- new_frame : _Py_UOpsAbstractFrame *)) {
        (void)container;
        (void)sub;
        new_frame = NULL;
        ctx->done = true;
    ...
    op(_LOAD_CONST, (--value)) {
        ...
        int opcode =
            _Py_IsImmortal(val) ? _LOAD_CONST_INLINE_BORROW : _LOAD_CONST_INLINE;
        REPLACE_OP(this_instr, opcode, 0, (uintptr_t)val);
        value = sym_new_const(ctx, val);
    }

    op(_LOAD_CONST_INLINE, (ptr / 4 --value)) { value = sym_new_const(ctx, ptr); }
    ...
    op(_SWAP,
       (bottom_in, unused[oparg - 2], top_in-- top_out, unused[oparg - 2], bottom_out)
    ) {
    ...
    op(_LOAD_ATTR_MODULE, (index / 1, owner-- attr, null if (oparg & 1))) {
        ...
        if (this_instr[-1].opcode == _NOP) {
            // Preceding _CHECK_ATTR_MODULE was removed: mod is const and dict is
            // watched.
    ...
    op(_LOAD_ATTR_PROPERTY_FRAME,
       (fget / 4, owner-- new_frame : _Py_UOpsAbstractFrame *)) {
        (void)fget;
        (void)owner;
        new_frame = NULL;
        ctx->done = true;
    ...
    op(_INIT_CALL_BOUND_METHOD_EXACT_ARGS,
       (callable, unused, unused[oparg]-- func, self, unused[oparg])) {
        (void)callable;
    ...
    op(_CHECK_FUNCTION_EXACT_ARGS,
       (callable, self_or_null, unused[oparg]-- callable, self_or_null, unused[oparg])
    ) {
    ...
    op(_CHECK_CALL_BOUND_METHOD_EXACT_ARGS,
       (callable, null, unused[oparg]-- callable, null, unused[oparg])) {
        sym_set_null(null);
        sym_set_type(callable, &PyMethod_Type);
    }

    op(_INIT_CALL_PY_EXACT_ARGS,
       (callable, self_or_null, args[oparg]-- new_frame : _Py_UOpsAbstractFrame *)) {
        int argcount = oparg;
        (void)callable;
    ...
    op(_MAYBE_EXPAND_METHOD,
       (callable, self_or_null, args[oparg]-- func, maybe_self, args[oparg])) {
        (void)callable;
        (void)self_or_null;
        (void)args;
        func = sym_new_not_null(ctx);
    ...
    op(_PY_FRAME_KW,
       (callable,
        self_or_null,
        args[oparg],
        kwnames-- new_frame : _Py_UOpsAbstractFrame *)) {
        (void)callable;
        (void)self_or_null;
        (void)args;
        (void)kwnames;
    ...
    op(_CHECK_AND_ALLOCATE_OBJECT,
       (type_version / 2, callable, null, args[oparg]-- self, init, args[oparg])) {
        (void)type_version;
        (void)callable;
        (void)null;
        (void)args;
    ...
    op(_CREATE_INIT_FRAME,
       (self, init, args[oparg]-- init_frame : _Py_UOpsAbstractFrame *)) {
        (void)self;
        (void)init;
        (void)args;
        init_frame = NULL;
    ...
    op(_YIELD_VALUE, (unused-- res)) { res = sym_new_unknown(ctx); }
