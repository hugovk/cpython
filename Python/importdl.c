...
_Py_ext_module_loader_info_init(
    ...
    if (filename != NULL) {
        if (!PyUnicode_Check(filename)) {
            PyErr_SetString(PyExc_TypeError, "module filename must be a string");
            _Py_ext_module_loader_info_clear(&info);
            return -1;
        }
...
_Py_ext_module_loader_info_init_for_builtin(
    struct _Py_ext_module_loader_info *info, PyObject *name
) {
...
_Py_ext_module_loader_info_init_from_spec(
    struct _Py_ext_module_loader_info *p_info, PyObject *spec
) {
...
_Py_ext_module_loader_result_apply_error(
    ...
    switch (err.kind) {
        ...
        default:
            ...
            assert(0);
            PyErr_Format(
                PyExc_SystemError, "loading %s failed due to init function", name
            );
            return;
    }
...
_PyImport_GetModInitFunc(struct _Py_ext_module_loader_info *info, FILE *fp) {
...
#else
    {
        ...
        exportfunc =
            _PyImport_FindSharedFuncptr(info->hook_prefix, name_buf, path_buf, fp);
    }
#endif
