...
case _CHECK_PERIODIC_IF_NOT_YIELD_FROM: {
    ...
    /* _QUICKEN_RESUME is not a viable micro-op for tier 2 because it uses the
     * 'this_instr' variable */
...
case _RESUME_CHECK: {
    ...
    /* _MONITOR_RESUME is not a viable micro-op for tier 2 because it uses the
     * 'this_instr' variable */
...
case _UNARY_NOT: {
    ...
    res = PyStackRef_Is(value, PyStackRef_False) ? PyStackRef_True : PyStackRef_False;
    stack_pointer[-1] = res;
    break;
}
...
case _BINARY_SUBSCR_INIT_CALL: {
    ...
    PyObject *getitem = ht->_spec_cache.getitem;
    new_frame =
        _PyFrame_PushUnchecked(tstate, PyStackRef_FromPyObjectNew(getitem), 2, frame);
...
case _LIST_APPEND: {
    ...
    int err = _PyList_AppendTakeRef(
        (PyListObject *)PyStackRef_AsPyObjectBorrow(list), PyStackRef_AsPyObjectSteal(v)
    );
    if (err < 0)
        JUMP_TO_ERROR();
    stack_pointer += -1;
...
case _SET_ADD: {
    ...
    _PyFrame_SetStackPointer(frame, stack_pointer);
    int err =
        PySet_Add(PyStackRef_AsPyObjectBorrow(set), PyStackRef_AsPyObjectBorrow(v));
    stack_pointer = _PyFrame_GetStackPointer(frame);
    PyStackRef_CLOSE(v);
    if (err)
        JUMP_TO_ERROR();
...
case _GET_AWAITABLE: {
    ...
    /* _SEND is not a viable micro-op for tier 2 because it uses the 'this_instr'
     * variable */
...
case _UNPACK_EX: {
    ...
    _PyFrame_SetStackPointer(frame, stack_pointer);
    int res =
        _PyEval_UnpackIterableStackRef(tstate, seq, oparg & 0xFF, oparg >> 8, top);
    stack_pointer = _PyFrame_GetStackPointer(frame);
    PyStackRef_CLOSE(seq);
    if (res == 0)
        JUMP_TO_ERROR();
...
case _LOAD_LOCALS: {
    ...
    /* _LOAD_FROM_DICT_OR_GLOBALS is not a viable micro-op for tier 2 because it has
     * both popping and not-popping errors */
...
case _SETUP_ANNOTATIONS: {
    ...
    if (LOCALS() == NULL) {
        _PyFrame_SetStackPointer(frame, stack_pointer);
        _PyErr_Format(
            tstate, PyExc_SystemError, "no locals found when setting up annotations"
        );
        stack_pointer = _PyFrame_GetStackPointer(frame);
        if (true)
            JUMP_TO_ERROR();
...
case _MAP_ADD: {
    ...
    /* _INSTRUMENTED_LOAD_SUPER_ATTR is not a viable micro-op for tier 2 because it is
     * instrumented */
...
case _LOAD_ATTR_PROPERTY_FRAME: {
    ...
    if ((code->co_flags & (CO_VARKEYWORDS | CO_VARARGS | CO_OPTIMIZED)) !=
        CO_OPTIMIZED) {
        UOP_STAT_INC(uopcode, miss);
        JUMP_TO_JUMP_TARGET();
    }
    ...
    STAT_INC(LOAD_ATTR, hit);
    new_frame =
        _PyFrame_PushUnchecked(tstate, PyStackRef_FromPyObjectNew(fget), 1, frame);
    new_frame->localsplus[0] = owner;
    stack_pointer[-1].bits = (uintptr_t)new_frame;
    break;
}

    /* _LOAD_ATTR_GETATTRIBUTE_OVERRIDDEN is not a viable micro-op for tier 2 because it
     * uses the 'this_instr' variable */
...
case _STORE_ATTR_WITH_HINT: {
    ...
    // old_value should be DECREFed after GC track checking is done, if not, it could
    // raise a segmentation fault, when dict only holds the strong reference to value in
    // ep->me_value.
...
case _COMPARE_OP_STR: {
    ...
    assert(eq == 0 || eq == 1);
    assert(
        (oparg & 0xf) == COMPARISON_NOT_EQUALS || (oparg & 0xf) == COMPARISON_EQUALS
    );
...
case _IS_OP: {
    ...
    int res =
        Py_Is(PyStackRef_AsPyObjectBorrow(left), PyStackRef_AsPyObjectBorrow(right)) ^
        oparg;
#else
...
case _CHECK_EG_MATCH: {
    ...
    _PyFrame_SetStackPointer(frame, stack_pointer);
    int res = _PyEval_ExceptionGroupMatch(exc_value, match_type, &match_o, &rest_o);
    stack_pointer = _PyFrame_GetStackPointer(frame);
    PyStackRef_CLOSE(exc_value_st);
    PyStackRef_CLOSE(match_type_st);
    if (res < 0)
...
case _MATCH_KEYS: {
    ...
    PyObject *values_or_none_o = _PyEval_MatchKeys(
        tstate, PyStackRef_AsPyObjectBorrow(subject), PyStackRef_AsPyObjectBorrow(keys)
    );
    stack_pointer = _PyFrame_GetStackPointer(frame);
...
case _FOR_ITER_TIER_TWO: {
    ...
    /* _INSTRUMENTED_FOR_ITER is not a viable micro-op for tier 2 because it is
     * instrumented */
...
case _WITH_EXCEPT_START: {
    ...
    /* At the top of the stack are 4 values:
       ...
       - exit_func: FIFTH = the context.__exit__ function or context.__exit__ bound
       method We call FOURTH(type(TOP), TOP, GetTraceback(TOP)). Then we push the
...
case _CHECK_ATTR_METHOD_LAZY_DICT: {
    ...
    char *ptr =
        ((char *)PyStackRef_AsPyObjectBorrow(owner)) + MANAGED_DICT_OFFSET + dictoffset;
...
case _MAYBE_EXPAND_METHOD: {
    ...
    /* _DO_CALL is not a viable micro-op for tier 2 because it uses the 'this_instr'
     * variable */

    /* _MONITOR_CALL is not a viable micro-op for tier 2 because it uses the
     * 'this_instr' variable */
...
case _CHECK_AND_ALLOCATE_OBJECT: {
    ...
    if (!_PyThreadState_HasStackSpace(
...
case _CALL_BUILTIN_FAST_WITH_KEYWORDS: {
    ...
    _PyFrame_SetStackPointer(frame, stack_pointer);
    PyCFunctionFastWithKeywords cfunc = (PyCFunctionFastWithKeywords)(void (*)(void)
...
case _CALL_LIST_APPEND: {
    ...
    STAT_INC(CALL, hit);
    int err =
        _PyList_AppendTakeRef((PyListObject *)self_o, PyStackRef_AsPyObjectSteal(arg));
...
case _CALL_METHOD_DESCRIPTOR_FAST: {
    ...
    /* _INSTRUMENTED_CALL_KW is not a viable micro-op for tier 2 because it is
     * instrumented */
...
case _MAYBE_EXPAND_METHOD_KW: {
    ...
    /* _DO_CALL_KW is not a viable micro-op for tier 2 because it uses the 'this_instr'
     * variable */
...
case _CALL_KW_NON_PY: {
    ...
    _PyFrame_SetStackPointer(frame, stack_pointer);
    PyObject *res_o = PyObject_Vectorcall(
        callable_o, args_o, positional_args | PY_VECTORCALL_ARGUMENTS_OFFSET, kwnames_o
    );
    stack_pointer = _PyFrame_GetStackPointer(frame);
    PyStackRef_CLOSE(kwnames);
    STACKREFS_TO_PYOBJECTS_CLEANUP(args_o);
    ...
    /* _INSTRUMENTED_CALL_FUNCTION_EX is not a viable micro-op for tier 2 because it is
     * instrumented */
...
case _MAKE_CALLARGS_A_TUPLE: {
    ...
    oparg = CURRENT_OPARG();
    if (oparg & 1) {
        kwargs_in = stack_pointer[-(oparg & 1)];
    }
    ...
    if (oparg & 1)
        stack_pointer[-(oparg & 1)] = kwargs_out;
    break;
}

    /* _DO_CALL_FUNCTION_EX is not a viable micro-op for tier 2 because it uses the
     * 'this_instr' variable */
...
case _FORMAT_WITH_SPEC: {
    ...
    PyObject *res_o = PyObject_Format(
        PyStackRef_AsPyObjectBorrow(value), PyStackRef_AsPyObjectBorrow(fmt_spec)
    );
    stack_pointer = _PyFrame_GetStackPointer(frame);
    PyStackRef_CLOSE(value);
...
case _SWAP: {
    ...
    /* _INSTRUMENTED_INSTRUCTION is not a viable micro-op for tier 2 because it is
     * instrumented */

    /* _INSTRUMENTED_JUMP_FORWARD is not a viable micro-op for tier 2 because it is
     * instrumented */

    /* _MONITOR_JUMP_BACKWARD is not a viable micro-op for tier 2 because it uses the
     * 'this_instr' variable */

    /* _INSTRUMENTED_POP_JUMP_IF_TRUE is not a viable micro-op for tier 2 because it is
     * instrumented */

    /* _INSTRUMENTED_POP_JUMP_IF_FALSE is not a viable micro-op for tier 2 because it is
     * instrumented */

    /* _INSTRUMENTED_POP_JUMP_IF_NONE is not a viable micro-op for tier 2 because it is
     * instrumented */

    /* _INSTRUMENTED_POP_JUMP_IF_NOT_NONE is not a viable micro-op for tier 2 because it
     * is instrumented */
