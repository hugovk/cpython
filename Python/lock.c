...
_PyMutex_LockTimed(PyMutex *m, PyTime_t timeout, _PyLockFlags flags) {
    ...
    for (;;) {
        ...
        int ret = _PyParkingLot_Park(
            &m->_bits,
            &newv,
            sizeof(newv),
            timeout,
            &entry,
            (flags & _PY_LOCK_DETACH) != 0
        );
        if (ret == Py_PARK_OK) {
            if (entry.handed_off) {
                // We own the lock now.
...
PyEvent_WaitTimed(PyEvent *evt, PyTime_t timeout_ns, int detach) {
    for (;;) {
        ...
        (void)_PyParkingLot_Park(
            &evt->v, &expected, sizeof(evt->v), timeout_ns, NULL, detach
        );
...
unlock_once(_PyOnceFlag *o, int res) {
    ...
    switch (res) {
        case -1:
            new_value = _Py_UNLOCKED;
            break;
        case 0:
            new_value = _Py_ONCE_INITIALIZED;
            break;
        default: {
...
_PyRecursiveMutex_LockTimed(
    _PyRecursiveMutex *m, PyTime_t timeout, _PyLockFlags flags
) {
    PyThread_ident_t thread = PyThread_get_thread_ident_ex();
    if (recursive_mutex_is_owned_by(m, thread)) {
        m->level++;
...
_PyRecursiveMutex_Unlock(_PyRecursiveMutex *m) {
    if (_PyRecursiveMutex_TryUnlock(m) < 0) {
        Py_FatalError(
            "unlocking a recursive mutex that is not "
...
rwmutex_set_parked_and_wait(_PyRWMutex *rwmutex, uintptr_t bits) {
    ...
    if ((bits & _Py_HAS_PARKED) == 0) {
        ...
        if (!_Py_atomic_compare_exchange_uintptr(&rwmutex->bits, &bits, newval)) {
            return bits;
        }
        bits = newval;
    }
...
_PyRWMutex_RUnlock(_PyRWMutex *rwmutex) {
    uintptr_t bits =
        _Py_atomic_add_uintptr(&rwmutex->bits, -(1 << _PyRWMutex_READER_SHIFT));
    ...
    for (;;) {
        ...
        if ((bits & ~_Py_HAS_PARKED) == 0) {
            if (!_Py_atomic_compare_exchange_uintptr(
                    &rwmutex->bits, &bits, bits | _Py_WRITE_LOCKED
                )) {
                continue;
            }
            return;
...
void
_PySeqLock_LockWrite(_PySeqLock *seqlock) {
    ...
    while (1) {
        ...
        } else if (_Py_atomic_compare_exchange_uint32(
                       &seqlock->sequence, &prev, prev + 1
                   )) {
...
void
_PySeqLock_AbandonWrite(_PySeqLock *seqlock) {
...
void
_PySeqLock_UnlockWrite(_PySeqLock *seqlock) {
    uint32_t new_seq = _Py_atomic_load_uint32_relaxed(&seqlock->sequence) + 1;
    assert(!SEQLOCK_IS_UPDATING(new_seq));
    _Py_atomic_store_uint32(&seqlock->sequence, new_seq);
}
...
uint32_t
_PySeqLock_BeginRead(_PySeqLock *seqlock) {
...
int
_PySeqLock_EndRead(_PySeqLock *seqlock, uint32_t previous) {
...
int
_PySeqLock_AfterFork(_PySeqLock *seqlock) {
