...
int
_PyInstructionSequence_Addop(instr_sequence *seq, int opcode, int oparg, location loc) {
...
_PyInstructionSequence_InsertInstruction(
    instr_sequence *seq, int pos, int opcode, int oparg, location loc
) {
...
/*[clinic end generated code: output=14540fad459f7971 input=f2c482568b3b3c0f]*/
{
    if (!_PyInstructionSequence_Check(nested)) {
        PyErr_Format(
            PyExc_TypeError, "expected an instruction sequence, not %T", Py_TYPE(nested)
        );
        return NULL;
    }
...
static PyMethodDef inst_seq_methods[] = {
    INSTRUCTIONSEQUENCETYPE_ADDOP_METHODDEF INSTRUCTIONSEQUENCETYPE_NEW_LABEL_METHODDEF
...
inst_seq_dealloc(_PyInstructionSequence *seq) {
    ...
    Py_TRASHCAN_BEGIN(seq, inst_seq_dealloc) PyInstructionSequence_Fini(seq);
    PyObject_GC_Del(seq);
    Py_TRASHCAN_END
}
...
PyTypeObject _PyInstructionSequence_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type, 0) "InstructionSequence",
