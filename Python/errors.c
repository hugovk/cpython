...
static PyObject *
_PyErr_FormatV(
    PyThreadState *tstate, PyObject *exception, const char *format, va_list vargs
);
...
void
_PyErr_Restore(
    PyThreadState *tstate, PyObject *type, PyObject *value, PyObject *traceback
) {
    if (type == NULL) {
        assert(value == NULL);
...
static PyObject *
get_normalization_failure_note(
    PyThreadState *tstate, PyObject *exception, PyObject *value
) {
    ...
    if (args == NULL) {
        ...
        note =
            PyUnicode_FromFormat("Normalization failed: type=%s args=%S", tpname, args);
        Py_DECREF(args);
    }
    return note;
}
...
_PyErr_SetObject(PyThreadState *tstate, PyObject *exception, PyObject *value) {
    ...
    if (!is_subclass) {
        ...
        if (fixed_value == NULL) {
            ...
            Py_XDECREF(value);
            if (note != NULL) {
                /* ignore errors in _PyException_AddNote - they will be overwritten
                 * below */
...
void
_PyErr_SetString(PyThreadState *tstate, PyObject *exception, const char *string) {
...
_PyErr_NormalizeException(
    PyThreadState *tstate, PyObject **exc, PyObject **val, PyObject **tb
) {
...
void
_PyErr_Fetch(
    PyThreadState *tstate, PyObject **p_type, PyObject **p_value, PyObject **p_traceback
) {
    PyObject *exc = _PyErr_GetRaisedException(tstate);
    *p_value = exc;
    if (exc == NULL) {
...
PyErr_BadArgument(void) {
    PyThreadState *tstate = _PyThreadState_GET();
    _PyErr_SetString(
        tstate, PyExc_TypeError, "bad argument type for built-in operation"
    );
    return 0;
}
...
PyErr_SetFromErrnoWithFilenameObjects(
    PyObject *exc, PyObject *filenameObject, PyObject *filenameObject2
) {
    PyThreadState *tstate = _PyThreadState_GET();
    PyObject *message;
    PyObject *v, *args;
    ...
    if (filenameObject != NULL) {
        if (filenameObject2 != NULL)
            args = Py_BuildValue(
                "(iOOiO)", i, message, filenameObject, 0, filenameObject2
            );
        else
...
PyErr_SetExcFromWindowsErrWithFilenameObject(
    PyObject *exc, int ierr, PyObject *filenameObject
) {
    return PyErr_SetExcFromWindowsErrWithFilenameObjects(
        exc, ierr, filenameObject, NULL
    );
}

PyObject *
PyErr_SetExcFromWindowsErrWithFilenameObjects(
    PyObject *exc, int ierr, PyObject *filenameObject, PyObject *filenameObject2
) {
    PyThreadState *tstate = _PyThreadState_GET();
    int len;
...
PyObject *
PyErr_SetExcFromWindowsErrWithFilename(PyObject *exc, int ierr, const char *filename) {
    PyObject *name = NULL;
    if (filename) {
        if ((DWORD)ierr == 0) {
    ...
    PyObject *ret =
        PyErr_SetExcFromWindowsErrWithFilenameObjects(exc, ierr, name, NULL);
    Py_XDECREF(name);
    return ret;
}

PyObject *
PyErr_SetExcFromWindowsErr(PyObject *exc, int ierr) {
    return PyErr_SetExcFromWindowsErrWithFilename(exc, ierr, NULL);
}

PyObject *
PyErr_SetFromWindowsErr(int ierr) {
    return PyErr_SetExcFromWindowsErrWithFilename(PyExc_OSError, ierr, NULL);
}
...
PyErr_SetFromWindowsErrWithFilename(int ierr, const char *filename) {
    ...
    PyObject *result =
        PyErr_SetExcFromWindowsErrWithFilenameObjects(PyExc_OSError, ierr, name, NULL);
    Py_XDECREF(name);
    return result;
}
...
_PyErr_SetImportErrorSubclassWithNameFrom(
    ...
    if (issubclass < 0) {
        ...
        _PyErr_SetString(tstate, PyExc_TypeError, "expected a subclass of ImportError");
        return NULL;
    }

    if (msg == NULL) {
        _PyErr_SetString(tstate, PyExc_TypeError, "expected a message argument");
        return NULL;
    }
...
PyErr_SetImportErrorSubclass(
    PyObject *exception, PyObject *msg, PyObject *name, PyObject *path
) {
...
_PyErr_SetImportErrorWithNameFrom(
    PyObject *msg, PyObject *name, PyObject *path, PyObject *from_name
) {
    return _PyErr_SetImportErrorSubclassWithNameFrom(
        PyExc_ImportError, msg, name, path, from_name
    );
}
...
PyErr_BadInternalCall(void) {
    ...
    PyThreadState *tstate = _PyThreadState_GET();
    _PyErr_SetString(tstate, PyExc_SystemError, "bad argument to internal function");
}
...
PyObject *
_PyErr_Format(PyThreadState *tstate, PyObject *exception, const char *format, ...) {
    va_list vargs;
    va_start(vargs, format);
    _PyErr_FormatV(tstate, exception, format, vargs);
    va_end(vargs);
...
PyErr_NewException(const char *name, PyObject *base, PyObject *dict) {
    ...
    if (dot == NULL) {
        _PyErr_SetString(
            tstate, PyExc_SystemError, "PyErr_NewException: name must be module.class"
        );
        return NULL;
    }
    if (base == NULL) {
    ...
    if (r == 0) {
        modulename = PyUnicode_FromStringAndSize(name, (Py_ssize_t)(dot - name));
        if (modulename == NULL)
            goto failure;
    ...
    /* Create a real class. */
    result =
        PyObject_CallFunction((PyObject *)&PyType_Type, "sOO", dot + 1, bases, dict);
failure:
    Py_XDECREF(bases);
    Py_XDECREF(mydict);
...
_PyErr_InitTypes(PyInterpreterState *interp) {
    if (_PyStructSequence_InitBuiltin(
            interp, &UnraisableHookArgsType, &UnraisableHookArgs_desc
        ) < 0) {
...
write_unraisable_exc(
    ...
    /* Hold a strong reference to ensure that sys.stderr doesn't go away
       while we use it */
    Py_INCREF(file);
    int res = write_unraisable_exc_file(
        tstate, exc_type, exc_value, exc_tb, err_msg, obj, file
    );
    Py_DECREF(file);
...
format_unraisable_v(const char *format, va_list va, PyObject *obj) {
    ...
    PyObject *hook_args =
        make_unraisable_hook_args(tstate, exc_type, exc_value, exc_tb, err_msg, obj);
    if (hook_args == NULL) {
        err_msg_str =
            ("Exception ignored on building "
...
error:
    ...
    Py_XSETREF(
        ...
        PyUnicode_FromString(
            err_msg_str ? err_msg_str : "Exception ignored in sys.unraisablehook"
        )
    );
    Py_XDECREF(exc_type);
...
default_hook:
    ...
    (void)write_unraisable_exc(tstate, exc_type, exc_value, exc_tb, err_msg, obj);
...
PyErr_SyntaxLocationObjectEx(
    PyObject *filename, int lineno, int col_offset, int end_lineno, int end_col_offset
) {
    PyThreadState *tstate = _PyThreadState_GET();
...
_PyErr_RaiseSyntaxError(
    ...
    PyObject *args = Py_BuildValue(
        "O(OiiOii)", msg, filename, lineno, col_offset, text, end_lineno, end_col_offset
    );
    if (args == NULL) {
        goto exit;
    }
...
_PyErr_EmitSyntaxWarning(
    ...
    if (PyErr_WarnExplicitObject(
            PyExc_SyntaxWarning, msg, filename, lineno, NULL, NULL
        ) < 0) {
        if (PyErr_ExceptionMatches(PyExc_SyntaxWarning)) {
            ...
            PyErr_Clear();
            _PyErr_RaiseSyntaxError(
                msg, filename, lineno, col_offset, end_lineno, end_col_offset
            );
        }
        return -1;
    }
...
err_programtext(FILE *fp, int lineno, const char *encoding) {
    ...
    for (int i = 0; i < lineno;) {
        line_size = 0;
        if (_Py_UniversalNewlineFgetsWithSize(
                linebuf, sizeof(linebuf), fp, NULL, &line_size
            ) == NULL) {
            /* Error or EOF. */
            return NULL;
        }
        ...
        if (i + 1 < lineno && line_size == sizeof(linebuf) - 1 &&
...
extern char *
_PyTokenizer_FindEncodingFilename(int, PyObject *);
