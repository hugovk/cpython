...
tb_create_raw(PyTracebackObject *next, PyFrameObject *frame, int lasti, int lineno) {
    ...
    if ((next != NULL && !PyTraceBack_Check(next)) || frame == NULL ||
        !PyFrame_Check(frame)) {
...
/*[clinic end generated code: output=fa077debd72d861a input=b88143145454cb59]*/
{
    ...
    return tb_create_raw((PyTracebackObject *)tb_next, tb_frame, tb_lasti, tb_lineno);
}

static PyObject *
tb_dir(PyTracebackObject *self, PyObject *Py_UNUSED(ignored)) {
    return Py_BuildValue("[ssss]", "tb_frame", "tb_next", "tb_lasti", "tb_lineno");
}
...
tb_dealloc(PyTracebackObject *tb) {
    ...
    Py_TRASHCAN_BEGIN(tb, tb_dealloc) Py_XDECREF(tb->tb_next);
...
void
_PyTraceback_Add(const char *funcname, const char *filename, int lineno) {
    PyObject *globals;
    PyCodeObject *code;
    PyFrameObject *frame;
...
tb_displayline(
    ...
    int rc = display_source_line(
        f, filename, lineno, _TRACEBACK_SOURCE_LINE_INDENT, &truncation, &source_line
    );
    if (rc != 0 || !source_line) {
...
tb_print_line_repeated(PyObject *f, long cnt) {
    ...
    PyObject *line = PyUnicode_FromFormat(
        (cnt > 1) ? "  [Previous line repeated %ld more times]\n"
...
tb_printinternal(PyTracebackObject *tb, PyObject *f, long limit) {
    ...
    while (tb != NULL) {
        ...
        if (last_file == NULL || code->co_filename != last_file || last_line == -1 ||
            tb_lineno != last_line || last_name == NULL || code->co_name != last_name) {
        ...
        if (cnt <= TB_RECURSIVE_CUTOFF) {
            if (tb_displayline(
                    tb, f, code->co_filename, tb_lineno, tb->tb_frame, code->co_name
                ) < 0) {
                goto error;
            }
...
write_thread_id(int fd, PyThreadState *tstate, int is_current) {
    if (is_current)
        ...
        PUTS(fd, "Thread 0x");
    _Py_DumpHexadecimal(fd, tstate->thread_id, sizeof(unsigned long) * 2);
