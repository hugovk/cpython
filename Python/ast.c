...
static int
validate_stmts(struct validator *, asdl_stmt_seq *);
static int
validate_exprs(struct validator *, asdl_expr_seq *, expr_context_ty, int);
static int
validate_patterns(struct validator *, asdl_pattern_seq *, int);
static int
validate_type_params(struct validator *, asdl_type_param_seq *);
static int
_validate_nonempty_seq(asdl_seq *, const char *, const char *);
static int
validate_stmt(struct validator *, stmt_ty);
static int
validate_expr(struct validator *, expr_ty, expr_context_ty);
static int
validate_pattern(struct validator *, pattern_ty, int);
static int
validate_typeparam(struct validator *, type_param_ty);
...
validate_name(PyObject *name) {
    ...
    static const char *const forbidden[] = {"None", "True", "False", NULL};
...
validate_arguments(struct validator *state, arguments_ty args) {
    ...
    if (asdl_seq_LEN(args->defaults) >
        asdl_seq_LEN(args->posonlyargs) + asdl_seq_LEN(args->args)) {
        PyErr_SetString(
            PyExc_ValueError, "more positional defaults than args on arguments"
        );
        return 0;
    }
    ...
    return validate_exprs(state, args->defaults, Load, 0) &&
           validate_exprs(state, args->kw_defaults, Load, 1);
}
...
validate_constant(struct validator *state, PyObject *value) {
    ...
    if (PyLong_CheckExact(value) || PyFloat_CheckExact(value) ||
        PyComplex_CheckExact(value) || PyBool_Check(value) ||
    ...
    switch (exp->kind) {
        ...
        default:
            if (ctx != Load) {
                PyErr_Format(
                    PyExc_ValueError,
                    "expression which can't be "
                    "assigned to in %s context",
...
#undef COMP
        ...
        case Compare_kind:
            ...
            if (asdl_seq_LEN(exp->v.Compare.comparators) !=
                ...
                PyErr_SetString(
                    PyExc_ValueError,
                    "Compare has a different number "
        ...
        case NamedExpr_kind:
            if (exp->v.NamedExpr.target->kind != Name_kind) {
                PyErr_SetString(PyExc_TypeError, "NamedExpr target must be a Name");
                return 0;
            }
...
validate_pattern_match_value(struct validator *state, expr_ty exp) {
    ...
    switch (exp->kind) {
        case Constant_kind:
            ...
            PyErr_SetString(
                PyExc_ValueError, "unexpected constant inside of a literal pattern"
            );
            return 0;
        case Attribute_kind:
    ...
    PyErr_SetString(
        PyExc_ValueError, "patterns may only match literals and attribute lookups"
    );
    return 0;
}
...
validate_pattern(struct validator *state, pattern_ty p, int star_ok) {
    ...
    switch (p->kind) {
        ...
        case MatchSingleton_kind:
            ret = p->v.MatchSingleton.value == Py_None ||
                  PyBool_Check(p->v.MatchSingleton.value);
            if (!ret) {
                PyErr_SetString(
                    PyExc_ValueError,
        ...
        case MatchMapping_kind:
            if (asdl_seq_LEN(p->v.MatchMapping.keys) !=
                asdl_seq_LEN(p->v.MatchMapping.patterns)) {
                PyErr_SetString(
                    PyExc_ValueError,
        ...
        case MatchClass_kind:
            if (asdl_seq_LEN(p->v.MatchClass.kwd_attrs) !=
                asdl_seq_LEN(p->v.MatchClass.kwd_patterns)) {
                PyErr_SetString(
                    PyExc_ValueError,
                    "MatchClass doesn't have the same number of keyword attributes as "
                    "patterns"
                );
                ret = 0;
                break;
        ...
        case MatchOr_kind:
            if (asdl_seq_LEN(p->v.MatchOr.patterns) < 2) {
                PyErr_SetString(
                    PyExc_ValueError, "MatchOr requires at least 2 patterns"
                );
                ret = 0;
                break;
            }
...
#define validate_nonempty_seq(seq, what, owner) \
    _validate_nonempty_seq((asdl_seq *)seq, what, owner)

static int
validate_assignlist(
    struct validator *state, asdl_expr_seq *targets, expr_context_ty ctx
) {
    assert(!PyErr_Occurred());
    return validate_nonempty_seq(
               targets, "targets", ctx == Del ? "Delete" : "Assign"
           ) &&
    ...
    switch (stmt->kind) {
        ...
        case AnnAssign_kind:
            if (stmt->v.AnnAssign.target->kind != Name_kind &&
                ...
                PyErr_SetString(
                    PyExc_TypeError, "AnnAssign with simple non-Name target"
                );
                return 0;
            }
        ...
        case TypeAlias_kind:
            if (stmt->v.TypeAlias.name->kind != Name_kind) {
                PyErr_SetString(PyExc_TypeError, "TypeAlias with non-Name name");
                return 0;
            }
        ...
        case With_kind:
            ...
            for (Py_ssize_t i = 0; i < asdl_seq_LEN(stmt->v.With.items); i++) {
                ...
                if (!validate_expr(state, item->context_expr, Load) ||
                    (item->optional_vars &&
                     !validate_expr(state, item->optional_vars, Store)))
                    return 0;
            }
        ...
        case Raise_kind:
            if (stmt->v.Raise.exc) {
                ret = validate_expr(state, stmt->v.Raise.exc, Load) &&
                      (!stmt->v.Raise.cause ||
                       validate_expr(state, stmt->v.Raise.cause, Load));
                break;
            }
        ...
        case Try_kind:
            ...
            if (!asdl_seq_LEN(stmt->v.Try.handlers) &&
                ...
                PyErr_SetString(
                    PyExc_ValueError, "Try has neither except handlers nor finalbody"
                );
                return 0;
            }
            if (!asdl_seq_LEN(stmt->v.Try.handlers) &&
                ...
                PyErr_SetString(
                    PyExc_ValueError, "Try has orelse but no except handlers"
                );
                return 0;
            }
            for (Py_ssize_t i = 0; i < asdl_seq_LEN(stmt->v.Try.handlers); i++) {
                ...
                if ((handler->v.ExceptHandler.type &&
                    ...
                    !validate_body(
                        state, handler->v.ExceptHandler.body, "ExceptHandler"
                    ))
                    return 0;
            }
        ...
        case TryStar_kind:
            ...
            if (!asdl_seq_LEN(stmt->v.TryStar.handlers) &&
                ...
                PyErr_SetString(
                    PyExc_ValueError, "TryStar has orelse but no except handlers"
                );
                return 0;
            }
        ...
        case Assert_kind:
            ret =
                validate_expr(state, stmt->v.Assert.test, Load) &&
        ...
        case ImportFrom_kind:
            ...
            ret =
                validate_nonempty_seq(stmt->v.ImportFrom.names, "names", "ImportFrom");
            break;
        case Global_kind:
...
static int
validate_exprs(
    struct validator *state, asdl_expr_seq *exprs, expr_context_ty ctx, int null_ok
) {
    assert(!PyErr_Occurred());
    for (Py_ssize_t i = 0; i < asdl_seq_LEN(exprs); i++) {
        ...
        if (expr) {
            ...
            PyErr_SetString(PyExc_ValueError, "None disallowed in expression list");
            return 0;
        }
    }
    return 1;
...
_PyAST_Validate(mod_ty mod) {
    ...
    switch (mod->kind) {
        ...
        case FunctionType_kind:
            res = validate_exprs(
                      &state, mod->v.FunctionType.argtypes, Load, /*null_ok=*/0
                  ) &&
