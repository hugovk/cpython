...
#define HAVE_GETENTRYPY_GETRANDOM_RUNTIME \
    __builtin_available(macOS 10.12, iOS 10.10, tvOS 10.0, watchOS 3.0, *)
#endif
#endif
...
py_getrandom(void *buffer, Py_ssize_t size, int blocking, int raise) {
    ...
    while (0 < size) {
        ...
        if (raise) {
            Py_BEGIN_ALLOW_THREADS n = getrandom(dest, n, flags);
        ...
        if (raise) {
            Py_BEGIN_ALLOW_THREADS n = syscall(SYS_getrandom, dest, n, flags);
            Py_END_ALLOW_THREADS
        } else {
...
py_getentropy(char *buffer, Py_ssize_t size, int raise) {
    ...
    while (size > 0) {
        ...
        if (raise) {
            Py_BEGIN_ALLOW_THREADS res = getentropy(buffer, len);
            Py_END_ALLOW_THREADS
        } else {
    ...
    if (raise) {
        ...
        if (urandom_cache.fd >= 0) {
            ...
            Py_END_ALLOW_THREADS
                ...
                if (fstat_result || st.st_dev != urandom_cache.st_dev ||
