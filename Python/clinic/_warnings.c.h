...
#define WARNINGS_WARN_METHODDEF        \
    {"warn",                           \
     _PyCFunction_CAST(warnings_warn), \
...
#define KWTUPLE NULL
    ...
    static const char *const _keywords[] = {
        "message", "category", "stacklevel", "source", "skip_file_prefixes", NULL
    };
...
skip_optional_kwonly:
    return_value = warnings_warn_impl(
        module, message, category, stacklevel, source, skip_file_prefixes
    );
...
#define WARNINGS_WARN_EXPLICIT_METHODDEF        \
    {"warn_explicit",                           \
     _PyCFunction_CAST(warnings_warn_explicit), \
     METH_FASTCALL | METH_KEYWORDS,             \
     warnings_warn_explicit__doc__},
...
#undef KWTUPLE
    ...
    args =
        _PyArg_UnpackKeywords(args, nargs, NULL, kwnames, &_parser, 4, 8, 0, argsbuf);
    if (!args) {
        goto exit;
    }
    message = args[0];
    ...
    sourceobj = args[7];
skip_optional_pos:
    return_value = warnings_warn_explicit_impl(
        module,
        message,
        category,
        filename,
...
#define WARNINGS_FILTERS_MUTATED_METHODDEF  \
    {"_filters_mutated",                    \
     (PyCFunction)warnings_filters_mutated, \
     METH_NOARGS,                           \
     warnings_filters_mutated__doc__},
