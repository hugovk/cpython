...
static PyObject *
run_pyc_file(FILE *, PyObject *, PyObject *, PyCompilerFlags *);
static int
PyRun_InteractiveOneObjectEx(FILE *, PyObject *, PyCompilerFlags *);
static PyObject *
pyrun_file(
    FILE *fp,
    PyObject *filename,
...
_PyRun_AnyFileObject(
    FILE *fp, PyObject *filename, int closeit, PyCompilerFlags *flags
) {
...
pyrun_one_parse_ast(
    ...
    int errcode = 0;
    *pmod = _PyParser_InteractiveASTFromFile(
        fp,
        filename,
        encoding,
        Py_single_input,
 ...
 * error on failure. */
static int
PyRun_InteractiveOneObjectEx(FILE *fp, PyObject *filename, PyCompilerFlags *flags) {
    ...
    PyObject *interactive_src;
    int parse_res =
        pyrun_one_parse_ast(fp, filename, flags, arena, &mod, &interactive_src);
    if (parse_res != 0) {
        _PyArena_Free(arena);
        return parse_res;
    }
    ...
    PyObject *res =
        run_mod(mod, filename, main_dict, main_dict, flags, arena, interactive_src, 1);
    ...
    if (res == NULL) {
        ...
        if (PyType_IsSubtype(Py_TYPE(exc), (PyTypeObject *)PyExc_SyntaxError)) {
            /* fix "text" attribute */
...
set_main_loader(PyObject *d, PyObject *filename, const char *loader_name) {
    ...
    PyObject *loader_type = _PyImport_GetImportlibExternalLoader(interp, loader_name);
    if (loader_type == NULL) {
        return -1;
    }

    PyObject *loader = PyObject_CallFunction(loader_type, "sO", "__main__", filename);
...
_PyRun_SimpleFileObject(
    ...
    if (pyc) {
        ...
        v = pyrun_file(fp, filename, Py_file_input, dict, dict, closeit, flags);
    }
    flush_io();
    if (v == NULL) {
...
_PyRun_SimpleStringFlagsWithName(
    const char *command, const char *name, PyCompilerFlags *flags
) {
    PyObject *main_module = PyImport_AddModuleRef("__main__");
    if (main_module == NULL) {
        return -1;
    ...
    if (name == NULL) {
        ...
        res = _PyRun_StringFlagsWithName(
...
_PyErr_PrintEx(PyThreadState *tstate, int set_sys_last_vars) {
    ...
    if (_PySys_Audit(
            tstate, "sys.excepthook", "OOOO", hook ? hook : Py_None, typ, exc, tb
        ) < 0) {
...
static int
print_exception_file_and_line(struct exception_print_context *ctx, PyObject **value_p) {
    PyObject *f = ctx->file;
...
print_exception_message(
    struct exception_print_context *ctx, PyObject *type, PyObject *value
) {
    PyObject *f = ctx->file;
 ...
 */
static bool
print_exception_seen_lookup(struct exception_print_context *ctx, PyObject *value) {
...
static int
print_exception_cause_and_context(
    struct exception_print_context *ctx, PyObject *value
) {
...
_PyErr_Display(PyObject *file, PyObject *unused, PyObject *value, PyObject *tb) {
    ...
    PyObject *print_exception_fn =
        PyObject_GetAttrString(traceback_module, "_print_exception_bltin");
...
#ifdef Py_DEBUG
    if (PyErr_Occurred()) {
        PyErr_FormatUnraisable("Exception ignored in the internal traceback machinery");
    }
#endif
    PyErr_Clear();
    struct exception_print_context ctx;
...
void
_PyErr_DisplayException(PyObject *file, PyObject *exc) {
    _PyErr_Display(file, NULL, exc, NULL);
}

void
PyErr_DisplayException(PyObject *exc) {
...
_PyRun_StringFlagsWithName(
    ...
    if (mod != NULL) {
        ret = run_mod(
            mod, name, globals, locals, flags, arena, source, generate_new_source
        );
    }
    Py_XDECREF(source);
    _PyArena_Free(arena);
...
pyrun_file(
    ...
    mod = _PyParser_ASTFromFile(
        fp, filename, NULL, start, NULL, NULL, flags, NULL, arena
    );
...
PyRun_FileExFlags(
    ...
    PyObject *res =
        pyrun_file(fp, filename_obj, start, globals, locals, closeit, flags);
    Py_DECREF(filename_obj);
    return res;
}
...
static PyObject *
run_eval_code_obj(
    PyThreadState *tstate, PyCodeObject *co, PyObject *globals, PyObject *locals
) {
    PyObject *v;
    /*
    ...
    if (!has_builtins) {
        if (PyDict_SetItemString(globals, "__builtins__", tstate->interp->builtins) <
            0) {
            return NULL;
        }
    }
...
run_mod(
    ...
    if (interactive_src) {
        ...
        PyObject *print_tb_func =
            PyObject_GetAttrString(linecache_module, "_register_code");
        ...
        if (!PyCallable_Check(print_tb_func)) {
            ...
            PyErr_SetString(
                PyExc_ValueError, "linecache._register_code is not callable"
            );
            return NULL;
        }

        PyObject *result = PyObject_CallFunction(
            print_tb_func, "OOO", interactive_filename, interactive_src, filename
        );
...
static PyObject *
run_pyc_file(FILE *fp, PyObject *globals, PyObject *locals, PyCompilerFlags *flags) {
    PyThreadState *tstate = _PyThreadState_GET();
    PyCodeObject *co;
    PyObject *v;
    long magic;
...
Py_CompileStringObject(
    const char *str, PyObject *filename, int start, PyCompilerFlags *flags, int optimize
) {
    PyCodeObject *co;
    mod_ty mod;
    PyArena *arena = _PyArena_New();
    ...
    *cmd_copy = NULL;
    if (PyUnicode_Check(cmd)) {
        ...
        *cmd_copy = PyBytes_FromStringAndSize((const char *)view.buf, view.len);
        ...
        PyErr_Format(PyExc_TypeError, "%s() arg 1 must be a %s object", funcname, what);
        return NULL;
    }

    if (strlen(str) != (size_t)size) {
        PyErr_SetString(
            PyExc_SyntaxError, "source code string cannot contain null bytes"
        );
        Py_CLEAR(*cmd_copy);
        return NULL;
    }
...
PyOS_CheckStack(void) {
    __try {
        ...
        GetExceptionCode() == STATUS_STACK_OVERFLOW ? EXCEPTION_EXECUTE_HANDLER
...
PyAPI_FUNC(int) PyRun_AnyFileFlags(FILE *fp, const char *name, PyCompilerFlags *flags) {
...
PyAPI_FUNC(PyObject *) PyRun_FileFlags(
    FILE *fp, const char *p, int s, PyObject *g, PyObject *l, PyCompilerFlags *flags
) {
...
PyAPI_FUNC(PyObject *) Py_CompileString(const char *str, const char *p, int s) {
    return Py_CompileStringExFlags(str, p, s, NULL, -1);
}

#undef Py_CompileStringFlags
PyAPI_FUNC(PyObject *) Py_CompileStringFlags(
    const char *str, const char *p, int s, PyCompilerFlags *flags
) {
