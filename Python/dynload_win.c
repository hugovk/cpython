...
const char *_PyImport_DynLoadFiletab[] = {PYD_TAGGED_SUFFIX, PYD_UNTAGGED_SUFFIX, NULL};
...
GetPythonImport(HINSTANCE hModule) {
    ...
    if (DWORD_AT(dllbase + opt_offset + num_dict_off) >= 2) {
        ...
        while (DWORD_AT(import_data)) {
            ...
            if (strlen(import_name) >= 6 && !strncmp(import_name, "python", 6)) {
                char *pch;
...
#define MAXPATHLEN 512
    ...
    if (config->prefix) {
        ...
        if (py3path[0] &&
            _Py_add_relfile(py3path, L"DLLs\\" PY3_DLLNAME, MAXPATHLEN) >= 0) {
            hPython3 = LoadLibraryExW(py3path, NULL, LOAD_LIBRARY_SEARCH_DEFAULT_DIRS);
        }
    }
    return hPython3 != NULL;
...
_PyImport_FindSharedFuncptrWindows(
    const char *prefix, const char *shortname, PyObject *pathname, FILE *fp
) {
    dl_funcptr p;
...

    {
        ...
        Py_BEGIN_ALLOW_THREADS hDLL = LoadLibraryExW(
            wpathname,
            NULL,
            LOAD_LIBRARY_SEARCH_DEFAULT_DIRS | LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR
        );
        Py_END_ALLOW_THREADS PyMem_Free(wpathname);
        ...
        if (hDLL == NULL) {
            ...
            if (theLength == 0) {
                ...
                if (theLength >= 2 && theInfo[theLength - 2] == '\r' &&
                    theInfo[theLength - 1] == '\n') {
                    theLength -= 2;
                    theInfo[theLength] = '\0';
                }
                ...
                PyUnicode_AppendAndDel(
                    &message, PyUnicode_FromWideChar(theInfo, theLength)
                );
            }
            if (message != NULL) {
            ...
            if (import_python && _stricmp(buffer, import_python)) {
                PyErr_Format(
                    PyExc_ImportError,
                ...
                Py_BEGIN_ALLOW_THREADS FreeLibrary(hDLL);
                Py_END_ALLOW_THREADS return NULL;
            }
        }
        Py_BEGIN_ALLOW_THREADS p = GetProcAddress(hDLL, funcname);
