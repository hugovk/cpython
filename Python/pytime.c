...
pytime_overflow(void) {
    PyErr_SetString(
        PyExc_OverflowError, "timestamp too large to convert to C PyTime_t"
    );
}
...
pytime_object_to_denominator(
    PyObject *obj, time_t *sec, long *numerator, long denominator, _PyTime_round_t round
) {
    ...
    if (PyFloat_Check(obj)) {
        ...
        return pytime_double_to_denominator(d, sec, numerator, denominator, round);
    } else {
        ...
        *numerator = 0;
        if (*sec == (time_t)-1 && PyErr_Occurred()) {
            if (PyErr_ExceptionMatches(PyExc_TypeError)) {
                PyErr_Format(
                    PyExc_TypeError, "argument must be int or float, not %T", obj
                );
            }
            return -1;
        }
...
_PyTime_ObjectToTimespec(
    PyObject *obj, time_t *sec, long *nsec, _PyTime_round_t round
) {
    return pytime_object_to_denominator(obj, sec, nsec, SEC_TO_NS, round);
}

int
_PyTime_ObjectToTimeval(PyObject *obj, time_t *sec, long *usec, _PyTime_round_t round) {
...
_PyTime_FromLong(PyTime_t *tp, PyObject *obj) {
    ...
    static_assert(sizeof(long long) == sizeof(PyTime_t), "PyTime_t is not long long");
...
pytime_fromtimespec(PyTime_t *tp, const struct timespec *ts, int raise_exc) {
    ...
    static_assert(
        sizeof(ts->tv_sec) <= sizeof(PyTime_t),
...
pytime_fromtimeval(PyTime_t *tp, struct timeval *tv, int raise_exc) {
    static_assert(
        sizeof(tv->tv_sec) <= sizeof(PyTime_t), "timeval.tv_sec is larger than PyTime_t"
    );
...
static int
pytime_from_double(PyTime_t *tp, double value, _PyTime_round_t round, long unit_to_ns) {
    /* volatile avoids optimization changing how numbers are rounded */
    volatile double d;
...
static int
pytime_from_object(
    PyTime_t *tp, PyObject *obj, _PyTime_round_t round, long unit_to_ns
) {
    if (PyFloat_Check(obj)) {
        double d;
        d = PyFloat_AsDouble(obj);
        ...
        static_assert(
            sizeof(long long) <= sizeof(PyTime_t), "PyTime_t is smaller than long long"
        );
...
_PyTime_AsLong(PyTime_t ns) {
    static_assert(
        sizeof(long long) >= sizeof(PyTime_t), "PyTime_t is larger than long long"
    );
...
static PyTime_t
pytime_divide(const PyTime_t t, const PyTime_t k, const _PyTime_round_t round) {
    assert(k > 1);
    if (round == _PyTime_ROUND_HALF_EVEN) {
        PyTime_t x = t / k;
        PyTime_t r = t % k;
...
static int
pytime_divmod(const PyTime_t t, const PyTime_t k, PyTime_t *pq, PyTime_t *pr) {
    assert(k > 1);
    PyTime_t q = t / k;
    PyTime_t r = t % k;
    if (r < 0) {
...
static int
pytime_as_timeval(
    PyTime_t ns, PyTime_t *ptv_sec, int *ptv_usec, _PyTime_round_t round
) {
...
pytime_as_timeval_struct(
    PyTime_t t, struct timeval *tv, _PyTime_round_t round, int raise_exc
) {
    PyTime_t tv_sec;
    int tv_usec;
...
int
_PyTime_AsTimevalTime_t(PyTime_t t, time_t *p_secs, int *us, _PyTime_round_t round) {
    PyTime_t secs;
...
py_get_win_perf_counter(PyTime_t *tp, _Py_clock_info_t *info, int raise_exc) {
    ...
    PyTime_t ticks;
    static_assert(sizeof(ticksll) <= sizeof(ticks), "LONGLONG is larger than PyTime_t");
...
py_mach_timebase_info(_PyTimeFraction *base, int raise_exc) {
    ...
    if (_PyTimeFraction_Set(base, numer, denom) < 0) {
        if (raise_exc) {
            PyErr_SetString(PyExc_RuntimeError, "invalid mach_timebase_info");
        }
        return -1;
    }
    return 0;
