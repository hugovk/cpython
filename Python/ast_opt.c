...
safe_multiply(PyObject *v, PyObject *w) {
    if (PyLong_Check(v) && PyLong_Check(w) && !_PyLong_IsZero((PyLongObject *)v) &&
        !_PyLong_IsZero((PyLongObject *)w)) {
        ...
        Py_ssize_t size =
            PyUnicode_Check(w) ? PyUnicode_GET_LENGTH(w) : PyBytes_GET_SIZE(w);
        if (size) {
            long n = PyLong_AsLong(v);
...
safe_power(PyObject *v, PyObject *w) {
    if (PyLong_Check(v) && PyLong_Check(w) && !_PyLong_IsZero((PyLongObject *)v) &&
        _PyLong_IsPositive((PyLongObject *)w)) {
        int64_t vbits = _PyLong_NumBits(v);
...
parse_literal(PyObject *fmt, Py_ssize_t *ppos, PyArena *arena) {
    ...
    *ppos = pos;
    ...
    if (str && has_percents) {
        ...
        Py_SETREF(
            str, PyUnicode_Replace(str, &_Py_STR(dbl_percent), _Py_LATIN1_CHR('%'), -1)
        );
    }
    if (!str) {
        return NULL;
...
static int
simple_format_arg_parse(
    PyObject *fmt, Py_ssize_t *ppos, int *spec, int *flags, int *width, int *prec
) {
...
fold_binop(expr_ty node, PyArena *arena, _PyASTOptimizeState *state) {
    ...
    if (node->v.BinOp.op == Mod && rhs->kind == Tuple_kind && PyUnicode_Check(lv) &&
...
fold_subscr(expr_ty node, PyArena *arena, _PyASTOptimizeState *state) {
    ...
    if (node->v.Subscript.ctx != Load || arg->kind != Constant_kind ||
...
astfold_comprehension(
    comprehension_ty node_, PyArena *ctx_, _PyASTOptimizeState *state
);
static int
...
astfold_excepthandler(
    excepthandler_ty node_, PyArena *ctx_, _PyASTOptimizeState *state
);
static int
...
astfold_body(asdl_stmt_seq *stmts, PyArena *ctx_, _PyASTOptimizeState *state) {
    ...
    if (!docstring && _PyAST_GetDocString(stmts) != NULL) {
        ...
        expr_ty expr = _PyAST_JoinedStr(
            values, st->lineno, st->col_offset, st->end_lineno, st->end_col_offset, ctx_
        );
        if (!expr) {
            return 0;
        }
...
astfold_expr(expr_ty node_, PyArena *ctx_, _PyASTOptimizeState *state) {
    ...
    switch (node_->kind) {
        ...
        case ListComp_kind:
            ...
            CALL_SEQ(
                astfold_comprehension, comprehension, node_->v.ListComp.generators
            );
            break;
        case SetComp_kind:
        ...
        case DictComp_kind:
            ...
            CALL_SEQ(
                astfold_comprehension, comprehension, node_->v.DictComp.generators
            );
            break;
        case GeneratorExp_kind:
            CALL(astfold_expr, expr_ty, node_->v.GeneratorExp.elt);
            CALL_SEQ(
                astfold_comprehension, comprehension, node_->v.GeneratorExp.generators
            );
            break;
        case Await_kind:
...
astfold_stmt(stmt_ty node_, PyArena *ctx_, _PyASTOptimizeState *state) {
    ...
    switch (node_->kind) {
        ...
        case AsyncFunctionDef_kind:
            CALL_SEQ(
                astfold_type_param, type_param, node_->v.AsyncFunctionDef.type_params
            );
