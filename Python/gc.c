...
gc_set_refs(PyGC_Head *g, Py_ssize_t refs) {
    g->_gc_prev =
        (g->_gc_prev & ~_PyGC_PREV_MASK) | ((uintptr_t)(refs) << _PyGC_PREV_SHIFT);
}

static inline void
gc_reset_refs(PyGC_Head *g, Py_ssize_t refs) {
    g->_gc_prev = (g->_gc_prev & _PyGC_PREV_MASK_FINALIZED) | PREV_MASK_COLLECTING |
                  ((uintptr_t)(refs) << _PyGC_PREV_SHIFT);
}

static inline void
gc_decref(PyGC_Head *g) {
    _PyObject_ASSERT_WITH_MSG(FROM_GC(g), gc_get_refs(g) > 0, "refcount is too small");
...
gc_list_merge(PyGC_Head *from, PyGC_Head *to) {
    ...
    if (!gc_list_is_empty(from)) {
        ...
        assert(
            gc_list_is_empty(to) || gc_old_space(to_tail) == gc_old_space(from_tail)
        );
...
move_unreachable(PyGC_Head *young, PyGC_Head *unreachable) {
    ...
    /* Record which old space we are in, and set NEXT_MASK_UNREACHABLE bit for
     * convenience */
    uintptr_t flags =
        NEXT_MASK_UNREACHABLE | (gc->_gc_next & _PyGC_NEXT_MASK_OLD_SPACE_1);
    while (gc != young) {
        if (gc_get_refs(gc)) {
...
move_legacy_finalizers(PyGC_Head *unreachable, PyGC_Head *finalizers) {
    PyGC_Head *gc, *next;
    _PyObject_ASSERT(
        FROM_GC(unreachable), (unreachable->_gc_next & NEXT_MASK_UNREACHABLE) == 0
    );
...
clear_unreachable_mask(PyGC_Head *unreachable) {
    ...
    _PyObject_ASSERT(
        FROM_GC(unreachable), ((uintptr_t)unreachable & NEXT_MASK_UNREACHABLE) == 0
    );
    _PyObject_ASSERT(
        FROM_GC(unreachable), (unreachable->_gc_next & NEXT_MASK_UNREACHABLE) == 0
    );
...
move_legacy_finalizer_reachable(PyGC_Head *finalizers) {
    ...
    for (; gc != finalizers; gc = GC_NEXT(gc)) {
        ...
        (void)traverse(FROM_GC(gc), visit_move, (void *)finalizers);
    }
}
...
handle_legacy_finalizers(
    PyThreadState *tstate, GCState *gcstate, PyGC_Head *finalizers, PyGC_Head *old
) {
...
static void
delete_garbage(
    PyThreadState *tstate, GCState *gcstate, PyGC_Head *collectable, PyGC_Head *old
) {
    assert(!_PyErr_Occurred(tstate));
            ...
            if ((clear = Py_TYPE(op)->tp_clear) != NULL) {
                ...
                if (_PyErr_Occurred(tstate)) {
                    PyErr_FormatUnraisable(
                        "Exception ignored in tp_clear of %s", Py_TYPE(op)->tp_name
                    );
                }
                Py_DECREF(op);
            }
...
/* Handle objects that may have resurrected after a call to 'finalize_garbage', moving
   ...
   Contracts:
       * After this function 'unreachable' must not be used anymore and
'still_unreachable' will contain the objects that did not resurrect.
...
visit_add_to_container(PyObject *op, void *arg) {
    ...
    if (!_Py_IsImmortal(op) && _PyObject_IS_GC(op)) {
        PyGC_Head *gc = AS_GC(op);
        if (_PyObject_GC_IS_TRACKED(op) && gc_old_space(gc) != visited) {
...
static uintptr_t
expand_region_transitively_reachable(
    PyGC_Head *container, PyGC_Head *gc, GCState *gcstate
) {
    validate_list(container, collecting_clear_unreachable_clear);
    struct container_and_flag arg = {
        .container = container, .visited_space = gcstate->visited_space, .size = 0
    };
...
static void
gc_collect_full(PyThreadState *tstate, struct gc_collection_stats *stats) {
    ...
    gc_collect_region(tstate, visited, visited, UNTRACK_TUPLES | UNTRACK_DICTS, stats);
    gcstate->young.count = 0;
...
_PyGC_Unfreeze(PyInterpreterState *interp) {
    GCState *gcstate = &interp->gc;
    gc_list_merge(&gcstate->permanent_generation.head, &gcstate->old[0].head);
    validate_old(gcstate);
}
...
show_stats_each_generations(GCState *gcstate) {
    ...
    for (int i = 0; i < NUM_GENERATIONS && pos < sizeof(buf); i++) {
        pos += PyOS_snprintf(
            buf + pos, sizeof(buf) - pos, " %zd", gc_list_size(GEN_HEAD(gcstate, i))
        );
    }
    PySys_FormatStderr(
        ...
        buf,
        gc_list_size(&gcstate->permanent_generation.head)
    );
}
...
#ifdef Py_STATS
    if (_Py_stats) {
        GC_STAT_ADD(generation, object_visits, _Py_stats->object_stats.object_visits);
...
_PyGC_DumpShutdownStats(PyInterpreterState *interp) {
    GCState *gcstate = &interp->gc;
    if (!(gcstate->debug & _PyGC_DEBUG_SAVEALL) && gcstate->garbage != NULL &&
        PyList_GET_SIZE(gcstate->garbage) > 0) {
        const char *message;
...
visit_validate(PyObject *op, void *parent_raw) {
    ...
    if (_PyObject_IsFreed(op)) {
        _PyObject_ASSERT_FAILED_MSG(parent, "PyObject_GC_Track() object is not valid");
    }
    return 0;
}
#endif
...
_PyObject_GC_Link(PyObject *op) {
    ...
    gcstate->heap_size++;
    if (gcstate->young.count > gcstate->young.threshold && gcstate->enabled &&
...
PyObject_GC_Del(void *op) {
    ...
    if (_PyObject_GC_IS_TRACKED(op)) {
        ...
        if (PyErr_WarnExplicitFormat(
                ...
                "gc",
                NULL,
                "Object of type %s is not untracked before destruction",
                Py_TYPE(op)->tp_name
            )) {
            PyErr_WriteUnraisable(NULL);
        }
