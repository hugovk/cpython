...
append_repr(_PyUnicodeWriter *writer, PyObject *obj) {
    ...
    if ((PyFloat_CheckExact(obj) && isinf(PyFloat_AS_DOUBLE(obj))) ||
        ...
        PyObject *new_repr =
            PyUnicode_Replace(repr, &_Py_ID(inf), &_Py_STR(str_replace_inf), -1);
        Py_DECREF(repr);
        if (!new_repr) {
            return -1;
        }
...
append_ast_binop(_PyUnicodeWriter *writer, expr_ty e, int level) {
    ...
    switch (e->v.BinOp.op) {
        ...
        case Pow:
            ...
            break;
        default:
            PyErr_SetString(PyExc_SystemError, "unknown binary operator");
            return -1;
    }
...
append_ast_unaryop(_PyUnicodeWriter *writer, expr_ty e, int level) {
    ...
    switch (e->v.UnaryOp.op) {
        ...
        case USub:
            ...
            break;
        default:
            PyErr_SetString(PyExc_SystemError, "unknown unary operator");
            return -1;
    }
...
append_ast_lambda(_PyUnicodeWriter *writer, expr_ty e, int level) {
    ...
    Py_ssize_t n_positional =
        (asdl_seq_LEN(e->v.Lambda.args->args) +
...
append_ast_comprehensions(
    _PyUnicodeWriter *writer, asdl_comprehension_seq *comprehensions
) {
    ...
    for (i = 0; i < comparator_count; i++) {
        switch ((cmpop_ty)asdl_seq_GET(ops, i)) {
            ...
            case NotIn:
                ...
                break;
            default:
                PyErr_SetString(PyExc_SystemError, "unexpected comparison kind");
                return -1;
        }
...
escape_braces(PyObject *orig) {
    PyObject *temp;
    PyObject *result;
    temp = PyUnicode_Replace(orig, _Py_LATIN1_CHR('{'), &_Py_STR(dbl_open_br), -1);
    if (!temp) {
        return NULL;
    }
    result = PyUnicode_Replace(temp, _Py_LATIN1_CHR('}'), &_Py_STR(dbl_close_br), -1);
    Py_DECREF(temp);
    return result;
}
...
append_fstring_element(_PyUnicodeWriter *writer, expr_ty e, bool is_format_spec) {
    switch (e->kind) {
        ...
        default:
            PyErr_SetString(
                PyExc_SystemError, "unknown expression kind inside f-string"
            );
            return -1;
    }
}
...
build_fstring_body(asdl_expr_seq *values, bool is_format_spec) {
    ...
    value_count = asdl_seq_LEN(values);
    for (i = 0; i < value_count; ++i) {
        if (-1 == append_fstring_element(
                      &body_writer, (expr_ty)asdl_seq_GET(values, i), is_format_spec
                  )) {
...
append_joinedstr(_PyUnicodeWriter *writer, expr_ty e, bool is_format_spec) {
    ...
    if (!is_format_spec) {
        if (-1 != append_charp(writer, "f") && -1 != append_repr(writer, body)) {
            result = 0;
        }
    } else {
...
append_formattedvalue(_PyUnicodeWriter *writer, expr_ty e) {
    ...
    if (e->v.FormattedValue.conversion > 0) {
        switch (e->v.FormattedValue.conversion) {
            ...
            case 's':
                ...
                break;
            default:
                PyErr_SetString(PyExc_SystemError, "unknown f-value conversion kind");
                return -1;
        }
    ...
    if (e->v.FormattedValue.format_spec) {
        if (-1 == _PyUnicodeWriter_WriteASCIIString(writer, ":", 1) ||
            -1 ==
                append_fstring_element(writer, e->v.FormattedValue.format_spec, true)) {
            return -1;
        }
    }
...
append_ast_expr(_PyUnicodeWriter *writer, expr_ty e, int level) {
    ...
    PyErr_SetString(PyExc_SystemError, "unknown expression kind");
    return -1;
}
