...
gil_flag_to_str(int flag) {
    switch (flag) {
        ...
        default:
            PyErr_SetString(
                PyExc_SystemError, "invalid interpreter config 'gil' value"
            );
            return NULL;
    }
}
...
gil_flag_from_str(const char *str, int *p_flag) {
    ...
    if (str == NULL) {
        ...
        PyErr_Format(
            PyExc_ValueError, "unsupported interpreter config .gil value '%s'", str
        );
        return -1;
    }
    *p_flag = flag;
...
#define ADD_BOOL(FIELD) ADD(#FIELD, Py_NewRef(config->FIELD ? Py_True : Py_False))
...
static int
_config_dict_copy_str(PyObject *dict, const char *name, char *buf, size_t bufsize) {
    PyObject *item;
    if (_config_dict_get(dict, name, &item) < 0) {
        return -1;
    }
...
interp_config_from_dict(
    PyObject *origdict, PyInterpreterConfig *config, bool missing_allowed
) {
    PyObject *dict = PyDict_New();
    if (dict == NULL) {
        return -1;
...
#undef CHECK
    ...
    if (unused == 1) {
        PyErr_Format(PyExc_ValueError, "config dict has 1 extra item (%R)", dict);
        goto error;
    } else if (unused > 0) {
        PyErr_Format(
            PyExc_ValueError, "config dict has %d extra items (%R)", unused, dict
        );
        goto error;
    }
...
_PyInterpreterConfig_InitFromState(
    PyInterpreterConfig *config, PyInterpreterState *interp
) {
    // Populate the config by re-constructing the values from the interpreter.
    *config = (PyInterpreterConfig){
#define FLAG(flag) (interp->feature_flags & Py_RTFLAGS_##flag)
...
#undef FLAG
        .gil = interp->ceval.own_gil ? PyInterpreterConfig_OWN_GIL
