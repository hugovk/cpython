...
TARGET(BINARY_SUBSCR_GETITEM) {
    ...
    // _BINARY_SUBSCR_CHECK_FUNC
    {
        ...
        DEOPT_IF(
            ((PyFunctionObject *)getitem)->func_version != cached_version, BINARY_SUBSCR
        );
        PyCodeObject *code = (PyCodeObject *)PyFunction_GET_CODE(getitem);
        assert(code->co_argcount == 2);
        DEOPT_IF(
            !_PyThreadState_HasStackSpace(tstate, code->co_framesize), BINARY_SUBSCR
        );
        STAT_INC(BINARY_SUBSCR, hit);
    }
...
TARGET(CALL_BOUND_METHOD_EXACT_ARGS) {
    ...
    // _CHECK_CALL_BOUND_METHOD_EXACT_ARGS
    {
        ...
        DEOPT_IF(!PyStackRef_IsNull(null[0]), CALL);
        DEOPT_IF(
            Py_TYPE(PyStackRef_AsPyObjectBorrow(callable[0])) != &PyMethod_Type, CALL
        );
    }
    ...
    // _CHECK_FUNCTION_EXACT_ARGS
    {
        ...
        DEOPT_IF(
            code->co_argcount != oparg + (!PyStackRef_IsNull(self_or_null[0])), CALL
        );
    }
    // _CHECK_STACK_SPACE
    {
...
TARGET(CALL_BUILTIN_FAST_WITH_KEYWORDS) {
    ...
    // _CALL_BUILTIN_FAST_WITH_KEYWORDS
    {
        ...
        DEOPT_IF(!PyCFunction_CheckExact(callable_o), CALL);
        DEOPT_IF(
            PyCFunction_GET_FLAGS(callable_o) != (METH_FASTCALL | METH_KEYWORDS), CALL
        );
        STAT_INC(CALL, hit);
...
TARGET(CALL_FUNCTION_EX) {
    ...
    // _DO_CALL_FUNCTION_EX
    {
        ...
        if (opcode == INSTRUMENTED_CALL_FUNCTION_EX) {
            ...
            if (oparg & 1)
                stack_pointer[-(oparg & 1)] = kwargs_st;
            _PyFrame_SetStackPointer(frame, stack_pointer);
            int err = _Py_call_instrumentation_2args(
...
TARGET(COMPARE_OP_STR) {
    ...
    // _COMPARE_OP_STR
    {
        ...
        res =
            ((COMPARISON_NOT_EQUALS + eq) & oparg) ? PyStackRef_True : PyStackRef_False;
        // It's always a bool, so we don't care about oparg & 16.
    }
    stack_pointer[-2] = res;
    stack_pointer += -1;
...
TARGET(CONTAINS_OP_SET) {
    ...
    PyObject *right_o = PyStackRef_AsPyObjectBorrow(right);
    DEOPT_IF(
        !(PySet_CheckExact(right_o) || PyFrozenSet_CheckExact(right_o)), CONTAINS_OP
    );
    STAT_INC(CONTAINS_OP, hit);
...
TARGET(FOR_ITER_LIST) {
    ...
    // _ITER_CHECK_LIST
    {
        iter = stack_pointer[-1];
        DEOPT_IF(
            Py_TYPE(PyStackRef_AsPyObjectBorrow(iter)) != &PyListIter_Type, FOR_ITER
        );
    }
    // _ITER_JUMP_LIST
    {
...
TARGET(FOR_ITER_TUPLE) {
    ...
    // _ITER_CHECK_TUPLE
    {
        iter = stack_pointer[-1];
        DEOPT_IF(
            Py_TYPE(PyStackRef_AsPyObjectBorrow(iter)) != &PyTupleIter_Type, FOR_ITER
        );
    }
    // _ITER_JUMP_TUPLE
    {
...
TARGET(INSTRUMENTED_INSTRUCTION) {
    ...
    _PyFrame_SetStackPointer(frame, stack_pointer);
    int next_opcode = _Py_call_instrumentation_instruction(tstate, frame, this_instr);
    stack_pointer = _PyFrame_GetStackPointer(frame);
    if (next_opcode < 0)
        goto error;
    next_instr = this_instr;
...
TARGET(INSTRUMENTED_LINE) {
    ...
    if (tstate->tracing) {
        ...
        _PyFrame_SetStackPointer(frame, stack_pointer);
        original_opcode =
            _Py_call_instrumentation_line(tstate, frame, this_instr, prev_instr);
        stack_pointer = _PyFrame_GetStackPointer(frame);
...
TARGET(LOAD_ATTR_METHOD_LAZY_DICT) {
    ...
    // _CHECK_ATTR_METHOD_LAZY_DICT
    {
        ...
        char *ptr = ((char *)PyStackRef_AsPyObjectBorrow(owner)) + MANAGED_DICT_OFFSET +
                    dictoffset;
...
TARGET(LOAD_ATTR_METHOD_WITH_VALUES) {
    ...
    // _GUARD_KEYS_VERSION
    {
        ...
        DEOPT_IF(
            owner_heap_type->ht_cached_keys->dk_version != keys_version, LOAD_ATTR
        );
    }
...
TARGET(LOAD_SUPER_ATTR) {
    ...
    // _LOAD_SUPER_ATTR
    {
        ...
        if (opcode == INSTRUMENTED_LOAD_SUPER_ATTR) {
            ...
            _PyFrame_SetStackPointer(frame, stack_pointer);
            int err = _Py_call_instrumentation_2args(
                tstate, PY_MONITORING_EVENT_CALL, frame, this_instr, global_super, arg
            );
            stack_pointer = _PyFrame_GetStackPointer(frame);
            if (err) {
