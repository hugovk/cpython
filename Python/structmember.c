...
PyMember_GetOne(const char *obj_addr, PyMemberDef *l) {
    ...
    if (l->flags & Py_RELATIVE_OFFSET) {
        PyErr_SetString(
            PyExc_SystemError, "PyMember_GetOne used with Py_RELATIVE_OFFSET"
        );
        return NULL;
    }
...
PyMember_SetOne(char *addr, PyMemberDef *l, PyObject *v) {
    ...
    if (l->flags & Py_RELATIVE_OFFSET) {
        PyErr_SetString(
            PyExc_SystemError, "PyMember_SetOne used with Py_RELATIVE_OFFSET"
        );
        return -1;
    }
    ...
    switch (l->type) {
        ...
        case Py_T_PYSSIZET: {
            *(Py_ssize_t *)addr = PyLong_AsSsize_t(v);
            if ((*(Py_ssize_t *)addr == (Py_ssize_t)-1) && PyErr_Occurred())
                return -1;
            break;
        }
        case Py_T_FLOAT: {
        ...
        default:
            PyErr_Format(PyExc_SystemError, "bad memberdescr type for %s", l->name);
            return -1;
    }
    return 0;
}
