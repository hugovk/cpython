...
same_location(location a, location b) {
    return a.lineno == b.lineno && a.end_lineno == b.end_lineno &&
           a.col_offset == b.col_offset && a.end_col_offset == b.end_col_offset;
}
    ...
    for (int i = 0; i < instrs->s_used; i++) {
        ...
        if (instr->i_except_handler_info.h_label != handler.h_label) {
            if (handler.h_label >= 0) {
                ...
                RETURN_IF_ERROR(assemble_emit_exception_table_entry(
                    a, start, ioffset, handler_offset, &handler
                ));
            }
            start = ioffset;
...
write_location_first_byte(struct assembler *a, int code, int length) {
    a->a_location_off += write_location_entry_start(location_pointer(a), code, length);
}
...
write_location_info_short_form(
    struct assembler *a, int length, int column, int end_column
) {
...
write_location_info_oneline_form(
    struct assembler *a, int length, int line_delta, int column, int end_column
) {
    assert(length > 0 && length <= 8);
...
static int
assemble_location_info(struct assembler *a, instr_sequence *instrs, int firstlineno) {
    a->a_lineno = firstlineno;
...
static int
assemble_emit(
    struct assembler *a, instr_sequence *instrs, int first_lineno, PyObject *const_cache
) {
...
extern void
_Py_set_localsplus_info(int, PyObject *, unsigned char, PyObject *, PyObject *);

static int
compute_localsplus_info(
    _PyCompile_CodeUnitMetadata *umd, int nlocalsplus, PyObject *names, PyObject *kinds
) {
    PyObject *k, *v;
    Py_ssize_t pos = 0;
...
makecode(
    ...
    if (compute_localsplus_info(umd, nlocalsplus, localsplusnames, localspluskinds) ==
        ERROR) {
        goto error;
    }
    ...
    for (int i = 0; i < instrs->s_used; i++) {
        ...
        switch (instr->i_opcode) {
            ...
            case JUMP_NO_INTERRUPT:
                ...
                instr->i_opcode =
                    is_forward ? JUMP_FORWARD : JUMP_BACKWARD_NO_INTERRUPT;
                break;
            default:
                if (OPCODE_HAS_JUMP(instr->i_opcode) &&
...
_PyAssemble_MakeCodeObject(
    ...
    if (res == SUCCESS) {
        co = makecode(
            umd, &a, const_cache, consts, maxdepth, nlocalsplus, code_flags, filename
        );
    }
    assemble_free(&a);
    return co;
