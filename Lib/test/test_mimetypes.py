...
class CommandLineTest(unittest.TestCase):
    def test_parse_args(self):
        ...
        args, help_text = mimetypes._parse_args(shlex.split("-l -e image/jpg"))
        self.assertTrue(args.extension)
        self.assertTrue(args.lenient)
        self.assertEqual(args.type, ["image/jpg"])

        args, help_text = mimetypes._parse_args(shlex.split("-e image/jpg"))
        self.assertTrue(args.extension)
        self.assertFalse(args.lenient)
        self.assertEqual(args.type, ["image/jpg"])

        args, help_text = mimetypes._parse_args(shlex.split("-l foo.webp"))
        self.assertFalse(args.extension)
        self.assertTrue(args.lenient)
        self.assertEqual(args.type, ["foo.webp"])

        args, help_text = mimetypes._parse_args(shlex.split("foo.pic"))
        self.assertFalse(args.extension)
        self.assertFalse(args.lenient)
