...
class HelpFormatter(object):
    ...
    def is_long_option(self, string):
        return len(string) >= 2 and string[1] in self._prefix_chars

    def is_short_option(self, string):
        return (
            not self.is_long_option(string)
            and len(string) >= 1
        ...
        for action in actions:
            ...
            # produce the first way to invoke the option in brackets
                ...
                if action.nargs == 0:
                    ...
                    if self.is_long_option(part):
                        part = f"{cyan}{part}{reset}"
                    elif self.is_short_option(part):
                ...
                #    -s ARGS or --long ARGS
                    ...
                    if self.is_long_option(option_string):
                        option_string = f"{cyan}{option_string}"
                    elif self.is_short_option(option_string):
    ...
    def _format_action_invocation(self, action, *, colorize=False):
        ...
        if not action.option_strings:
            ...
            def color_option_strings(strings):
                parts = []
                for s in strings:
                    if self.is_long_option(s):
                        parts.append(f"{bold_cyan}{s}{reset}")
                    elif self.is_short_option(s):
